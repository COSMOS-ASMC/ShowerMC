...
#include "Zmass.h"
!        
!          test muon nuclear interaction sampling 
!
       type(epmedia):: media
 ...
 10   continue
      ...
      write(msg,*)
     *  '# "muon nuc. int. sampling Emu=',Emu,' GeV" '
      write(*, *) msg
      if(norm .eq. 1) then
         normf = 1.
         write(msg,*)
     *      '# "Et/Emu" "log10(Et/Emu)" "sampled path(r.l)" '
     *                 // ' " prob/r.l"' 
      else
         normf =  media%mbtoPgrm/media%mbtoPX0
         write(msg,*)
     *   '# "Et/Emu" "log10(Et/Emu)" "sampled path(g/cm2)" '//
     *   ' " prob/(g/cm2)"' 
      endif
      write(*,*) msg
      do i = 1,  50000

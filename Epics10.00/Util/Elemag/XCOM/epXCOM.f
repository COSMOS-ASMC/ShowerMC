      PROGRAM XCOM
!
!       Version 3.1, 23 June 1999.  
!       
!       Copyright 1999, Martin J. Berger. Permission is given to
!       use the program for non-commercial purposes. 
!
!       A descripton of the program is available in National Bureau of
!       Standards Report NBSIR-87 by M. J. Berger and J. H. Hubbell. 
!       The authors can be reached at the National Institute of
!       Standards and Technology, Gaithersburg, MD 20899, Phone: (301)
!       975-5550.
!
!       XCOM3 calculates tables of cross sections for the interactions
!       of photons with any element, compound or mixture, for photons
!       with energies between 1 keV and 100 GeV.
!      
!       Files MDATX3.001, MDATX3.002,..., MDATX3.100 contain input data
!       for each or the elements. 
!
!       XCOM3 uses the following subroutines:
!       SPEC, FORM, MERGE, REV, SCOF, BSPOL and BLIN.
!
       CHARACTER PSOR*30,INPUT*10, XFIL2*30,SUB*72,
     1 IND(100)*3,FF*1
!///////
       character*72 XFIL1
!//////////
...
!////////
   ...
   10 FORMAT(1H )
      FF=CHAR(12)
!kk      PRINT *,' Program XCOM, Version 3.1'                             
      write(0,*)' Program XCOM, Version 3.1'                              
      write(0,*)' M.J.Berger and J.H.Hubbell, 23 June 1999'               
!kk      PRINT 10
      SUB=" "
  ...
  622 CONTINUE
!kk      PRINT *,' Specify file on which output (cross section table)'
!kk      PRINT *,' is to be stored. (Specification can include drive'
!kk      PRINT *,' and path): '
!kkkk      READ 5, XFIL1
!///////////
!      XFIL1=dir(1:ndir)//SUB(1:klena(SUB))//".xcom"
!///////
!      OPEN (UNIT=8,FILE=XFIL1)
  ...
  662 FORMAT(8X,'Constituents (Atomic Number:Fraction by Weight)')
      WRITE (*,665) (NZ(K),WEIGHT(K),K=1,KMAX)
...
! 792 FORMAT(9X,'(MeV)',3x,7('(b/atom) '))
  ...
  800 WRITE (*,10)
!kkk  805 ENN=EN(N)*(1.0E-06)
  805 ENN=EN(N)*1.0E-09
      IF(KZ(N))870,870,810
...
! 811 AT1=AT(N)-0.6022045*PHDIF(N)/ATWT
  ...
  815 WRITE(*,820) ENN,SCTCO(N),SCTIN(N),PHT1,
...
! 820 FORMAT(7X,1PE10.3,1P7E9.2)
  820 FORMAT(7X,1PE10.3,1P7E10.3)                                       SMS
      ...
      X(NN,8)=ATNC1
      NN=NN+1
      WRITE(*,840) KZ(N),ALAB(N),ENN,SCTCO(N),SCTIN(N),PHT(N),
...
! 840 FORMAT(1X,I2,1X,A2,1X,1PE10.3,1P7E9.2)
  ...
  860 JTAL=0
      GO TO 880
  870 WRITE (*,820) ENN,SCTCO(N),SCTIN(N),PHT(N),PRAT(N),PREL(N),
     1 AT(N),ATNC(N)
      X(NN,1)=ENN
      X(NN,2)=SCTCO(N)
      X(NN,3)=SCTIN(N)
  ...
  880 CONTINUE
      CLOSE (8)
!kk      PRINT 890, XFIL1
!kk  890 FORMAT(' Cross-section table with headings has been stored',
!kk     1' in file ',A)
!kk      PRINT *,' Options for further output:'
!kk      PRINT *,'     1. No more output'
!kk      PRINT *,'     2. Selected arrays stored on disk'
!kk      PRINT *,' Enter choice: '
!kk      READ *, NOUT
!//////
      NOUT=1
      stop
!///////
 ...
 1010 FORMAT(' Selected cross section arrays have been stored',
     1' on file ',A)
 1020 write(0,*)' Calculation is finished.'
      STOP
      END 

      SUBROUTINE SPEC(SUBST,NFORM,MMAX,JZ,WT,JING,EADD,NELL,SOURCE)
!     2 Apr 87. Reads various input parameters. 
      CHARACTER*72 FORMLA,FRM(100),SUBST
      CHARACTER*30 ENGIN,SOURCE
!////////
#include "Zmedia.h"
      integer nmatter, icon, kgetenv2,  nbasepath
      character*70  basepath
       type(epmedia)::  media
      character*2 symb
!//////
...
!kkkk     PRINT *,' Enter name of substance: '
!kkkk      READ 12, SUBST
!///////
      SUBST=" "
      nmatter = kgetenv2("MATTER", SUBST)
      if(nmatter .le. 0) then
         write(0,*) " basic matter not given"
         stop 1234
      endif
      nbasepath = kgetenv2("BASEPATH", basepath)
      if(nbasepath .le. 0 ) then
         write(0,*) " base media path  not given"
         stop 3456
      endif
      
!      SUBST=SUBST(1:nmatter)//" "
!      write(0,*) " nmatter=", nmatter
!//////////
!kk      PRINT 10
!kkk      PRINT *,' Options for characterization of substance:'
!kkk      PRINT *,'    1. Elemental substance, specified by atomic number'
!kkk      PRINT *,'    2. Elemental substance, specified by chemical symbol'
!kkk      PRINT *,'    3. Compound, specified by chemical formula'
!kkk      PRINT *,'    4. Mixture of elements and/or compounds'
!kkk      PRINT *,' Enter choice: '
!kkk      READ *,NSUB
!///////
      NSUB = 4
!kkk      PRINT 10
   ...
   40 CONTINUE
!     GO TO 150                                                         SMS
!        NOW THAT IN ASCENDING ORDER, COMBINE IDENTICAL Zs              SMS
      M=0                                                               SMS
   42 M=M+1                                                             SMS
   ...
   50 PRINT *,'Enter chemical formula for compound: '
      ...
      GO TO 25
   55 continue
!kkk      PRINT *,' How many components in mixture? Enter number: '
!kkk      READ *,NCOMP
!kkk      PRINT 10
!//////
      call copenfw2(55,
     *   basepath(1:nbasepath), 1, icon)
      if(icon .ne. 1) then
         write(0,*) ' basic matter file for ',SUBST,' not exists'
         stop 2345
      endif
      call epReadMTbl(55, media)
      NCOMP=media%noOfElem
      call epGetEffZA(media)
      write(0,*) media%name, ' No. of constituent atoms=', NCOMP
!/////////
      DO 65 N=1,NCOMP
!kkk      PRINT 60,N
!kkk   60 FORMAT(' Enter chemical symbol or formula for component',
!kkk     1 I3,': ')
!kkk      READ 12, FRM(N)
!//////
         FRM(N)=" "
         call epZ2ChemSymb(int(media%elem(N)%Z), symb)
         FRM(N)=symb
         write(0,*) symb, ' Z=', media%elem(N)%Z, ' W=',media%w(N)
!////////
!kkk      PRINT 61,N
!kkk   61 FORMAT(' Enter fraction by weight for component',I3,': ')
!kk      READ *, FRAC(N)
!///////
         FRAC(N)=media%w(N)
!///////
...
!kkkk     PRINT 10
   ...
   70 SUMF=SUMF+FRAC(N)
!kk      write(0,72)
!kk   72 FORMAT('   Component    Fraction')
!kk      write(0,73)
!kk   73 FORMAT('               by Weight')
!kkk      PRINT 10
!kk      DO 75 N=1,NCOMP
!kk         write(0, 74) N,FRAC(N),FRM(N)
!kk   74 FORMAT(I12,F12.6,6X,A)
!kk   75 CONTINUE
!kkk      PRINT 10
!kk       write(0, 80) SUMF
   80 FORMAT(6X,'Sum = ',F12.6)
!kkk      PRINT 10
!kkk      PRINT *,' Options for accepting or rejecting composition data:'
!kkk      PRINT *,'     1. Accept, but let program normalize fractions'
!kkk      PRINT *,'        by weight so that their sum is unity'
!kk      PRINT *,'     2. Reject, and enter different set of fractions'
!kk      PRINT *,' Enter choice: '
!kkk      READ *,MSUMGO
!kkk      PRINT 10
!/////
...
!////////
  ...
  140 CONTINUE
      MMAX=LL
  150 continue 
!kkk      PRINT *,' Options for energy list for output data:'
!kkk      PRINT *,'     1. Standard energy grid only'
!kkk      PRINT *,'     2. Standard grid plus additional energies'
!kkk      PRINT *,'     3. Additional energies only'
!kkk      PRINT *,' Enter choice: '
!kk      READ *,NELL
!kk     PRINT 10
!//////
...
!ccccccccccccccccccccccccccccc
  240 CONTINUE                                                          SMS
! 240 PRINT *,' Options for entry of Database files:'
!     PRINT *,'    1. Read files from floppy-disk drive A'
!     PRINT *,'    2. Read files from floppy-disk drive B'
!     PRINT *,'    3. Read files from CURRENT directory on hard disk'
!     PRINT *,' Enter choice: '
!     READ *, INDAT
!     GO TO (241,242,243),INDAT
! 241 SOURCE='A:MDATX3.'
!     RETURN
! 242 SOURCE='B:MDATX3.'
!     RETURN
...
!     24 Mar 87. Reads element symbols or chemical formulas.
      CHARACTER*72,W
...
!     16 Jun 99. Sorts into monotonically increasing order.             SMS
      DIMENSION E(1)                                                    SMS
...
!     24 Mar 87. Reverses the order of lists.
      DIMENSION X(1)
      NH=NMAX/2 
      DO 10 N=1,NH
      N1=NMAX-N+1 
...
!     22 Feb 83. Fits F as a function of X, and calculates 
!                cubic spline coefficients A,B,C and D.
...
!     22 Feb 83. Evaluates cubic spline as function of S, to obtain
!                fitted result G.
...
!     12 Apr 87. Linear interpolation routine

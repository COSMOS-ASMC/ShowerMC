...
!  hyperboloid
!                                        
!  circle center is at (0,0).  hight is directed to Z.
!      
!     (x/a)**2 + (y/b)**2 = 1 + (z/g)**2
!
...
!
!      
      subroutine eprhypb(comp)
       implicit none
#include "Zglobalc.h"
#include "Zep3Vec.h"
#include "Zcnfig.h"
!
!         interface to read configuration data for "hypb"
!
       ...
       real*8 a, b, g, h1,  h2
!
!           read hyperboloid data as 'new-*'
!           hypb has 5 volume attributes and the direction cosines
!           of the  h (1~6)
!
!             next is mandatory
...
!   ***************************************
      subroutine epbhypb(comp, pos, dir, length, icon)
       implicit none
#include "Zglobalc.h"
#include "ZepTrackp.h"
#include "Zep3Vec.h"
...
! 
       ...
       f(xx, yy, zz) = (xx/a)**2 + (yy/b)**2 -1. - (zz/g)**2
     ...
     *  (pos%x*dir%x/a**2 + pos%y*dir%y/b**2 - pos%z*dir%z/g**2)
      ...
      subroutine epshypb(comp, pos, icon)
      implicit none
#include "Zglobalc.h"
#include "Zep3Vec.h"
#include "Zcnfig.h"
...
!         
      ...
      integer icon  ! output. 0--> pos is inside
       ...
       elseif(pos%z .lt. h1) then
          icon = 1
       elseif(
     *    (pos%x/a)**2 + (pos%y/b)**2 -1. - (pos%z/g)**2 
     *      .gt. 0.)    then
          icon = 1
       else
          icon = 0
...
!     **************************************
      subroutine epenvlphypb(comp, org, abc)
      implicit none
...
!
       ...
       h2 = Volat( comp%vol + ih2)
       org%x =
     *  min(-sqrt(1.d0+ (h1/g)**2), -sqrt(1.d0+ (h2/g))**2)*a
       org%y =
     *  min(-sqrt(1.d0+ (h1/g)**2), -sqrt(1.d0+ (h2/g))**2)*b
       org%z = h1
       abc%x = -2*org%x
       abc%y = -2*org%y
       abc%z = h2 - h1
...
!     *************************************
      subroutine epatlochypb(comp, loc)
      implicit none
#include "Zep3Vec.h"
#include "Zcnfig.h"

...
!         been set).  Do your own final init. for the event
      end
...
!  #include "Zcode.h"     
      ...
      integer,intent(in):: info  !  0--> particle is still active.
                                 !  1--> particle is dying.
                                 !    (i.e, after moving Move.dl, it 
                                 !    dies) .
      type(epTrack),intent(in):: aTrack   ! current track. before it is moved.
      type(epmove),intent(inout)::  Move     !  containes info of moved track.
...
!  #include "Zcode.h"     
      ...
      integer,intent(in):: info   ! 0--> ptcl is exiting to void
...
#include "ZepPos.h"
     ...
     *         " first col point Z= ", firstP%z, " process=", proc
       ...
       do i = 1, Ncomp
!     Withouth counting energy deposit as Eloss counting in the user
!     program,  we can know the energy deposit in all the components
!     by the next call:
!       Edepo:    sum of the deposit in GeV.
!     EdepoEff:  Effective loss contributing to light emission.
          call epqEloss(i, Edepo, EdepoEff)
          write(*, '(i3,1p, 3g12.3)')  i, Eloss(i), Edepo, EdepoEff
       enddo

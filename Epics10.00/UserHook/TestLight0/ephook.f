...
c                         this event, and skip ue1ev.
      ...
      ElossT(aTrack.cn) =  ElossT(aTrack.cn) + Move.dEeff
      x = Move.Track.pos.x-50.0
      y = Move.Track.pos.y-50.0
      r = sqrt(x**2 + y**2)
      idxr = r/dr + 1
      if(idxr .le.  idxmax) then
c         Eloss(idxr, aTrack.cn) = Eloss(idxr,aTrack.cn) + Move.dEeff
      endif
      Move.Abort = 0
...
c  
        ...
        Move.Abort = 0
        if(info .gt. 0  .and. Move.Track.w.z .gt. 0. .and.
     *   Move.Track.p.fm.p(4) .gt. Eth )  then
           if(Move.Track.p.code .eq. 1) then
              Ng(Move.Track.cn) = Ng(Move.Track.cn)  + 1
           else
              Nec(Move.Track.cn) = Nec(Move.Track.cn)  + 1
           endif
        endif
        if(info .eq. 0 ) then
c             Move and aTrack has the same  info in this case
           if( aTrack.pos.x .gt. 9.99d0)  then
*              write(*,
*     *       '(i3,i3, i3, i3,1pg13.3,0p, 2f7.3, 2f8.4,f9.5)') 
*     *       Nev, aTrack.cn,
*     *       aTrack.p.code,  aTrack.p.charge,
*     *       aTrack.p.fm.p(4)- aTrack.p.mass,
*     *       aTrack.pos.x,   aTrack.pos.z,
...
#include "Zptcl.h"
        ...
        evn = evn +1
c        write(*,*) ' event #=', evn
        if( firstP.z .gt. 0.) then
c           call epqFirstColPtcls(ptcls, nptcl, mul)
c           do i = 1, mul
c              write(*,*) i, ptcls(i).code, ptcls(i).charge,
c     *         ptcls(i).fm.p(4)
c           enddo
c           write(*,*)

c           write(*,*) "f  media= ", firstM.name, " A,Z = ", 
c     *       firstM.colA, firstM.colZ, firstM.elem(firstM.colElem).A,
c     *       firstM.elem(firstM.colElem).Z,
c     *      " first col point Z= ", firstP.z, " process=", proc
        endif
        end
c       *************************************************
c       *  This is called when all the system level "end process"
...
c       * process for the events
      ...
      call epqevn(nevent)
      do i = 1, Ncomp
c         if(ElossT(i) .gt. 0.) then
         if(i .lt. Ncomp) then
            dEdt = (ElossT(i) + ElossT(i+1))/2.
         else
            dEdt = ElossT(i)
         endif
         write(*, '(i3, 1p, g12.4, i7, i7, i5)')
     *         i, dEdt, Ng(i), Nec(i), nevent
            do j = 1, idxmax
               r = j-0.5
c               write(*, '(i3,f7.2,1p,g12.3)')
c     *             i, r, Eloss(j, i)
            enddo   
c         endif
      enddo

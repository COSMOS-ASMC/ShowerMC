...
#include "Zptcl.h"
        ...
        real(4)::pc(2,6), pct(6)
        real(4)::edepo, ing(5), total, avew, tracedp
        ...
        do i = 1, Ncomp
           ...
           if(icon == 0 .and. total > 0. ) then
              write(0,'(i3, 1p,7g12.3,i3)') i, edepo, ing, total, icon
           endif
        enddo
        do i = 1, Ncomp
!               exiting
           call epqLightPC(i, 0,  pc,  pct, icon)
           if(icon == 0 ) then
              write(*,'(i6,a,1p, 6g10.3)' )  -i, '  scin ',  pc(1,:)
              write(*,'(i6,a,1p, 6g10.3)' )  -i,  ' Ceren ', pc(2,:)
              write(*,'(i6,a,1p,  6g10.3)' )  -i, '   sum ', pct(:)
           endif
!               entering
           call epqLightPC(i, 1,  pc,  pct, icon)
           if(icon == 0 ) then
              write(*,'(i6,a,1p, 6g10.3)' )  i, '  scin ',  pc(1,:)
              write(*,'(i6,a,1p, 6g10.3)' )  i,  ' Ceren ', pc(2,:)
              write(*,'(i6,a,1p,  6g10.3)' ) i, '   sum ', pct(:)
        ...
        do i = 1, Ncomp
           if(ElossT(i) > 0.) then
              write(*, '(i3, 1p, g12.4)')   i, ElossT(i)
           endif
        enddo

        call epqLightAveW(avew, tracedp)
        write(*,*) ' average weight of traced photons=',avew
        write(*,*) ' # of actually traced photons=', tracedp

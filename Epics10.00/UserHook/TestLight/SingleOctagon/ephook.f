...
#include "Zptcl.h"
        ...
        integer xyf   ! side face # for or PWO ... (smaller #)
        integer nevent
...
!              2 below is for scintillation and Cerenkov light
        ...
        do i = 1, Ncomp
           ...
           if(icon ==0 .and. total > 0) then
              if(first) then
                 write(*,*)  
     *          'comp#  dE   sinti     cheren    direct     sum'
                 first =.false.
              endif
     ...
     *         i, edepo, ing(1:2), ing(5), total
        ...
        do i = 1, Ncomp
           ...
           if(icon == 0  .and. sum(pct(:)) > 0.) then
                 ! we don't show count at the side wall;  for y-PWO
                 !  surface # 3 and 4  for x-PWO surface # 2 and 5
              !   
              call epqvolatr(i, na, vol)   !  get vol. attributes.  a,b,c of box
              if( vol(1) < 5.d0 ) then
                 ! boxa <  5cm so  this should be y-PWO
                 xyf = 3
              else
                 xyf = 2
              endif
              if(first) then
                 write(*,*) 
     *           'comp#  photon c @ surf',xyf, ' and ', 7-xyf
                 first = .false.
              endif
...
!          
        write(*,*) '<w>', avew, ' traced # of photons',tracedp
        write(*,*) 
        end
c       *************************************************
c       *  This is called when all the system level "end process"

...
!
!
  subroutine  kQinout3(p, n,  r, nv, sumteta, cond)
...
!    of the polygon or not.
    ...
    integer,intent(in) :: n    ! # of points composing the polygon.
                        ...
                        ! one of the edges.
    real(16),intent(in) :: p(3,n)  !  array containing x,y,z values of a polygon
    ...
    real(16),intent(in) :: r(3)    ! given point
    real(16),intent(in) :: nv(3)   ! normal vector of the surface
    real(16),intent(out):: sumteta  ! if the point is inside, and
    ...
    real(16)::  xmin, xmax,  ymin, ymax, zmin, zmax
...
!         point is inside otherwise, 0.
    ...
    real(16):: teta, absr1, absr2, para
    real(16):: cost
    real(16):: r1(3), r2(3),  r1xr2(3)
    integer:: i
    if(n <= 2  ) then
       ...
       do i = 1, n
          ...
          cost=max( min( cost, 1.q0), -1.q0)
          teta = acos(cost)
          if( dot_product( r1xr2, nv)  < 0.q0 ) then
             teta = -teta
          endif

 subroutine kQgetAngleBtw2Planes(i, x, p1, p2, p3, nv, teta, cond)
   implicit none
   ...
   real(16),intent(in):: p1(3)  !  vertex of a triangle
   real(16),intent(in):: p2(3)  !  vertex of the triangle
   real(16),intent(in):: p3(3)  !  vertex of the triangle
   real(16),intent(in):: x(3)   !  point to see p_i's
   real(16),intent(in):: nv(3)   !unit outwards going normal of
       ...
       ! polygon.
   real(16),intent(out):: teta   !   rad. angle obtained.
...
!           x
   ...
   real(16):: n1(3) 
   real(16),save:: n2(3)
   real(16),save::n1save(3)
   real(16):: n1xn2(3), r(3, 3), sumteta
   real(16):: ks, alpha, n1n2
   character(4)::condi
   integer:: jcon

   real(16),parameter::eps= 1.d-12

   cond = 'ok'
   if( i == 1 ) then
! normal vector of the  triangle surface formed by  x p1 p3 x   
      call epQ3p2plane0(x,   p1, p3, n1, ks, alpha, jcon)  !
...
! normal vector of the  triangle surface formed by  x p1 p2 x   
   if( i /= 3 ) then
      call epQ3p2plane0(x,  p1, p2, n2, ks, alpha, jcon)  ! now outwards
      if( jcon == 1 .or. jcon == 2 ) then
   ...
   n1n2 =max(min( dot_product(n1,n2), 1.q0), -1.q0)
   teta = acos(n1n2)
!/////////
!   if(bugbug2) then
!      write(0,*) ' n1=',n1(:)
...
!   the point is  exactly on the cap surface
   if( teta <1.q-10  .or. (abs(teta-pi) < eps )  ) then
      ...
      if(abs(sum(n1(:)+nv(:))) < eps .or. &
         ...
         r(:,2) =  p2(:)
         r(:,3) =  p3(:)
         call kQinout3(r, 3, x, n1, sumteta, condi)
!///////////
!         if(bugbug2) then
...
!        vector product of n1, n2
   call epQvectorProd(n1, n2,  n1xn2)
...
!     then,  teta should be  2pi - teta
   ...
   cond = 'ok'
 end subroutine kQgetAngleBtw2Planes

 subroutine kQProjAReaOfTriang(p1, p2, p3, x, nv, teta, area, cond)
...
!     formed by x and p_i's 
   ...
   real(16),intent(in):: x(3)   ! point to be judeged
   real(16),intent(inout):: nv(3)   ! outwards going normal
   ...
   real(16),intent(out):: teta(3)  !  angle between two surfaces 
                  !   having the common line x-pi.
   real(16),intent(out):: area  ! signed  area
...
!          formed by p1,p2,p3
      call epQ3p2plane0(p1, p2, p3, nv, ks, alpha, jcon)
      if( jcon /= 0 ) then
         cond = 'ng'
         area = 0.
         return
   ...
   call kQgetAngleBtw2Planes(1, x, p1, p2, p3, nv, teta(1), condi)
!///////
!   if(bugbug2) then
...
!////////
   ...
   call kQgetAngleBtw2Planes(2, x, p2, p3, p1, nv, teta(2), condi)
!///////
!   if(bugbug2) then
...
!////////
   ...
   cond = 'ok'
 end subroutine kQProjAReaOfTriang

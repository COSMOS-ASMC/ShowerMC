...
!      compute the coefficients of the cubic spline
      ...
      real(4):: coef(nc, 3)  ! output. obtained coeff.
      ...
      real(4):: a2, a1, x1, x2, x3, h, hy, dy1, dy2, y1, y2, hh, h1, h2
      real(4):: pinv
!
      if (n .lt. 2 .or. nc .lt. n-1 ) then
         write(0,*) ' n=', n, ' nc=',nc
         write(0,*) " if (n .lt. 2 .or. nc .lt. n-1 ) wrong"
         call cerrorMsg('input is wrong for kScslpCoef',0)
      endif
     ...
     *     'x must be ascending: kScsplCoef',1)
      ...
      if (n .ne. 2) then
         ...
         do i = 2, n-1
            ...
            hh = h1 + h2
            coef(i,1) = h2 / hh
            coef(i,2) = 1.0 - coef(i,1)
            coef(i,3) = 6.0 * (y2 / h2 - y1 / h1) / hh
            h1 = h2
            y1 = y2
         enddo
      endif

  subroutine kSNewtonRaphson(func, dfunc, param, x0, eps, ans, icon)
    implicit none
   ! solve  func(x, param) = 0 
    real(4):: func, dfunc  ! external function. with two  arguments
               ! dfunc is derivative func(x,param)  defined dfunc(x, param)
               ! with the same param.
    real(4),intent(in)::param(*) ! some possible parameter  needed to compute
               !         func and dfunc.  if no param is needed. give dummy
    real(4),intent(in):: x0   ! first guess of root
    real(4),intent(in):: eps   ! such as 1d-6. abso or relative error of root
                            !  if > 0, |func(root)| < eps is required
                            !  if < 0, |(x1-x2)/x2| < |eps| is required if
                            !           |x2|>1.  else |x1-x2| < |eps|
                            !For,  eps>0, to solve f(x) = c,
                            ! better to use func= f(x)/c -1
    real(4),intent(out):: ans  ! obtained root
...
!       x2 = -func(x1, param)/tang + x1
    ...
    enddo
    ans = x2
  end subroutine kSNewtonRaphson
!
!  program test
!    implicit none
!    real(4):: muave, param(1)
!    real(4),external:: myfunc, mydfunc
!    real(4)::a, eps, ans
!    integer::icon
!
!
...
!  implicit none
!!
!  real(4):: f  ! input. function name. to be used as f(x, param)
!  !   f(x, param) = 0 is solved.
!  real(4),intent(in):: param(*)
!  real(4),intent(in):: a ! input. lower bound of solution
!  real(4),intent(in):: b ! input. upper bound of solution
!  real(4),intent(in):: x   ! input. initial guess of  solution.  
!  real(4),intent(in):: eps  ! input. relative error of solution.
!  real(4),intent(out):: ans  ! output. obtained solution

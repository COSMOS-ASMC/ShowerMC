...
! Sorts anarray ra of length N into ascending numerical order using the
! heapsort algorithm. N is input: RA is replaced on output by its sorted 
! rearrangement
      DIMENSION RA(N)
      L=N/2+1
      IR=N
! The index L will be decremented from the inital value down to 1 during
! the "hiring" (heap creation) phase. Once it reaches 1, the index IR 
! will be decremented from its inital value down to 1 during the 

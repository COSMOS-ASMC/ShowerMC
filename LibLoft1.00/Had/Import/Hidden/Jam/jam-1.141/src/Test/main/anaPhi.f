c....Analysis program produced from mainPhi.f

      include 'jam1.inc'
      include 'jam2.inc'
      common/myana/ispec

c     open(33,file='phase.dat',form='unformatted',status='old')
      open(33,file='phase.dat',status='old')
...
cif(abs(ee-p(4,i)).gt.1d-7) then
c  print *,'ee p4',ee,p(4,i),kf
cendif
        pt=max(pt,1.d-8)
...
c         vr=sqrt(v(1,i)**2+v(2,i)**2)
          ...
          vt=r(4,i) 
c         if(vt.lt.abs(r(3,i))) then
c           print *,'tau',kf,v(4,i),v(3,i),r(4,i),r(3,i)
c         endif

!     this is minimum  modification of testcol in
!     Particle/Event/Test/testcol.f for testing qgsjet01
!     which is not included in COSMOS.
!
#include "ZcosmosBD.h"
...
!           defalut trace.  fix the directory
         else
            if(TraceDir .eq. ' ') then
               call cgetLoginN(uid)
...
!|||                call chAcol(pj, ia, iz, a, ntp0)
                call qgs01event(pj, ia, iz, a, ntp0)
             else
...
!                   if( xcms .gt. 0. .and.
!     *                a(j).code .eq. 1) then
!                      if( a(j).fm.p(4) .gt. 80.) then
!c                      if(pt/a(j).fm.p(4)*14000.0 .lt. 11.) then
!                         write(*,*) sngl( a(j).fm.p(4)),
!     *                      sngl(pt/a(j).fm.p(4)*14000.0),
!     *                      ptcms
!                      endif
!                   endif
!                   if(xcms .gt. 1.e-4) then
!                      write(*, '(2i3, 2g12.3)')
!     *                a(j).code, a(j).charge, sngl(xcms), sngl(x)
!                   endif   
!                   write(*, '(7g14.4, 3i3, i6, 2g16.6, 2g14.3, i8)' )
!                   write(*, '(2g14.4, 3i3, i6)')
!     *               sngl(x), pt, a(j).code, a(j).subcode, a(j).charge,
!     *               ntp
                   write(*, '(7g14.3, 3i3, i6, 2g16.6, 2g14.3, i8)' )

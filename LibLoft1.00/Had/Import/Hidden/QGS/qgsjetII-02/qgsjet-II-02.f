...
c      moniou=0             !output channel for debugging
c      debug=1              !debugging level 
...
c soft Pomeron parameters
       dels=.18d0          !overcriticality
       alfp=.095d0         !trajectory slope
       rq(1)=1.d0          !vertex slope (pi)
       cd(1,1)=1.6d0       !relative strenth of the 1st diffr. eigenst. (pi)
       cd(2,1)=.4d0        !relative strenth of the 2nd diffr. eigenst. (pi)
       rq(2)=2.1d0         !vertex slope (p)
       cd(1,2)=1.5d0       !relative strenth of the 1st diffr. eigenst. (p)
       cd(2,2)=.5d0        !relative strenth of the 2nd diffr. eigenst. (p)
       rq(3)=.6d0          !vertex slope (kaon)
       cd(1,3)=1.65d0      !relative strenth of the 1st diffr. eigenst. (kaon)
       cd(2,3)=.35d0       !relative strenth of the 2nd diffr. eigenst. (kaon)
       sigs=1.3d0          !soft parton cross section
       alpd=0.d0           !Pomeron end exponent
       r3p=.0175d0         !triple-Pomeron vertex
       g3p=.6d0            !slope of multi-Pomeron vertex         
       almpt=1.3d0         !string fragmentation parameter
c hard Pomeron parameters
       qt0=2.5d0           !q**2 cutoff
       factk=1.5d0         !k-factor value
       betp=1.3d0          !gluon structure function parameter for soft P
       fqscal=4.d0         !factor for Q^2-scale (Q^2=p_t^2/fqscal)
       dgqq=.24d0          !sea quark contribution
       alm=.04d0           !lambda_QCD squared
       qtf=.5d0            !p_t**2 resolution scale
...
c (4-5) - for diquark (ud, u~d~) fragmentation
      ...
      wwm=1.d0             !mass threshold for 3-particle string decay  
...
c-----------------------------------------------------------------------------
     ...
     *,evs(40,100,3,2),ixemax(40,3,2),qfan0(51,11,7),fann(7)
     *,gz0(5),gz1(3),wa(3)
     ...
     *,cr1(2),cr2(2),cr3(2)
      ...
      common /qgarr19/ ahl(3)
      common /qgarr20/ spmax
      common /qgarr24/ qpomr(11,11,6,650,12)
      common /qgarr25/ ahv(3)
      common /qgarr26/ factk,fqscal
      common /qgarr27/ qlegi(51,11,11,3,3),qfan(51,11,11,217,6)
      common /qgarr28/ arr(5)
...
c-------------------------------------------------
 ...
 100  format(' ',
     ...
     *     /,' ','| S. Ostapchenko, hep-ph/0412332, hep-ph/0501093   |',
     *     /,' ','|                                                  |',
     *     /,' ','| last modification:  12.04.2005                   |',
...
c///////////
      ...
      inquire(file=file1, exist=lcalc)
ccc      inquire(file='qgsdat-II-1',exist=lcalc)
c//////////
      if(lcalc)then
       if(debug.ge.1)write (moniou,203)
       open(1,file=file1, status='old')
c       open(1,file='qgsdat-II-1',status='old')
cc       //// kk
cc       read (1,*)csborn,cs0,cstot,evk,qppdi,qlegi,qfan
cc     * ,qpomr,qlegc,qpomc,fsud,qrt,qpomi,gsect,qpomi0
       read(1, *) csborn
       read(1, *) cs0
       read(1, *) cstot
       read(1, *) evk
...
c integrated Pomeron leg eikonals
      ...
      do iv=1,11
       ...
       qxl=qgleg(sy,b*b,vvx,1,icz,0)
       if(iz.eq.1)then
        qlegi(iy,iz,iv,1,icz)=dlog(qxl)
       else
        if(qxl.gt.0.d0)then
         qlegi(iy,iz,iv,1,icz)=log(qxl/z)
        else
         qlegi(iy,iz,iv,1,icz)=qlegi(iy,iz,iv-1,1,icz)
        endif
       endif
      enddo
      enddo
...
c integrated fan contributions
      ...
      do iv=1,11
       ...
       do iz=1,11 
        qfan(iy,iz,iv,1,icdp+2*(icz-1))=qlegi(iy,iz,iv,1,icz)
     *  +log(cd(icdp,icz))
       enddo
       enddo      
       do iy=2,51
        ...
        do iz=2,11 
         qfan(iy,iz,iv,1,icdp+2*(icz-1))
     *   =qfan(iy-1,iz,iv,1,icdp+2*(icz-1))
        enddo
...
3       n=n+1                                    !number of t-channel iterations
	 ...
	 if(abs(qfan0(iy,iz,1)-qfan(iy,iz,iv,1,icdp+2*(icz-1)))
     *   .gt.1.d-3)nrep=1
        enddo       
        do iz=2,11 
         qfan(iy,iz,iv,1,icdp+2*(icz-1))=qfan0(iy,iz,1)
        enddo
        if(nrep.eq.1)goto 3
       enddo
      enddo
...
c integrated cut fan contributions
      ...
      do icdp=1,2                                 !diffractive eigenstate
      do iv=1,11
       vvxt=dble(iv-1)/10.d0             !screening corrections from target fans
      do iv1=1,6
       vvxp=dble(iv1-1)/5.d0   !screening corrections from proj. fans
                               !(except the current nucleon; including cut fans)
      do iv2=1,6
       vvxpa=dble(iv2-1)/5.d0  !all screening corrections from proj. fans
                               !minus ones in vvxp
       do iy=1,51
        sy=spmax**((iy-1)/50.d0)                  !Pomeron mass squared
        rp=(rq(icz)+alfp*log(sy))*4.d0*.0389d0
       do iz=1,11 
        ...
        do iqq=2,3                                !initialization
	 qfan(iy,iz,iv,1+iv1+6*(iv2-1)+36*(iqq-2),icdp+2*(icz-1))
     *   =qfan(iy,iz,iv,1,icdp+2*(icz-1))
	enddo
        do iqq=4,6
	 qfan(iy,iz,iv,1+iv1+6*(iv2-1)+36*(iqq-2),icdp+2*(icz-1))
     *   =qlegi(iy,iz,iv,1,icz)+dlog(cd(icdp,icz))
	enddo
       enddo
       enddo
       ...
       do iy=2,51
        ...
        do iz=2,11 
         qfan(iy,iz,iv,1+iv1+6*(iv2-1)+36*(iqq-2),icdp+2*(icz-1))
     *   =qfan(iy-1,iz,iv,1+iv1+6*(iv2-1)+36*(iqq-2),icdp+2*(icz-1))
        enddo
        enddo
...
4       n=n+1                                    !number of t-channel iterations
        ...
        do iz=2,11  
         ...
         call qgfan1(fann,sy,b*b,vvxt,vvxp,vvxpa,icdp,icz)
         do iqq=2,6
	  ...
	  if(abs(qfan0(iy,iz,iqq)-qfan(iy,iz,iv,1+iv1+6*(iv2-1)
     *    +36*(iqq-2),icdp+2*(icz-1))).gt.1.d-3)nrep=1
         enddo
        enddo      
        do iqq=2,6
        do iz=2,11 
...
c integrated cut Pomeron eikonals
     ...
     * /gamfun(real(2.d0-alpd+dels+ahl(2)))
       ...
       do icdt=1,2                              !targ.diffractive eigenstate       
        if(iz.eq.1)then
         qpomr(iy,iz,1,1,icdp+2*(icdt-1)+4*(icz-1))
     *   =log(vsoft*cd(icdp,icz)*cd(icdt,2))
         do iv=1,6
          qpomr(iy,iz,iv,2,icdp+2*(icdt-1)+4*(icz-1))
     *    =qpomr(iy,iz,1,1,icdp+2*(icdt-1)+4*(icz-1))
         do iv1=1,6
         do iv2=1,6
         do iv3=1,6
          qpomr(iy,iz,iv,2+iv1+6*(iv2-1)+36*(iv3-1)
     *    ,icdp+2*(icdt-1)+4*(icz-1))=-100.d0
          qpomr(iy,iz,iv,218+iv1+6*(iv2-1)+36*(iv3-1)
     *    ,icdp+2*(icdt-1)+4*(icz-1))=-100.d0
          qpomr(iy,iz,iv,434+iv1+6*(iv2-1)+36*(iv3-1)
     *    ,icdp+2*(icdt-1)+4*(icz-1))=-100.d0
	 enddo
	 enddo
	 enddo
	 enddo
         ...
         qxp=qg3pom(sy,b,icdp,icdt,icz)
         qxp=qxp+(vsoft*z+vgg+vqg+vgq+vqq)*cd(icdp,icz)*cd(icdt,2)
	                                         !total uncut eikonal
         ...
         do iv=1,6  
          ...
          qpomr(iy,iz,iv,2,icdp+2*(icdt-1)+4*(icz-1))=log(qxp1/z)
	 
         do iv1=1,5                 !screening factors for h-A (AA) interactions
          vvxt=(iv1-1)/5.d0
         do iv2=1,6
          vvxpa=(iv2-1)/5.d0
         do iv3=1,6
          ...
          if(iv.ne.6.and.iv1.ne.6)then
           v3p=qg3pa(sy,b,vvxp,vvxt,vvxpa,vvxta,icdp,icdt,icz,2) 
	                                 !contribution of multi-Pomeron vertexes
	   vdtg=qg3pa(sy,b,vvxp,vvxt,vvxpa,vvxta,icdp,icdt,icz,3)  !targ. diffr.
	   vdpr=qg3pa(sy,b,vvxp,vvxt,vvxpa,vvxta,icdp,icdt,icz,4)  !proj. diffr.
           if(v3p.gt.0.d0)then
            qpomr(iy,iz,iv,2+iv1+6*(iv2-1)+36*(iv3-1)
     *      ,icdp+2*(icdt-1)+4*(icz-1))=log(v3p/z)
           else
            qpomr(iy,iz,iv,2+iv1+6*(iv2-1)+36*(iv3-1),icdp+2*(icdt-1)
     *      +4*(icz-1))=qpomr(iy,iz-1,iv,2+iv1+6*(iv2-1)+36*(iv3-1)
     *      ,icdp+2*(icdt-1)+4*(icz-1))
	   endif
           if(vdtg.gt.0.d0)then
            qpomr(iy,iz,iv,218+iv1+6*(iv2-1)+36*(iv3-1)
     *      ,icdp+2*(icdt-1)+4*(icz-1))=log(vdtg/z)
           else
            qpomr(iy,iz,iv,218+iv1+6*(iv2-1)+36*(iv3-1),icdp+2*(icdt-1)
     *      +4*(icz-1))=qpomr(iy,iz-1,iv,218+iv1+6*(iv2-1)+36*(iv3-1)
     *      ,icdp+2*(icdt-1)+4*(icz-1))
	   endif
           if(vdpr.gt.0.d0)then
            qpomr(iy,iz,iv,434+iv1+6*(iv2-1)+36*(iv3-1)
     *      ,icdp+2*(icdt-1)+4*(icz-1))=log(vdpr/z)
           else
            qpomr(iy,iz,iv,434+iv1+6*(iv2-1)+36*(iv3-1),icdp+2*(icdt-1)
     *      +4*(icz-1))=qpomr(iy,iz-1,iv,434+iv1+6*(iv2-1)+36*(iv3-1)
     *      ,icdp+2*(icdt-1)+4*(icz-1))
	   endif
     
           if(iv.eq.1.and.iv1.eq.1.and.iv2.eq.1.and.iv3.eq.1)then
	    qxp2=qxp1+v3p+vdtg+vdpr                           !total cut eikonal
            qpomr(iy,iz,1,1,icdp+2*(icdt-1)+4*(icz-1))=log(qxp2/z)
            if(debug.ge.2)write(moniou,217)ty(icz)
     *      ,e0n,icdp,icdt,qxp2,qxp1,v3p,vdtg,vdpr  
	   endif
	  endif
         enddo
         enddo
...
c nuclear densities
	  ...
	  wbnuc(2)=twbnuc(ia(2))                   !"wine-bottle" parameter
          ...
          rnuc(2)=0.7d0*float(ia(2))**.446         !nuclear radius
...
c cut Pomeron eikonals (soft and total)
     ...
     *   =log(qpt/sy**dels*(xp*xm)**alpd/z)
      ...
      open(1,file='qgsdat-II-1',status='unknown')
cc        ///// kk
cc      write  (1,*)csborn,cs0,cstot,evk,qppdi,qlegi,qfan
cc     *,qpomr,qlegc,qpomc,fsud,qrt,qpomi,gsect,qpomi0
      write(1, *) csborn
      write(1, *) cs0
      write(1, *) cstot
...
c////////  KK
      inquire(file=file2,exist=lcalc)
ccc      inquire(file='sectnu-II-1',exist=lcalc)
c/////////
      if(lcalc)then
       if(debug.ge.1)write (moniou,228)       
ccc ////// kk
       open(2,file=file2, status='old')
cc       open(2,file='sectnu-II-1',status='old')
       read (2,*)asect
       close(2)
	...
	elseif(iia2.eq.5)then
	 ...
	 iat=40
       ...
       enddo
       enddo
       open(2,file='sectnu-II-1',status='unknown')
       write (2,*)asect
       close(2)
      endif
...
c nuclear radii and weights for sampling nuclear configurations
	 ...
	 wbnuc(i)=twbnuc(ia(i))               !"wine-bottle" parameter
         ...
         rnuc(i)=0.7d0*float(ia(i))**.446     !nuclear radius
...
c=============================================================================
      double precision function qgleg(sy,bb,vvx,icdp,icz,jj)
c-----------------------------------------------------------------------------
...
c-----------------------------------------------------------------------------
      ...
      if(debug.ge.3)write (moniou,201)sy,bb,vvx,icdp,icz,jj
      qgleg=0.d0
     ...
     * **(1./(1.+dels-alpd))
       ...
       ws=qgls(xp*sy,xp,bb,vvx,icdp,icz,jj)/xp**alpd       !soft Pomeron
       if(bb.gt.1.d9)then    !asymptotics for b->infty
        wg=0.d0
	wq=0.d0
       else
        wg=qglsh(xp*sy,xp,bb,vvx,icdp,icz,0,jj)/xp**alpd   !g-Pomeron
        wq=qglsh(xp*sy,xp,bb,vvx,icdp,icz,1,jj)/dsqrt(xp)  !q-Pomeron
...
201   format(2x,'qgleg - Pomeron leg eikonal:'/4x,'s=',e10.3,2x,'b^2='
     *,e10.3,2x,'vvx=',e10.3,2x,'icdp=',i1,2x,'icz=',i1,2x,'jj=',i1)
...
c------------------------------------------------------------------------
      double precision function qglegi(sy,bb,vvx,icdp,icz,jj)
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
      ...
      do k1=1,iymax
       k2=k+k1-1
       qglegi=qglegi+qlegi(k2,l2,j2,1+jj*icdp,icz)*wk(k1)*wz(l1)*wj(j1)
      enddo
      enddo
      enddo
      qglegi=exp(qglegi)*z
     **(1.d0-(1.d0-(1.d0-1.d0/sy)**(1.+ahl(icz)))**(1.+dels-alpd))
      if(jj.eq.0)qglegi=qglegi*cd(icdp,icz)
...
c-----------------------------------------------------------------------
      ...
      qgfan=qglegi(sy,bb,vvx,icdp,icz,0)
...
c------------------------------------------------------------------------
      subroutine qgfan1(fann,sy,bb,vvxt,vvxp,vvxpa,icdp,icz)
c-----------------------------------------------------------------------
c qgfan1 - integrated cut fan-contributions
c sy    - Pomeron mass squared,
c bb    - squared impact parameter,
c vvxt  - screening corrections from target fans,
c vvxp  - screening corrections from proj. fans 
c         (based on sum_{i<=ip} V^F(cut)_i - sum_i V^F(uncut)_i})
c vvxpa - other screening corrections from proj. fans 
c         (based on sum_{i>ip} V^F(cut)_i)
c icdp  - diffractive eigenstate,
c icz   - hadron class,
c iqq=2 - total cut,
c iqq=3 - without uncut end,
c iqq=4 - 1-Pomeron contribution (screened); "holes" included,
c iqq=5 - 1-Pomeron contribution without uncut end
c iqq=6 - 1-Pomeron contribution without "holes"
c-----------------------------------------------------------------------
      implicit double precision (a-h,o-z)
      integer debug
      dimension fann(6)
      ...
      if(debug.ge.3)write (moniou,201)sy,bb,vvxt,vvxp,vvxpa,icdp,icz,iqq
      do iqq=2,6
       fann(iqq)=0.d0
      enddo      
	...
	bb2=(dsqrt(bb)-dsqrt(bb1)*cos(phi))**2+bb1*sin(phi)**2
                                            !impact parameter for multi-P vertex       
        v1p=qgfani(1.d0/xpomr1,bb2,1.d0-(1.d0-vvxt)*(1.d0-vvxp) !uncut proj. fan
     *  *(1.d0-vvxpa),0.d0,0.d0,icdp,icz,1)
	v1pc=qgfani(1.d0/xpomr1,bb2,vvxt,vvxp,vvxpa,icdp,icz,2)   !cut proj. fan
	v1pc0=min(v1pc,qgfani(1.d0/xpomr1,bb2,vvxt,vvxp,vvxpa
     *  ,icdp,icz,3))                                         !without uncut end
	v1pcl=qgfani(1.d0/xpomr1,bb2,vvxt,vvxp,vvxpa,icdp,icz,4)        !cut leg
	v1pcl0=min(v1pcl,qgfani(1.d0/xpomr1,bb2,vvxt,vvxp,vvxpa
     *  ,icdp,icz,5))                                 !cut leg without uncut end
        v1p1=min(v1pcl,qgfani(1.d0/xpomr1,bb2,vvxt,vvxp,vvxpa
     *  ,icdp,icz,6))                                   !cut leg without "holes"
        vvxi1=1.d0-(1.d0-vvxt)*((1.d0-vvxp)*(1.d0-vvxpa))**2 !cut intermediate P
        vvxi2=1.d0-(1.d0-vvxt)**2*(1.d0-vvxp)*(1.d0-vvxpa)
        vvxi3=1.d0-((1.d0-vvxt)*(1.d0-vvxp)*(1.d0-vvxpa))**2
        v1i1=qgpini(xpomr1*sy,bb1,vvxi1)                      !screening factors
	v1i2=qgpini(xpomr1*sy,bb1,vvxi2)
	v1i3=qgpini(xpomr1*sy,bb1,vvxi3)
	
	do iqq=2,6
         if(iqq.eq.2)then                                         !total cut fan 
	  dpx=v1i1*(1.d0-vvxt)*((1.d0-exp(-v1p))**2*(1.d0-vvxpa)**2+1.d0
     *    -exp(-2.d0*v1pc)-2.d0*v1pc*(1.d0-vvxpa)**2)*(1.d0-vvxp)**2
         elseif(iqq.eq.3)then                         !cut fan without uncut end
          dpx=v1i1*(1.d0-vvxt)*((1.d0-exp(-v1p))**2*(1.d0-vvxpa)**2+1.d0
     *    -exp(-2.d0*v1pc)-2.d0*v1pc*(1.d0-vvxpa)**2)*(1.d0-vvxp)**2
     *    -v1i2*(vvxt*(1.d0-vvxt)*((1.d0-exp(-v1p))**2*(1.d0-vvxpa)**2
     *    +1.d0-exp(-2.d0*v1pc))*(1.d0-vvxp)**2
     *    +2.d0*(1.d0-vvxt)**2*(1.d0-vvxp)*(1.d0-vvxpa)
     *    *((exp(v1pc-v1pc0)-1.d0)*exp(-v1p)-(v1pc-v1pc0)))
         elseif(iqq.eq.4)then     !1-Pomeron contr. (screened); "holes" included
          dpx=2.d0*v1i1*(1.d0-vvxt)*v1pcl*(exp(-2.d0*v1p)-1.d0)
     *    *(1.d0-vvxp)**2*(1.d0-vvxpa)**2
         elseif(iqq.eq.5)then          !1-Pomeron contribution without uncut end
          dpx=2.d0*v1i1*(1.d0-vvxt)*v1pcl*(exp(-2.d0*v1p)-1.d0)
     *    *(1.d0-vvxp)**2*(1.d0-vvxpa)**2
     *    -2.d0*v1i2*(vvxt*(1.d0-vvxt)*v1pcl*exp(-2.d0*v1p)
     *    *(1.d0-vvxp)**2*(1.d0-vvxpa)**2+(1.d0-vvxt)**2
     *    *(v1pcl-v1pcl0)*(exp(-v1p)-1.d0)*(1.d0-vvxp)*(1.d0-vvxpa))
         elseif(iqq.eq.6)then            !1-Pomeron contribution without "holes"
          dpx=2.d0*v1i3*((1.d0-vvxt)*(1.d0-vvxp)*(1.d0-vvxpa))**2
     *    *v1p1*(exp(-2.d0*v1p)-1.d0)
         endif     
...
1     continue
      do iqq=2,6                                           !add leg contribution
       fann(iqq)=fann(iqq)*dlog(sy)*pi*r3p/g3p*sigs/4.d0
       if(iqq.lt.6)then
        fann(iqq)=fann(iqq)+qglegi(sy,bb
     *  ,1.d0-((1.d0-vvxp)*(1.d0-vvxpa))**2*(1.d0-vvxt),icdp,icz,0)
       elseif(iqq.eq.6)then
        fann(iqq)=fann(iqq)+qglegi(sy,bb
     *  ,1.d0-((1.d0-vvxp)*(1.d0-vvxpa)*(1.d0-vvxt))**2,icdp,icz,0)
       endif
       fann(iqq)=fann(iqq)/(1.d0-(1.d0-(1.d0-1.d0/sy)**(1.+ahl(icz)))
     * **(1.+dels-alpd))
      enddo
      if(debug.ge.4)write (moniou,202)
     
201   format(2x,'qgfan1 - integrated cut fan-contributions:'
     */4x,'s=',e10.3,2x,'b^2=',e10.3,2x,'vvxt=',e10.3,2x
     */4x,'vvxp=',e10.3,'vvxpa=',e10.3
...
c------------------------------------------------------------------------
      double precision function qgfani(sy,bb,vvxt,vvxp,vvxpa
     *,icdp,icz,iqq)
c-----------------------------------------------------------------------
...
c icz   - hadron class,
c iqq=1 - uncut fan,
c iqq=2 - total cut fan,
c iqq=3 - without uncut end ("hole"),
c iqq=4 - 1-Pomeron contribution (screened); "holes" included,
c iqq=5 - 1-Pomeron contribution without uncut end
c-----------------------------------------------------------------------
      implicit double precision (a-h,o-z)
      ...
      if(iqq.eq.1)then                               !uncut fan
       ...
       do k1=1,iymax
        k2=k+k1-1
        qgfani=qgfani+qfan(k2,l2,j2,1,icdp+2*(icz-1))
     *  *wk(k1)*wz(l1)*wj(j1)
       enddo
       enddo
       enddo
      else
       if(vvxt.eq.1.d0)then                          !full screening
        ivmax=1
        iv1max=1
        iv2max=1
	i=1
	...
	j=11
        ...
        wj(1)=1.d0
        wn(1)=1.d0
       elseif(vvxp.eq.0.d0.and.vvxpa.eq.0.d0)then    !hadron (no nuclear graphs)
        iv1max=1
        iv2max=1
	i=1
	n=1
       ...
       do k1=1,iymax
        k2=k+k1-1
        qgfani=qgfani+qfan(k2,l2,j2,i+i1+6*(n+n1-2)+36*(iqq-2)
     *  ,icdp+2*(icz-1))*wk(k1)*wz(l1)*wj(j1)*wi(i1)*wn(n1)
       enddo
       enddo
       enddo
       enddo
...
c------------------------------------------------------------------------
      ...
      do mx1=1,2
       ...
       v1p1=qglegi(1.d0/xpomr1,bb1,0.d0,icdp,icz,0)        !unscreened proj. leg
       v1t1=qglegi(xpomr1*sy,bb2,0.d0,icdt,2,0)            !unscreened targ. leg
...
c------------------------------------------------------------------------
     ...
     *,icdp,icdt,icz
      ...
      do mx1=1,2
       ...
       vpau0=qgfani(1.d0/xpomr1,bb1,1.d0-(1.d0-vvxp)*(1.d0-vvxpa)
     * ,0.d0,0.d0,icdp,icz,1)
       vtau0=qgfani(xpomr1*sy,bb2,1.d0-(1.d0-vvxt)*(1.d0-vvxta)
     * ,0.d0,0.d0,icdt,2,1)
     
       nn=0
1      nn=nn+1
       vpau=qgfani(1.d0/xpomr1,bb1,1.d0-exp(-vtau0)*(1.d0-vvxp)
     * *(1.d0-vvxpa)*(1.d0-vvxt)*(1.d0-vvxta),0.d0,0.d0,icdp,icz,1)
       vtau=qgfani(xpomr1*sy,bb2,1.d0-exp(-vpau0)*(1.d0-vvxt)
     * *(1.d0-vvxta)*(1.d0-vvxp)*(1.d0-vvxpa),0.d0,0.d0,icdt,2,1)
       if((abs(vpau-vpau0).gt.1.d-2.or.abs(vtau-vtau0).gt.1.d-2)
     * .and.nn.lt.100)then
        vpau0=vpau
	vtau0=vtau
	goto 1
       ...
       vpac=qgfani(1.d0/xpomr1,bb1,1.d0-exp(-vtau)*(1.d0-vvxt)
     * *(1.d0-vvxta),vvxp,vvxpa,icdp,icz,2)
       vtac=qgfani(xpomr1*sy,bb2,1.d0-exp(-vpau)*(1.d0-vvxp)
     * *(1.d0-vvxpa),vvxt,vvxta,icdt,2,2)
       vpac0=min(vpac,qgfani(1.d0/xpomr1,bb1,1.d0-exp(-vtau)
     * *(1.d0-vvxt)*(1.d0-vvxta),vvxp,vvxpa,icdp,icz,3))
       vtac0=min(vtac,qgfani(xpomr1*sy,bb2,1.d0-exp(-vpau)
     * *(1.d0-vvxp)*(1.d0-vvxpa),vvxt,vvxta,icdt,2,3))

       v1p=min(vpac,qgfani(1.d0/xpomr1,bb1,1.d0-exp(-vtau)
     * *(1.d0-vvxt)*(1.d0-vvxta),vvxp,vvxpa,icdp,icz,4))
       v1t=min(vtac,qgfani(xpomr1*sy,bb2,1.d0-exp(-vpau)
     * *(1.d0-vvxp)*(1.d0-vvxpa),vvxt,vvxta,icdt,2,4))
       v1p0=min(v1p,qgfani(1.d0/xpomr1,bb1,1.d0-exp(-vtau)
     * *(1.d0-vvxt)*(1.d0-vvxta),vvxp,vvxpa,icdp,icz,5))
       v1t0=min(v1t,qgfani(xpomr1*sy,bb2,1.d0-exp(-vpau)
     * *(1.d0-vvxp)*(1.d0-vvxpa),vvxt,vvxta,icdt,2,5))
       v1p1=qglegi(1.d0/xpomr1,bb1,0.d0,icdp,icz,0)
       v1pl=min(v1p,qgfani(1.d0/xpomr1,bb1,1.d0-exp(-vtau)
     * *(1.d0-vvxt)*(1.d0-vvxta),vvxp,vvxpa,icdp,icz,6))
       v1t1=qglegi(xpomr1*sy,bb2,0.d0,icdt,2,0)
       v1tl=min(v1t,qgfani(xpomr1*sy,bb2,1.d0-exp(-vpau)
     * *(1.d0-vvxp)*(1.d0-vvxpa),vvxt,vvxta,icdt,2,6))

       if(iqq.eq.2)then
        dpx=.25d0*(max(0.d0,1.d0-exp(-2.d0*vpac)-2.d0*vpac
     *  *exp(-2.d0*vpau)*(1.d0-vvxpa)**2)*(1.d0-vvxp)
     *  *max(0.d0,1.d0-exp(-2.d0*vtac)-2.d0*vtac*exp(-2.d0*vtau)
     *  *(1.d0-vvxta)**2)*(1.d0-vvxt)
     *  +max(0.d0,1.d0-exp(-2.d0*vpac)-2.d0*vpac*exp(-2.d0*vpau)
     *  *(1.d0-vvxpa)**2)*(1.d0-vvxp)
     *  *((vtac0+v1t0)*exp(-2.d0*vtau)*(1.d0-vvxta)**2*(1.d0-vvxt)
     *  -(vtac+v1t-vtac0-v1t0)*exp(-vtau)*(1.d0-vvxta)
     *  *(1.d0-exp(-vtau)*(1.d0-vvxt)*(1.d0-vvxta)))
     *  +max(0.d0,1.d0-exp(-2.d0*vtac)-2.d0*vtac*exp(-2.d0*vtau)
     *  *(1.d0-vvxta)**2)*(1.d0-vvxt)
     *  *((vpac0+v1p0)*exp(-2.d0*vpau)*(1.d0-vvxpa)**2*(1.d0-vvxp)
     *  -(vpac+v1p-vpac0-v1p0)*exp(-vpau)*(1.d0-vvxpa)
     *  *(1.d0-exp(-vpau)*(1.d0-vvxp)*(1.d0-vvxpa)))
     *  -2.d0*max(0.d0,exp(vpac-vpac0)-1.d0-(vpac-vpac0))
     *  *exp(-vpau)*(1.d0-vvxpa)
     *  *max(0.d0,1.d0-exp(-2.d0*vtac)-(vtac-v1t)*exp(-2.d0*vtau)
     *  *(1.d0-vvxta)**2)*(1.d0-vvxt)
     *  -2.d0*max(0.d0,exp(vtac-vtac0)-1.d0-(vtac-vtac0))
     *  *exp(-vtau)*(1.d0-vvxta)
     *  *max(0.d0,1.d0-exp(-2.d0*vpac)-(vpac-v1p)*exp(-2.d0*vpau)
     *  *(1.d0-vvxpa)**2)*(1.d0-vvxp))
       elseif(iqq.eq.3)then        
        dpx=.25d0*((1.d0-exp(-vpau))**2*(1.d0-vvxp)
     *  +2.d0*(1.d0-exp(-vpau))*vvxp)
     *  *(max(0.d0,max(0.d0,1.d0-exp(-2.d0*vtac)
     *  -2.d0*vtac*exp(-2.d0*vtau)*(1.d0-vvxta)**2)*(1.d0-vvxt)
     *  -2.d0*max(0.d0,exp(vtac-vtac0)-1.d0-(vtac-vtac0))
     *  *exp(-vtau)*(1.d0-vvxta))
     *  +(vtac0+v1t0)*exp(-2.d0*vtau)*(1.d0-vvxta)**2*(1.d0-vvxt)
     *  -(vtac+v1t-vtac0-v1t0)*exp(-vtau)*(1.d0-vvxta)
     *  *(1.d0-exp(-vtau)*(1.d0-vvxt)*(1.d0-vvxta)))     
     *  +(1.d0-exp(-vpau))*(1.d0-vvxp)*max(0.d0,exp(vtac-vtac0)-1.d0
     *  -(vtac-vtac0))*exp(-vtau)*(1.d0-vvxta)
       elseif(iqq.eq.4)then   
        dpx=.25d0*((1.d0-exp(-vtau))**2*(1.d0-vvxt)
     *  +2.d0*(1.d0-exp(-vtau))*vvxt)
     *  *(max(0.d0,max(0.d0,1.d0-exp(-2.d0*vpac)
     *  -2.d0*vpac*exp(-2.d0*vpau)*(1.d0-vvxpa)**2)*(1.d0-vvxp)
     *  -2.d0*max(0.d0,exp(vpac-vpac0)-1.d0-(vpac-vpac0))
     *  *exp(-vpau)*(1.d0-vvxpa))
     *  +(vpac0+v1p0)*exp(-2.d0*vpau)*(1.d0-vvxpa)**2*(1.d0-vvxp)
     *  -(vpac+v1p-vpac0-v1p0)*exp(-vpau)*(1.d0-vvxpa)
     *  *(1.d0-exp(-vpau)*(1.d0-vvxp)*(1.d0-vvxpa)))
     *  +(1.d0-exp(-vtau))*(1.d0-vvxt)*max(0.d0,exp(vpac-vpac0)-1.d0
     *  -(vpac-vpac0))*exp(-vpau)*(1.d0-vvxpa)
       endif
       qg3pa=qg3pa+a4(ib1)*a4(ib2)*a1(ix1)/z*min(1.d0,dpx)
      enddo
      enddo
      enddo
      enddo
...
c=============================================================================
      subroutine qgfdf(xxp,yyp,xpomr,vpau,vtau,vpac,vtac,vpac0,vtac0
     *,sumup,sumut,sumcp,sumct,sumdp,sumdt,ip,it,imp,imt)
c-----------------------------------------------------------------------
...
c ip       - proj. index,
c it       - targ. index,
c uncut fans are calculated for all nucleons;
c cut fans - for proj. nucleons from 1 to imp, 
c and for target nucleons from 1 to imt
c------------------------------------------------------------------------
      implicit double precision (a-h,o-z)
      integer debug
      parameter(iapmax=209)
      dimension vpau0(iapmax),vtau0(iapmax),vpau(iapmax),vtau(iapmax)
     *,vpac(iapmax),vtac(iapmax),vpac0(iapmax),vtac0(iapmax)
      common /qgarr1/  ia(2),icz,icp
      common /qgarr2/  scm,wp0,wm0
      common /qgarr7/  xa(iapmax,3),xb(iapmax,3),b
      common /qgarr16/ cc(2,3),iddp(iapmax),iddt(iapmax)
     ...
     *,iconb(iapmax)
      ...
      sumup0=0.d0                      !proj. uncut fans without targ. screening
      do i=1,ia(1)
       ipp=ia(1)-i+1
       if(ipp.ne.ip)then
     ...
     *   ,0.d0,0.d0,iddp(ipp),icz,1)
      ...
      vpau0(ip)=qgfani(1.d0/xpomr,bbp,1.d0-exp(-sumup0)
     *,0.d0,0.d0,iddp(ip),icz,1)
             	
      sumut0=0.d0                      !targ. uncut fans without proj. screening
      do i=1,ia(2)
       itt=ia(2)-i+1
       if(itt.ne.it)then
     ...
     *   ,0.d0,0.d0,iddt(itt),2,1)
      ...
      enddo
      bbt=(xb(it,1)-xxp)**2+(xb(it,2)-yyp)**2
      vtau0(it)=qgfani(xpomr*scm,bbt,1.d0-exp(-sumut0)
     *,0.d0,0.d0,iddt(it),2,1)
             	
      nn=0
1     nn=nn+1 
      sumup=0.d0                       !proj. uncut fans with targ. screening
      do i=1,ia(1)
       ipp=ia(1)-i+1
       if(ipp.ne.ip)then
        if(iconab(ipp,it).eq.0)then    !no connection
         ...
         bbp=(xa(ipp,1)+b-xxp)**2+(xa(ipp,2)-yyp)**2
         vpau(ipp)=qgfani(1.d0/xpomr,bbp,1.d0-exp(-sumup-sumut0
     *   -vtau0(it)),0.d0,0.d0,iddp(ipp),icz,1)
         sumup=sumup+vpau(ipp)
	endif
       endif
      enddo
      bbp=(xa(ip,1)+b-xxp)**2+(xa(ip,2)-yyp)**2
      vpau(ip)=qgfani(1.d0/xpomr,bbp,1.d0-exp(-sumup-sumut0-vtau0(it))
     *,0.d0,0.d0,iddp(ip),icz,1)
      ...
      do i=1,ia(2)
       ...
       if(itt.ne.it)then
        if(iconab(ip,itt).eq.0)then
         ...
         bbt=(xb(itt,1)-xxp)**2+(xb(itt,2)-yyp)**2
         vtau(itt)=qgfani(xpomr*scm,bbt,1.d0-exp(-sumut-sumup0
     *   -vpau0(ip)),0.d0,0.d0,iddt(itt),2,1)
         sumut=sumut+vtau(itt)
	endif
       endif
      enddo
      bbt=(xb(it,1)-xxp)**2+(xb(it,2)-yyp)**2
      vtau(it)=qgfani(xpomr*scm,bbt,1.d0-exp(-sumut-sumup0-vpau0(ip))
     *,0.d0,0.d0,iddt(it),2,1)
      
      if((abs(sumup-sumup0).gt..1d0.or.abs(sumut-sumut0).gt..1d0
     *.or.abs(vpau(ip)-vpau0(ip)).gt..1d0
     *.or.abs(vtau(it)-vtau0(it)).gt..1d0).and.nn.lt.100)then
       do i=1,ia(1)
        vpau0(i)=vpau(i)
       enddo
       do i=1,ia(2)
      ...
      sumcp=0.d0                       !proj. cut fans with targ. screening
      sumcp0=0.d0
      if(imp.gt.0)then
       do ipp=1,imp
        if(iconab(ipp,it).eq.0)then
         ...
         bbp=(xa(ipp,1)+b-xxp)**2+(xa(ipp,2)-yyp)**2
	 vvxt=1.d0-exp(-sumut-vtau(it))
	 vvxp=1.d0-exp(-sumcp0)
	 vvxpa=max(0.d0,1.d0-exp(-sumup+vpau(ipp)-vpau(ip)+sumcp0))
         vpac(ipp)=qgfani(1.d0/xpomr,bbp,vvxt,vvxp,vvxpa
     *   ,iddp(ipp),icz,2)
         vpac0(ipp)=min(vpac(ipp),qgfani(1.d0/xpomr,bbp,vvxt,vvxp,vvxpa
     *   ,iddp(ipp),icz,3))
	 if(ipp.lt.ip)sumcp=sumcp+vpac(ipp)
	 sumcp0=sumcp0+vpac(ipp)
	endif
       enddo
      endif
             	
      sumct=0.d0                       !targ. cut fans with proj. screening
      sumct0=0.d0
      if(imt.gt.0)then
       do itt=1,imt
        if(iconab(ip,itt).eq.0)then
         ...
         bbt=(xb(itt,1)-xxp)**2+(xb(itt,2)-yyp)**2
	 vvxp=1.d0-exp(-sumup-vpau(ip))
	 vvxt=1.d0-exp(-sumct0)
	 vvxta=max(0.d0,1.d0-exp(-sumut+vtau(itt)-vtau(it)+sumct0))
         vtac(itt)=qgfani(xpomr*scm,bbt,vvxp,vvxt,vvxta,iddt(itt),2,2)
         vtac0(itt)=min(vtac(itt),qgfani(xpomr*scm,bbt,vvxp,vvxt,vvxta
     *   ,iddt(itt),2,3))
	 if(itt.lt.it)sumct=sumct+vtac(itt)
	 sumct0=sumct0+vtac(itt)
	endif
       enddo
      endif
      
      sumdp=0.d0                       !proj. uncut fans for diffraction
      if(ip.lt.ia(1))then
       do ipp=ip+1,ia(1)
        sumdp=sumdp+vpau(ipp)
       enddo
      endif

      sumdt=0.d0                       !targ. uncut fans for diffraction
      if(it.lt.ia(2))then
       do itt=it+1,ia(2)
        sumdt=sumdt+vtau(itt)
       enddo
      endif
      if(debug.ge.4)write (moniou,202)
...
c iqq=2 - 1-cut,
c iqq=3 - multi-cut,
c iqq=4 - target diffraction,
c iqq=5 - projectile diffraction
c-----------------------------------------------------------------------
      implicit double precision (a-h,o-z)
      integer debug
     ...
     *,icdp,icdt,icz,iqq
      ...
      qgpomi=0.d0
      if(sy.le.1.d0.and.iqq.gt.2)return                     !below threshold
      ...
      if(iqq.eq.1)then                                !uncut eikonal
       ...
       do k1=1,3
        k2=k+k1-1
        qgpomi=qgpomi+qpomr(k2,l2,1,1,ml)*wk(k1)*wz(l1)
       enddo
       enddo
      else                                            !cut eikonals
       ...
       if(iqq.eq.2)then
        ...
        do k1=1,3
         k2=k+k1-1
         qgpomi=qgpomi+qpomr(k2,l2,j2,2,ml)*wk(k1)*wz(l1)*wj(j1)
        enddo
        enddo
        enddo       
       else
        ...
        do k1=1,3
         k2=k+k1-1
         qgpomi=qgpomi+qpomr(k2,l2,j2,2+i2+6*m2+36*n2+216*(iqq-3),ml)
     *   *wk(k1)*wz(l1)*wj(j1)*wi(i1)*wm(m1)*wn(n1)
      ...
      qgpomi=exp(qgpomi)*z
      if(iqq.ge.3)qgpomi=qgpomi*(1.d0-vvxp)*(1.d0-vvxt)      
...
c----------------------------------------------------------------------------
      ...
      integer debug
      parameter(iapmax=209)
      dimension xb(iapmax,3),fhard(3),vabs(2)
      common /qgarr1/  ia(2),icz,icp
      common /qgarr2/  scm,wp0,wm0
      common /qgarr16/ cc(2,3),iddp(iapmax),iddt(iapmax)
      common /qgarr43/ moniou
...
c-----------------------------------------------------------------------------
      implicit double precision (a-h,o-z)
      integer debug
      parameter(iapmax=209,npbmax=1000,npnmax=900,npmax=500,legmax=900)
      dimension xas(iapmax,3),vabs(2),vabsi(2,iapmax),wdifi(iapmax)
     *,vpau(iapmax),vtau(iapmax),vpac(iapmax),vtac(iapmax)
     *,vpac0(iapmax),vtac0(iapmax),xpomip(npmax),xpomim(npmax)
...
c calculate nuclear screening factors for "middle point" -> eikonals
	...
	xxp=.5d0*(x+xb(it,1))
	yyp=.5d0*(y+xb(it,2))
        call qgfdf(xxp,yyp,xpomr,vpau,vtau,vpac,vtac,vpac0,vtac0
     *  ,sumup,sumut,sumcp,sumct,sumdp,sumdt,ip,it,ip-1,it-1)
        vvx0=1.d0-exp(-sumup-sumut)
        vv1p=qgpomi(scm,bbp,vvx0,0.d0,0.d0,0.d0,icdp,icdt,icz,2) !1-Pomeron eikonal
        vvxp=1.d0-exp(-sumcp)
        vvxt=1.d0-exp(-sumct)
        vvxpa=1.d0-exp(min(0.d0,sumcp-sumup))
        vvxta=1.d0-exp(min(0.d0,sumct-sumut))
	vv3p=qgpomi(scm,bbp,vvxp,vvxt,vvxpa,vvxta,icdp,icdt,icz,3) !multi-Pomeron
        vvxp=1.d0-exp(-sumdp)
	vvxpa=0.d0
        vvxt=1.d0-exp(-sumct)
        vvxta=1.d0-exp(min(0.d0,sumct-sumut))
	vvdtg=qgpomi(scm,bbp,vvxp,vvxt,vvxpa,vvxta,icdp,icdt,icz,4) !targ diffr.
        vvxp=1.d0-exp(-sumcp)
        vvxpa=1.d0-exp(min(0.d0,sumcp-sumup))
        vvxt=1.d0-exp(-sumdt)
	vvxta=0.d0
	vvdpr=qgpomi(scm,bbp,vvxp,vvxt,vvxpa,vvxta,icdp,icdt,icz,5) !proj. diffr.
        vv=vv1p+vv3p+vvdpr+vvdtg                           !total eikonal
        if(debug.ge.2)write (moniou,213)vv,vv1p,vv3p,vvdpr,vvdtg
	...
	do i=1,n
	 aks=psran(b10)*vv
	 if(aks.gt.vv3p+vvdtg+vvdpr)then   !single Pomeron
	  ...
	  if(aks.lt.vv3p)then              !multi-Pomeron vertex
           if(debug.ge.2)write (moniou,216)i
	   idpom=2
	  elseif(aks.lt.vv3p+vvdtg)then    !targ. diffraction
           if(debug.ge.2)write (moniou,217)i
	   idpom=3
	  else                             !proj. diffraction
           if(debug.ge.2)write (moniou,218)i
	   idpom=4
	  endif
...
c configuration of multi-Pomeron vertexes	  
     ...
     *    ,xpomim,npompi,npomti,npin,ipompi,ipomti,idpom,ip,it,iret)
          if(iret.ne.0)goto 2
	...
	icdps=iddp(ip)
	 ...
	 do icdp=1,2
	   ...
	   yyp=.5d0*(y+xb(it,2))
           call qgfdf(xxp,yyp,xpomr,vpau,vtau,vpac,vtac,vpac0,vtac0
     *     ,sumup,sumut,sumcp,sumct,sumdp,sumdt,ip,it,ip-1,it-1)
           vvx=1.d0-exp(-sumup-sumut)
           vv1p=qgpomi(scm,bbp,vvx,0.d0,0.d0,0.d0,icdp,icdt,icz,2)
           vvxp=1.d0-exp(-sumcp)
           vvxt=1.d0-exp(-sumct)
           vvxpa=1.d0-exp(min(0.d0,sumcp-sumup))
           vvxta=1.d0-exp(min(0.d0,sumct-sumut))
           vv3p=qgpomi(scm,bbp,vvxp,vvxt,vvxpa,vvxta,icdp,icdt,icz,3)
           vvxp=1.d0-exp(-sumdp)
	   vvxpa=0.d0
           vvxt=1.d0-exp(-sumct)
           vvxta=1.d0-exp(min(0.d0,sumct-sumut))
	   vvdtg=qgpomi(scm,bbp,vvxp,vvxt,vvxpa,vvxta,icdp,icdt,icz,4)
           vvxp=1.d0-exp(-sumcp)
           vvxpa=1.d0-exp(min(0.d0,sumcp-sumup))
           vvxt=1.d0-exp(-sumdt)
	   vvxta=0.d0
           vvdpr=qgpomi(scm,bbp,vvxp,vvxt,vvxpa,vvxta,icdp,icdt,icz,5)
           vv=vv1p+vv3p+vvdpr+vvdtg
...
5     continue                            !end of ip-loop
     ...
     *     ,sumup,sumut,sumcp,sumct,sumdp,sumdt,ip,it,ip-1,it-1)        
	   ...
	   vvdtg=qgpomi(scm,bbp,vvxp,vvxt,vvxpa,vvxta,icdp,icdt,icz,4)
           ...
           vvxt=1.d0-exp(-sumdt)
	   vvxta=0.d0
	   vvdpr=qgpomi(scm,bbp,vvxp,vvxt,vvxpa,vvxt,icdp,icdt,icz,5)
           vv=vv1p+vv3p+vvdpr+vvdtg
...
213   format(2x,'qgconf: eikonals - total: ',e10.3,2x,'single: ',e10.3
     */4x,'multi-P: ',e10.3,2x,'proj-difr: ',e10.3
     *,2x,'targ-difr: ',e10.3)
...
c=============================================================================
     ...
     *,xpomip,xpomim,nppr,nptg,npin,ipompi,ipomti,idpom,ip,it,iret)
c-----------------------------------------------------------------------
...
c idpom = 2 - multi-Pomeron (cut+scr),
c idpom = 3 - targ diffraction,
c idpom = 4 - proj diffraction
c to determine:
...
c-----------------------------------------------------------------------
      implicit double precision (a-h,o-z)
      integer debug
      parameter(iapmax=209,npmax=500,levmax=20,legmax=900)
      dimension vpau(iapmax),vtau(iapmax)
     *,vpac(iapmax),vtac(iapmax),vpac0(iapmax),vtac0(iapmax)
     *,vpact0(iapmax),vtact0(iapmax)
...
c      external psran
      ...
      if(debug.ge.2)write (moniou,201)ip,it,idpom
...
c normalization of rejection function      
      ...
      if(idpom.eq.2)then
       xpomr=1.d0/dsqrt(scm)
      elseif(idpom.eq.3)then
       xpomr=scm**(-.8d0)
      elseif(idpom.eq.4)then
       xpomr=scm**(-.2d0)
      endif	   
      rp=(rq(icz)+rq(2)+alfp*log(scm))*4.d0*.0389d0  
      xxp=.5d0*(xa(ip,1)+b+xb(it,1))    
      ...
      call qgfdf(xxp,yyp,xpomr,vpau,vtau,vpac,vtac,vpac0,vtac0
     *,sumup,sumut,sumcp,sumct,sumdp,sumdt,ip,it,ia(1),ia(2))
      sumcp0=0.d0
      sumct0=0.d0
      if(ip.lt.ia(1))then
       do i=ip+1,ia(1)
        sumcp0=sumcp0+vpac0(i)
       enddo
      endif
      if(it.lt.ia(2))then
       do i=it+1,ia(2)
        sumct0=sumct0+vtac0(i)
       enddo
      endif
           	       	
      bbp=(xa(ip,1)+b-xxp)**2+(xa(ip,2)-yyp)**2    
      bbt=(xxp-xb(it,1))**2+(yyp-xb(it,2))**2    
      v1p=min(vpac(ip),qgfani(1.d0/xpomr,bbp,1.d0-exp(-sumut-vtau(it))
     *,1.d0-exp(-sumcp),1.d0-exp(-max(0.d0,sumup-sumcp))
     *,iddp(ip),icz,4))
      v1p0=min(v1p,qgfani(1.d0/xpomr,bbp,1.d0-exp(-sumut-vtau(it))
     *,1.d0-exp(-sumcp),1.d0-exp(-max(0.d0,sumup-sumcp))
     *,iddp(ip),icz,5))
      v1t=min(vtac(it),qgfani(xpomr*scm,bbt,1.d0-exp(-sumup-vpau(ip))
     *,1.d0-exp(-sumct),1.d0-exp(-max(0.d0,sumut-sumct)),iddt(it),2,4))
      v1t0=min(v1t,qgfani(xpomr*scm,bbt,1.d0-exp(-sumup-vpau(ip))
     *,1.d0-exp(-sumct),1.d0-exp(-max(0.d0,sumut-sumct)),iddt(it),2,5))

      if(idpom.eq.2)then     
       gb0=.25d0*(max(0.d0,(1.d0-exp(-2.d0*vpac(ip)))*exp(-2.d0*sumcp)
     * -2.d0*vpac(ip)*exp(-2.d0*(sumup+vpau(ip))))
     * *max(0.d0,(1.d0-exp(-2.d0*vtac(it)))*exp(-2.d0*sumct)
     * -2.d0*vtac(it)*exp(-2.d0*(sumut+vtau(it))))
     * +max(0.d0,(1.d0-exp(-2.d0*vpac(ip)))*exp(-2.d0*sumcp)
     * -2.d0*vpac(ip)*exp(-2.d0*(sumup+vpau(ip))))
     * *((vtac0(it)+v1t0)*exp(-2.d0*(sumut+vtau(it)))
     * -(vtac(it)-vtac0(it)+v1t-v1t0)*exp(-sumut-vtau(it))
     * *(1.d0-exp(-sumut-vtau(it))))
     * +((vpac0(ip)+v1p0)*exp(-2.d0*(sumup+vpau(ip)))
     * -(vpac(ip)-vpac0(ip)+v1p-v1p0)*exp(-sumup-vpau(ip))
     * *(1.d0-exp(-sumup-vpau(ip))))
     * *max(0.d0,(1.d0-exp(-2.d0*vtac(it)))*exp(-2.d0*sumct)
     * -2.d0*vtac(it)*exp(-2.d0*(sumut+vtau(it))))
     * -2.d0*max(0.d0,(exp(-vpac0(ip))-exp(-vpac(ip)))
     * *exp(-sumcp0-sumcp)-(vpac(ip)-vpac0(ip))*exp(-sumup-vpau(ip)))
     * *max(0.d0,(1.d0-exp(-2.d0*vtac(it)))*exp(-2.d0*sumct)
     * -(vtac(it)-v1t)*exp(-2.d0*(sumut+vtau(it))))
     * -2.d0*max(0.d0,(exp(-vtac0(it))-exp(-vtac(it)))
     * *exp(-sumct0-sumct)-(vtac(it)-vtac0(it))*exp(-sumut-vtau(it)))
     * *max(0.d0,(1.d0-exp(-2.d0*vpac(ip)))*exp(-2.d0*sumcp)
     * -(vpac(ip)-v1p)*exp(-2.d0*(sumup+vpau(ip)))))     
      elseif(idpom.eq.3)then
       vpau(ip)=qgfani(1.d0/xpomr,bbp,1.d0-exp(-sumdp-sumut-vtau(it))
     * ,0.d0,0.d0,iddp(ip),icz,1)
       gb0=.25d0*(max(0.d0,(1.d0-exp(-2.d0*vtac(it)))*exp(-2.d0*sumct)
     * -2.d0*vtac(it)*exp(-2.d0*(sumut+vtau(it))))
     * +(vtac0(it)+v1t0)*exp(-2.d0*(sumut+vtau(it)))
     * -(vtac(it)-vtac0(it)+v1t-v1t0)*exp(-sumut-vtau(it))
     * *(1.d0-exp(-sumut-vtau(it))))
     * *((1.d0-exp(-vpau(ip)))**2*exp(-2.d0*sumdp)
     * +2.d0*(1.d0-exp(-vpau(ip)))*(1.d0-exp(-sumdp))*exp(-sumdp))
     * +.5d0*(1.d0-exp(-2.d0*vpau(ip)))*exp(-2.d0*sumdp)
     * *max(0.d0,(exp(-vtac0(it))-exp(-vtac(it)))*exp(-sumct0-sumct)
     * -(vtac(it)-vtac0(it))*exp(-sumut-vtau(it)))
      elseif(idpom.eq.4)then
       vtau(it)=qgfani(xpomr*scm,bbt,1.d0-exp(-sumdt-sumup-vpau(ip))
     * ,0.d0,0.d0,iddt(it),2,1)
       gb0=.25d0*(max(0.d0,(1.d0-exp(-2.d0*vpac(ip)))*exp(-2.d0*sumcp)
     * -2.d0*vpac(ip)*exp(-2.d0*(sumup+vpau(ip))))
     * +(vpac0(ip)+v1p0)*exp(-2.d0*(sumup+vpau(ip)))
     * -(vpac(ip)-vpac0(ip)+v1p-v1p0)*exp(-sumup-vpau(ip))
     * *(1.d0-exp(-sumup-vpau(ip))))
     * *((1.d0-exp(-vtau(it)))**2*exp(-2.d0*sumdt)
     * +2.d0*(1.d0-exp(-vtau(it)))*(1.d0-exp(-sumdt))*exp(-sumdt))
     * +.5d0*(1.d0-exp(-2.d0*vtau(it)))*exp(-2.d0*sumdt)
     * *max(0.d0,(exp(-vpac0(ip))-exp(-vpac(ip)))*exp(-sumcp0-sumcp)
     * -(vpac(ip)-vpac0(ip))*exp(-sumup-vpau(ip)))
      else
       stop'wrong idpom'
      endif
     ...
     ***(1.d0+ahl(2)))**(1.d0+dels-alpd))	  
      gb0=min(1.d0,gb0)*max(3.d0,dlog10(scm)*5.d0)
...
1     continue
     ...
     *,sumup,sumut,sumcp,sumct,sumdp,sumdt,ip,it,ia(1),ia(2))
      ...
      v1p=min(vpac(ip),qgfani(1.d0/xpomr,bbpr,1.d0-exp(-sumut-vtau(it))
     *,1.d0-exp(-sumcp),1.d0-exp(-max(0.d0,sumup-sumcp))
     *,iddp(ip),icz,4))
      v1p0=min(v1p,qgfani(1.d0/xpomr,bbpr,1.d0-exp(-sumut-vtau(it))
     *,1.d0-exp(-sumcp),1.d0-exp(-max(0.d0,sumup-sumcp))
     *,iddp(ip),icz,5))
      v1t=min(vtac(it),qgfani(xpomr*scm,bbtg,1.d0-exp(-sumup-vpau(ip))
     *,1.d0-exp(-sumct),1.d0-exp(-max(0.d0,sumut-sumct)),iddt(it),2,4))
      v1t0=min(v1t,qgfani(xpomr*scm,bbtg,1.d0-exp(-sumup-vpau(ip))
     *,1.d0-exp(-sumct),1.d0-exp(-max(0.d0,sumut-sumct)),iddt(it),2,5))
      vpaud=qgfani(1.d0/xpomr,bbpr,1.d0-exp(-sumdp-sumut-vtau(it))
     *,0.d0,0.d0,iddp(ip),icz,1)
      vtaud=qgfani(xpomr*scm,bbtg,1.d0-exp(-sumdt-sumup-vpau(ip))
     *,0.d0,0.d0,iddt(it),2,1)

      if(idpom.eq.2)then                  !multi-Pomeron vertex 
       gb=.25d0*(max(0.d0,(1.d0-exp(-2.d0*vpac(ip)))*exp(-2.d0*sumcp)
     * -2.d0*vpac(ip)*exp(-2.d0*(sumup+vpau(ip))))
     * *max(0.d0,(1.d0-exp(-2.d0*vtac(it)))*exp(-2.d0*sumct)
     * -2.d0*vtac(it)*exp(-2.d0*(sumut+vtau(it))))
     * +max(0.d0,(1.d0-exp(-2.d0*vpac(ip)))*exp(-2.d0*sumcp)
     ...
     * *max(0.d0,(1.d0-exp(-2.d0*vpac(ip)))*exp(-2.d0*sumcp)
     * -(vpac(ip)-v1p)*exp(-2.d0*(sumup+vpau(ip)))))
      elseif(idpom.eq.3)then              !rap-gap with the projectile
       gb=.25d0*(max(0.d0,(1.d0-exp(-2.d0*vtac(it)))*exp(-2.d0*sumct)
     * -2.d0*vtac(it)*exp(-2.d0*(sumut+vtau(it))))
     * +(vtac0(it)+v1t0)*exp(-2.d0*(sumut+vtau(it)))
     * -(vtac(it)-vtac0(it)+v1t-v1t0)*exp(-sumut-vtau(it))
     * *(1.d0-exp(-sumut-vtau(it))))
     * *((1.d0-exp(-vpaud))**2*exp(-2.d0*sumdp)
     * +2.d0*(1.d0-exp(-vpaud))*(1.d0-exp(-sumdp))*exp(-sumdp))
     * +.5d0*(1.d0-exp(-2.d0*vpaud))*exp(-2.d0*sumdp)
     * *max(0.d0,(exp(-vtac0(it))-exp(-vtac(it)))*exp(-sumct0-sumct)
     * -(vtac(it)-vtac0(it))*exp(-sumut-vtau(it)))
      elseif(idpom.eq.4)then              !rap-gap with the target
       gb=.25d0*(max(0.d0,(1.d0-exp(-2.d0*vpac(ip)))*exp(-2.d0*sumcp)
     * -2.d0*vpac(ip)*exp(-2.d0*(sumup+vpau(ip))))
     * +(vpac0(ip)+v1p0)*exp(-2.d0*(sumup+vpau(ip)))
     * -(vpac(ip)-vpac0(ip)+v1p-v1p0)*exp(-sumup-vpau(ip))
     * *(1.d0-exp(-sumup-vpau(ip))))
     * *((1.d0-exp(-vtaud))**2*exp(-2.d0*sumdt)
     * +2.d0*(1.d0-exp(-vtaud))*(1.d0-exp(-sumdt))*exp(-sumdt))
     * +.5d0*(1.d0-exp(-2.d0*vtaud))*exp(-2.d0*sumdt)
     * *max(0.d0,(exp(-vpac0(ip))-exp(-vpac(ip)))*exp(-sumcp0-sumcp)
     * -(vpac(ip)-vpac0(ip))*exp(-sumup-vpau(ip)))
      else
       stop'wrong idpom'
...
c vv1 - one Pomeron into many, vv2 - many into one, vv3 - many into many
     ...
     * *exp(-sumcp0-sumcp)-(vpac(ip)-vpac0(ip))*exp(-sumup-vpau(ip)))
     * *(vtac(it)+v1t)*exp(-2.d0*(sumut+vtau(it)))
       vv3=((vpac0(ip)+v1p0)*exp(-2.d0*(sumup+vpau(ip)))
     * -(vpac(ip)-vpac0(ip)+v1p-v1p0)*exp(-sumup-vpau(ip))
     * *(1.d0-exp(-sumup-vpau(ip))))
     * *max(0.d0,(1.d0-exp(-2.d0*vtac(it)))*exp(-2.d0*sumct)
     * -2.d0*vtac(it)*exp(-2.d0*(sumut+vtau(it))))
     ...
     * *(vpac(ip)+v1p)*exp(-2.d0*(sumup+vpau(ip)))
       vvt=vv1+vv2+vv3
       aks=psran(b10)
       if(aks.lt.vv1/vvt)then
        jt=1
       elseif(aks.lt.(vv1+vv2)/vvt)then
        jt=2
       else
        jt=3
       endif
     ...
     * *((1.d0-exp(-vpaud))**2*exp(-2.d0*sumdp)
     * +2.d0*(1.d0-exp(-vpaud))*(1.d0-exp(-sumdp))*exp(-sumdp))
       vv5=((vtac0(it)+v1t0)*exp(-2.d0*(sumut+vtau(it)))
     * -(vtac(it)-vtac0(it)+v1t-v1t0)*exp(-sumut-vtau(it))
     * *(1.d0-exp(-sumut-vtau(it))))
     * *((1.d0-exp(-vpaud))**2*exp(-2.d0*sumdp)
     * +2.d0*(1.d0-exp(-vpaud))*(1.d0-exp(-sumdp))*exp(-sumdp))
       vv6=2.d0*(1.d0-exp(-2.d0*vpaud))*exp(-2.d0*sumdp)
     * *max(0.d0,(exp(-vtac0(it))-exp(-vtac(it)))*exp(-sumct0-sumct)
     * -(vtac(it)-vtac0(it))*exp(-sumut-vtau(it)))
       vvt=vv4+vv5+vv6
       aks=psran(b10)
       if(aks.lt.vv4/vvt)then
        jt=4
       elseif(aks.lt.(vv4+vv5)/vvt)then
        jt=5
       else
        jt=6
       endif
     ...
     * *((1.d0-exp(-vtaud))**2*exp(-2.d0*sumdt)
     * +2.d0*(1.d0-exp(-vtaud))*(1.d0-exp(-sumdt))*exp(-sumdt))
       vv8=((vpac0(ip)+v1p0)*exp(-2.d0*(sumup+vpau(ip)))
     * -(vpac(ip)-vpac0(ip)+v1p-v1p0)*exp(-sumup-vpau(ip))
     * *(1.d0-exp(-sumup-vpau(ip))))
     * *((1.d0-exp(-vtaud))**2*exp(-2.d0*sumdt)
     * +2.d0*(1.d0-exp(-vtaud))*(1.d0-exp(-sumdt))*exp(-sumdt))
       vv9=2.d0*(1.d0-exp(-2.d0*vtaud))*exp(-2.d0*sumdt)
     * *max(0.d0,(exp(-vpac0(ip))-exp(-vpac(ip)))*exp(-sumcp0-sumcp)
     * -(vpac(ip)-vpac0(ip))*exp(-sumup-vpau(ip)))
       vvt=vv7+vv8+vv9
       aks=psran(b10)
       if(aks.lt.vv7/vvt)then
        jt=7
       elseif(aks.lt.(vv7+vv8)/vvt)then
        jt=8
       else
        jt=9
       endif
...
5      ntry=ntry+1
       if(ip.eq.ia(1).or.ntry.gt.100)then
        nppr0=npgen(2.d0*(vpac(ip)-vpac0(ip)),2,20)
        do i=1,nppr0
	 itypr0(i)=1
	 ippr0(i)=ip
	enddo	 
       else
        nppr0=npgen(2.d0*(vpac(ip)-vpac0(ip)),1,20)
        do i=1,nppr0
	 itypr0(i)=1
	 ippr0(i)=ip
	enddo
	do ipp=ip+1,ia(1)
         ninc=npgen(2.d0*(vpac(ipp)-vpac0(ipp)),0,20)
	 if(ninc.ne.0)then
          nppr0=nppr0+ninc
          if(nppr0.gt.legmax)then
	   iret=1
...
6      ntry=ntry+1
       if(it.eq.ia(2).or.ntry.gt.100)then
        nptg0=npgen(2.d0*(vtac(it)-vtac0(it)),2,20)
        do i=1,nptg0
	 itytg0(i)=1
	 iptg0(i)=it
	enddo	 
       else
        nptg0=npgen(2.d0*(vtac(it)-vtac0(it)),1,20)
        do i=1,nptg0
	 itytg0(i)=1
	 iptg0(i)=it
	enddo
	do itt=it+1,ia(2)
         ninc=npgen(2.d0*(vtac(itt)-vtac0(itt)),0,20)
	 if(ninc.ne.0)then
          nptg0=nptg0+ninc
          if(nptg0.gt.legmax)then
	   iret=1
	...
	if(nptg0.eq.1)goto 6
      ...
      if(jt.eq.1.and.nptgh0.lt.nptg0.and.npprh0.eq.nppr0)then
       gbt=1.d0-exp(vpau(ip)+sumup+(1.d0-nppr0)*dlog(2.d0))
      elseif(jt.eq.1.and.nptgh0.eq.nptg0.and.npprh0.lt.nppr0)then
       gbt=1.d0-exp(vtau(ip)+sumut+(1.d0-nptg0)*dlog(2.d0))
      elseif(jt.eq.1.and.nptgh0.eq.nptg0.and.npprh0.eq.nppr0)then
       gbt=1.d0-exp(vtau(ip)+sumut+(1.d0-nptg0)*dlog(2.d0))
     * -exp(vpau(ip)+sumup+(1.d0-nppr0)*dlog(2.d0))
      elseif(jt.eq.2.and.npprh0.eq.nppr0)then
       gbt=((vtac0(it)+v1t0)*exp(-2.d0*(sumut+vtau(it)))
     * -(vtac(it)-vtac0(it)+v1t-v1t0)*exp(-sumut-vtau(it))
     * *(1.d0-exp(-sumut-vtau(it)))
     * -exp(vpau(ip)+sumup+(1.d0-nppr0)*dlog(2.d0))
     * *(vtac(it)+v1t)*exp(-2.d0*(sumut+vtau(it))))
     ...
     * *(1.d0-exp(-sumut-vtau(it))))
      elseif(jt.eq.3.and.nptgh0.eq.nptg0)then
       gbt=((vpac0(ip)+v1p0)*exp(-2.d0*(sumup+vpau(ip)))
     * -(vpac(ip)-vpac0(ip)+v1p-v1p0)*exp(-sumup-vpau(ip))
     * *(1.d0-exp(-sumup-vpau(ip)))
     * -exp(vtau(ip)+sumut+(1.d0-nptg0)*dlog(2.d0))
     * *(vpac(ip)+v1p)*exp(-2.d0*(sumup+vpau(ip))))
     * /((vpac0(ip)+v1p0)*exp(-2.d0*(sumup+vpau(ip)))
     * -(vpac(ip)-vpac0(ip)+v1p-v1p0)*exp(-sumup-vpau(ip))
     * *(1.d0-exp(-sumup-vpau(ip))))
     ...
     *.lt.(vtac(it)-vtac0(it)+v1t-v1t0)*exp(-sumut-vtau(it))
     **(1.d0-exp(-sumut-vtau(it)))/((vtac0(it)+v1t0)
     **exp(-2.d0*(sumut+vtau(it)))-(vtac(it)-vtac0(it)+v1t-v1t0)
     **exp(-sumut-vtau(it))*(1.d0-exp(-sumut-vtau(it))))
     *.or.jt.eq.2.and.npprh0.eq.nppr0.and.psran(b10)
     *.lt.(vtac(it)-vtac0(it)+v1t-v1t0)*(exp(-sumut-vtau(it))
     **(1.d0-exp(-sumut-vtau(it)))*2.d0**nppr0
     *+2.d0*exp(-2.d0*(sumut+vtau(it))))
     */(((vtac0(it)+v1t0)*exp(-2.d0*(sumut+vtau(it)))
     *-(vtac(it)-vtac0(it)+v1t-v1t0)*exp(-sumut-vtau(it))
     **(1.d0-exp(-sumut-vtau(it))))*2.d0**nppr0
     *-2.d0*(vtac(it)+v1t)*exp(-2.d0*(sumut+vtau(it)))))then
       vvx=max(0.d0,1.d0+exp(-2.d0*(vpau(ip)+vtau(it)+sumup+sumut))
     * -exp(-2.d0*(vpau(ip)+sumup)-vtau(it)-sumut)
     * -exp(-2.d0*(vtau(it)+sumut)-vpau(ip)-sumup))
7      xpomr1=xpomr/(xpomr*scm)**psran(b10)
     ...
     *.lt.(vpac(ip)-vpac0(ip)+v1p-v1p0)*exp(-sumup-vpau(ip))
     **(1.d0-exp(-sumup-vpau(ip)))/((vpac0(ip)+v1p0)
     **exp(-2.d0*(sumup+vpau(ip)))-(vpac(ip)-vpac0(ip)+v1p-v1p0)
     **exp(-sumup-vpau(ip))*(1.d0-exp(-sumup-vpau(ip))))
     *.or.jt.eq.3.and.nptgh0.eq.nptg0.and.psran(b10)
     *.lt.(vpac(ip)-vpac0(ip)+v1p-v1p0)*(exp(-sumup-vpau(ip))
     **(1.d0-exp(-sumup-vpau(ip)))*2.d0**nptg0
     *+2.d0*exp(-2.d0*(sumup+vpau(ip))))
     */(((vpac0(ip)+v1p0)*exp(-2.d0*(sumup+vpau(ip)))
     *-(vpac(ip)-vpac0(ip)+v1p-v1p0)*exp(-sumup-vpau(ip))
     **(1.d0-exp(-sumup-vpau(ip))))*2.d0**nptg0
     *-2.d0*(vpac(ip)+v1p)*exp(-2.d0*(sumup+vpau(ip)))))then
       vvx=max(0.d0,1.d0+exp(-2.d0*(vpau(ip)+vtau(it)+sumup+sumut))
     * -exp(-2.d0*(vpau(ip)+sumup)-vtau(it)-sumut)
     * -exp(-2.d0*(vtau(it)+sumut)-vpau(ip)-sumup))
8      xpomr1=xpomr**psran(b10)
	...
	goto 21
      ...
      call qgfdf(xxp,yyp,xpomr,vpau,vtau,vpac,vtac,vpac0,vtac0
     *,sumup,sumut,sumcp,sumct,sumdp,sumdt,1,1,0,0)
      sumcp0m=0.d0
      sumcp0t=0.d0
      do i=1,ia(1)
       ipp=ia(1)-i+1
       if(iconab(ipp,it).eq.0)then
        ...
        bbp=(xa(ipp,1)+b-xxp)**2+(xa(ipp,2)-yyp)**2
        vpac0(ipp)=min(vpau(ipp),qgfani(1.d0/xpomr,bbp,1.d0-exp(-2.d0
     *  *(vtau(it)+sumut)-sumcp0t),0.d0,0.d0,iddp(ipp),icz,1))
        sumcp0t=sumcp0t+vpac0(ipp)
        if(ipp.gt.ip)sumcp0m=sumcp0m+vpac0(ipp)
       endif
      enddo
      sumct0m=0.d0
      sumct0t=0.d0
      do i=1,ia(2)
       ...
       if(iconab(ip,itt).eq.0)then
        ...
        bbt=(xb(itt,1)-xxp)**2+(xb(itt,2)-yyp)**2
        vtac0(itt)=min(vtau(itt),qgfani(xpomr*scm,bbt,1.d0
     *  -exp(-2.d0*(vpau(ip)+sumup)-sumct0t),0.d0,0.d0,iddt(itt),2,1))
        sumct0t=sumct0t+vtac0(itt)
        if(itt.gt.it)sumct0m=sumct0m+vtac0(itt)
       endif
      enddo
      sumcpt=0.d0
      do i=1,ia(1)-ip+1
       ...
       if(iconab(ipp,it).eq.0)then
        ...
        bbp=(xa(ipp,1)+b-xxp)**2+(xa(ipp,2)-yyp)**2
        vpact0(ipp)=max(vpac0(ipp),qgfani(1.d0/xpomr,bbp,1.d0-exp(-2.d0
     *  *(vtau(it)+sumut-sumct0t)-sumcpt),0.d0,0.d0,iddp(ipp),icz,1))
        if(ipp.gt.ip)sumcpt=sumcpt+vpact0(ipp)
       endif
      enddo
      sumctt=0.d0
      do i=1,ia(2)-it+1
       ...
       if(iconab(ip,itt).eq.0)then
        ...
        bbt=(xb(itt,1)-xxp)**2+(xb(itt,2)-yyp)**2
        vtact0(itt)=max(vtac0(itt),qgfani(xpomr*scm,bbt,1.d0-exp(-2.d0
     *  *(vpau(ip)+sumup-sumcp0t)-sumctt),0.d0,0.d0,iddt(itt),2,1))
        if(itt.gt.it)sumctt=sumctt+vtact0(itt)
       endif
      enddo

      gbipr=.5d0*(vpact0(ip)-vpac0(ip))*max(0.d0,max(0.d0
     *,1.d0+exp(-2.d0*sumcp0t)-2.d0*exp(-2.d0*(vpau(ip)+sumup)))
     **(1.d0-exp(-vtac0(it)))*exp(-sumct0m)
     *-(2.d0+exp(-2.d0*sumcp0t)-2.d0*exp(-2.d0*(vpau(ip)+sumup)))
     **exp(-sumct0t)*max(0.d0,vtact0(it)-vtac0(it)
     *-(1.d0-exp(vtac0(it)-vtact0(it)))*exp(sumct0m-sumctt)))     
      gbitg=.5d0*(vtact0(it)-vtac0(it))*max(0.d0,max(0.d0
     *,1.d0+exp(-2.d0*sumct0t)-2.d0*exp(-2.d0*(vtau(it)+sumut)))
     **(1.d0-exp(-vpac0(ip)))*exp(-sumcp0m)
     *-(2.d0+exp(-2.d0*sumct0t)-2.d0*exp(-2.d0*(vtau(it)+sumut)))
     **exp(-sumcp0t)*max(0.d0,vpact0(ip)-vpac0(ip)
     *-(1.d0-exp(vpac0(ip)-vpact0(ip)))*exp(sumcp0m-sumcpt)))     
      nprin=npgen(gbipr/gbs,0,npmax)
      ntgin=npgen(gbitg/gbs,0,npmax)
	...
	goto 21
      ...
      if(nprin.ne.0)then
       vvxt0=exp(-sumut-vtau(it))
       gb0pr=(1.d0-exp(-vpact0(ip)))
     * *exp(-2.d0*(sumut+vtau(it)-sumct0t)-sumcpt)
     * -(1.d0-exp(-vpac0(ip)))*exp(-2.d0*(sumut+vtau(it))-sumcp0m)
     * -(vpact0(ip)-vpac0(ip))*exp(-sumut-vtau(it)-sumup-vpau(ip))
      endif
      if(ntgin.ne.0)then
       vvxp0=exp(-sumup-vpau(ip))
       gb0tg=(1.d0-exp(-vtact0(it)))
     * *exp(-2.d0*(sumup+vpau(ip)-sumcp0t)-sumctt)
     * -(1.d0-exp(-vtac0(it)))*exp(-2.d0*(sumup+vpau(ip))-sumct0m)
     * -(vtact0(it)-vtac0(it))*exp(-sumup-vpau(ip)-sumut-vtau(it))
      endif

      if(nprin.ne.0)then
       if(gb0pr.le.0.d0)then
        nprin=0
       else
        gbcor10=(1.d0-(1.d0-(1.d0-xpomr)**(1.d0+ahl(icz)))
     *  **(1.d0+dels-alpd))**2
        rp0=-alfp*dlog(xpomr)*4.d0*.0389d0
        do in=1,nprin
	 n=0
22       xpomr1=xpomr**psran(b10)
     ...
     *   **(1.d0+dels-alpd))**2
	 ...
	 bbi=dsqrt(-rp0*dlog(psran(b10)))
	 call qgcs(c,s)
	 xxpr=xxp+bbi*c
	 yypr=yyp+bbi*s
         call qgfdf(xxpr,yypr,xpomr1,vpau,vtau,vpac,vtac,vpac0,vtac0
     *   ,sumup,sumut,sumcp,sumct,sumdp,sumdt,1,1,0,0)
         sumcp0m=0.d0
         sumcp0t=0.d0
         do i=1,ia(1)
          ...
          if(iconab(ipp,it).eq.0)then
           ...
           bbp=(xa(ipp,1)+b-xxpr)**2+(xa(ipp,2)-yypr)**2
           vpac0(ipp)=min(vpau(ipp),qgfani(1.d0/xpomr1,bbp,1.d0-exp(
     *     -2.d0*(vtau(it)+sumut)-sumcp0t),0.d0,0.d0,iddp(ipp),icz,1))
           sumcp0t=sumcp0t+vpac0(ipp)
           if(ipp.gt.ip)sumcp0m=sumcp0m+vpac0(ipp)
	  endif
         enddo
         ...
         do i=1,ia(2)
          ...
          if(iconab(ip,itt).eq.0)then
           ...
           bbt=(xb(itt,1)-xxpr)**2+(xb(itt,2)-yypr)**2
           vtac0(itt)=min(vtau(itt),qgfani(xpomr1*scm,bbt,1.d0-exp(
     *     -2.d0*(vpau(ip)+sumup)-sumct0t),0.d0,0.d0,iddt(itt),2,1))
           sumct0t=sumct0t+vtac0(itt)
           if(itt.gt.it)sumct0m=sumct0m+vtac0(itt)
	  endif
         enddo
         ...
         do i=1,ia(1)-ip+1
          ...
          if(iconab(ipp,it).eq.0)then
           ...
           bbp=(xa(ipp,1)+b-xxpr)**2+(xa(ipp,2)-yypr)**2
           vpact0(ipp)=max(vpac0(ipp),qgfani(1.d0/xpomr1,bbp
     *     ,1.d0-exp(-2.d0*(vtau(it)+sumut-sumct0t)-sumcpt)
     *     ,0.d0,0.d0,iddp(ipp),icz,1))
           if(ipp.gt.ip)sumcpt=sumcpt+vpact0(ipp)
	  endif
         enddo
         ...
         do i=1,ia(2)-it+1
          ...
          if(iconab(ip,itt).eq.0)then
           ...
           bbt=(xb(itt,1)-xxpr)**2+(xb(itt,2)-yypr)**2
           vtact0(itt)=max(vtac0(itt),qgfani(xpomr1*scm,bbt
     *     ,1.d0-exp(-2.d0*(vpau(ip)+sumup-sumcp0t)-sumctt)
     *     ,0.d0,0.d0,iddt(itt),2,1))
           if(itt.gt.it)sumctt=sumctt+vtact0(itt)
	  endif
         enddo

         vvxt=1.d0-dsqrt(vvxt0*exp(-sumut-vtau(it)))
         rp=alfp*dlog(xpomr1/xpomr)*4.d0*.0389d0
	 v1i=((xpomr1/xpomr)**(dels-alf*vvxt)
     *   +qgpini(xpomr1/xpomr,bbi**2,vvxt)/sigs)*exp(-bbi**2/rp)*rp0/rp
         gbpr=v1i*((1.d0-exp(-vpact0(ip)))
     *   *exp(-2.d0*(sumut+vtau(it)-sumct0t)-sumcpt)
     *   -(1.d0-exp(-vpac0(ip)))*exp(-2.d0*(sumut+vtau(it))-sumcp0m)
     *   -(vpact0(ip)-vpac0(ip))*exp(-sumut-vtau(it)))
         gbpr=gbpr/gb0pr/gbcor1*gbcor10    *.05  !.01
...
23    continue
      ...
      if(ntgin.ne.0)then
       if(gb0tg.le.0.d0)then
        ntgin=0
       else
        gbcor20=(1.d0-(1.d0-(1.d0-1.d0/xpomr/scm)
     *  **(1.d0+ahl(2)))**(1.d0+dels-alpd))**2
        rp0=alfp*dlog(xpomr*scm)*4.d0*.0389d0
        do in=1,ntgin
	 n=0
24       xpomr1=xpomr/(xpomr*scm)**psran(b10)
     ...
     *     ,0.d0,0.d0,iddt(itt),2,1))
         ...
         enddo
         vvxp=1.d0-dsqrt(vvxp0*exp(-sumup-vpau(ip)))
         rp=alfp*dlog(xpomr/xpomr1)*4.d0*.0389d0
	 v1i=((xpomr/xpomr1)**(dels-alf*vvxp)
     *   +qgpini(xpomr/xpomr1,bbi**2,vvxp)/sigs)*exp(-bbi**2/rp)*rp0/rp
         gbtg=v1i*((1.d0-exp(-vtact0(it)))
     *   *exp(-2.d0*(sumup+vpau(ip)-sumcp0t)-sumctt)
     *   -(1.d0-exp(-vtac0(it)))*exp(-2.d0*(sumup+vpau(ip))-sumct0m)
     *   -(vtact0(it)-vtac0(it))*exp(-sumup-vpau(ip)))
         gbtg=gbtg/gb0tg/gbcor2*gbcor20   *.05  !.01
...
25    continue
      call qgfdf(xxp,yyp,xpomr,vpau,vtau,vpac,vtac,vpac0,vtac0
     *,sumup,sumut,sumcp,sumct,sumdp,sumdt,ip,it,ip,0)      
      
      if(nppr0.eq.1.and.((jt.eq.8.or.jt.eq.3.and.nptgh0.lt.nptg0)
     *.and.psran(b10).lt.(v1p0*exp(-2.d0*(sumup+vpau(ip)))
     *-(v1p-v1p0)*exp(-sumup-vpau(ip))*(1.d0-exp(-sumup-vpau(ip))))
     */max(.001d0,(vpac0(ip)+v1p0)*exp(-2.d0*(sumup+vpau(ip)))
     *-(vpac(ip)-vpac0(ip)+v1p-v1p0)*exp(-sumup-vpau(ip))
     **(1.d0-exp(-sumup-vpau(ip))))
     *.or.jt.eq.3.and.nptgh0.eq.nptg0.and.psran(b10)
     *.lt.((v1p0*exp(-2.d0*(sumup+vpau(ip)))-(v1p-v1p0)
     **exp(-sumup-vpau(ip))*(1.d0-exp(-sumup-vpau(ip))))*2.d0**nptg0
     **exp(-2.d0*(sumut+vtau(it)))-2.d0*v1p
     **exp(-2.d0*(sumup+vpau(ip))-sumut-vtau(it)))
     */max(.001d0,((vpac0(ip)+v1p0)*exp(-2.d0*(sumup+vpau(ip)))
     *-(vpac(ip)-vpac0(ip)+v1p-v1p0)*exp(-sumup-vpau(ip))
     **(1.d0-exp(-sumup-vpau(ip))))*2.d0**nptg0
     **exp(-2.d0*(sumut+vtau(it)))-2.d0*(vpac(ip)+v1p)
     **exp(-2.d0*(sumup+vpau(ip))-sumut-vtau(it)))))then     !just one proj. leg
       nppr=1                               !number of proj. legs in the process
       xpompi(1)=xpomr                      !LC momentum for the 3P-vertex
       ipompi(1)=ip                         !proj. index for the leg
       vvxpi(1)=1.d0-exp(-sumup-sumut-vtau(it))   !screening factor
...
10     ii(m)=ii(m)+1                         !next cut fan in the vertex
	...
	goto 10
       ...
       call qgfdf(xxp,yyp,xpomm(m),vpau,vtau,vpac,vtac,vpac0,vtac0
     * ,sumup,sumut,sumcp,sumct,sumdp,sumdt,ipp,it,ia(1),0)
       sumcp0=0.d0
       if(ipp.lt.ia(1))then
        do i=ipp+1,ia(1)
         sumcp0=sumcp0+vpac0(i)
       ...
       vvxt=1.d0-exp(-vtau(it)-sumut)
       v1p=min(vpac(ipp),qgfani(1.d0/xpomm(m),bbp,vvxt,1.d0-exp(-sumcp)
     * ,1.d0-exp(-max(0.d0,sumup-sumcp)),iddp(ipp),icz,4))
       v1p0=min(v1p,qgfani(1.d0/xpomm(m),bbp,vvxt,1.d0-exp(-sumcp)  !single leg
     * ,1.d0-exp(-max(0.d0,sumup-sumcp)),iddp(ipp),icz,5))
     ...
     * .or.m.eq.levmax)then                                         !single leg
	 ...
	 goto 21
        ...
        xpompi(nppr)=xpomm(m)
        vvxpi(nppr)=1.d0-exp(-sumup-sumut-vtau(it))
        ipompi(nppr)=ipp
	bpompi(nppr)=bbp
     ...
     *  ,vvxpi(nppr)
        ...
        vpaud=qgfani(1.d0/xpomm(m),bbp,1.d0-exp(-sumdp-sumut-vtau(it))
     *  ,0.d0,0.d0,iddp(ipp),icz,1)        
        if(itypom.eq.0)then
         v3pt0=((1.d0-exp(-vpaud))**2*exp(-2.d0*sumdp)
     *   +2.d0*(1.d0-exp(-vpaud))*exp(-sumdp)*(1.d0-exp(-sumdp))
     *   +max(0.d0,(1.d0-exp(-2.d0*vpac(ipp)))*exp(-2.d0*sumcp)
     *   -2.d0*vpac(ipp)*exp(-2.d0*(sumup+vpau(ipp))))
     *   -2.d0*max(0.d0,(exp(-vpac0(ipp))-exp(-vpac(ipp)))*exp(-sumcp0
     *   -sumcp)-(vpac(ipp)-vpac0(ipp))*exp(-sumup-vpau(ipp)))
     *   -2.d0*(vpac(ipp)-vpac0(ipp)-v1p+v1p0)*exp(-sumup-vpau(ipp))
     *   *(1.d0-exp(-sumup-vpau(ipp))))*(1.d0-vvxt)**2
          vvx=1.d0-exp(-2.d0*(vpau(ipp)+vtau(it)+sumup+sumut))
	else
         v3pt0=((1.d0-exp(-vpaud))**2*exp(-2.d0*sumdp)
     *   +2.d0*(1.d0-exp(-vpaud))*exp(-sumdp)*(1.d0-exp(-sumdp))
     *   +max(0.d0,(1.d0-exp(-2.d0*vpac(ipp)))*exp(-2.d0*sumcp)
     *   -2.d0*(vpac(ipp)-v1p0)*exp(-2.d0*(sumup+vpau(ipp)))))
     *   *vvxt*(1.d0-vvxt)     
     *   +2.d0*max(0.d0,(exp(-vpac0(ipp))-exp(-vpac(ipp)))*exp(-sumcp0
     *   -sumcp)-(vpac(ipp)-vpac0(ipp))*exp(-sumup-vpau(ipp)))
     *   *(1.d0-vvxt)**2
         vvx=max(0.d0,1.d0+exp(-2.d0*(vpau(ipp)+vtau(it)+sumup+sumut))
     *   -exp(-2.d0*(vpau(ipp)+sumup)-vtau(it)-sumut)
     *   -exp(-2.d0*(vtau(it)+sumut)-vpau(ipp)-sumup))
	endif
...
11      xpomm(m+1)=xpomm(m)**psran(b10)
     ...
     *  **(1.d0+dels-alpd)
        ...
        call qgfdf(xxp,yyp,xpomm(m+1),vpau,vtau,vpac,vtac,vpac0,vtac0
     *  ,sumup,sumut,sumcp,sumct,sumdp,sumdt,ipp,it,ia(1),0)
        sumcp0=0.d0
        if(ipp.lt.ia(1))then
         do i=ipp+1,ia(1)
	...
	vpaud=qgfani(1.d0/xpomm(m+1),bbp,1.d0-exp(-sumdp-sumut
     *  -vtau(it)),0.d0,0.d0,iddp(ipp),icz,1)
        vvxt=1.d0-exp(-vtau(it)-sumut)
        v1p=min(vpac(ipp),qgfani(1.d0/xpomm(m+1),bbp,vvxt,1.d0
     *  -exp(-sumcp),1.d0-exp(-max(0.d0,sumup-sumcp)),iddp(ipp),icz,4))
        v1p0=min(v1p,qgfani(1.d0/xpomm(m+1),bbp,vvxt,1.d0-exp(-sumcp)
     *  ,1.d0-exp(-max(0.d0,sumup-sumcp)),iddp(ipp),icz,5))
        if(itypom.eq.0)then
         v3pt=((1.d0-exp(-vpaud))**2*exp(-2.d0*sumdp)
     *   +2.d0*(1.d0-exp(-vpaud))*exp(-sumdp)*(1.d0-exp(-sumdp))
     *   +max(0.d0,(1.d0-exp(-2.d0*vpac(ipp)))*exp(-2.d0*sumcp)
     *   -2.d0*vpac(ipp)*exp(-2.d0*(sumup+vpau(ipp))))
     *   -2.d0*max(0.d0,(exp(-vpac0(ipp))-exp(-vpac(ipp)))*exp(-sumcp0
	...
	aks=psran(b10)*v3pt
	v3pd=(1.d0-exp(-vpaud))**2*exp(-2.d0*sumdp)      !diffr. cut probability
     *  +2.d0*(1.d0-exp(-vpaud))*exp(-sumdp)*(1.d0-exp(-sumdp))
        if(itypom.eq.0)then
	...
	if(itypom.eq.1)then
	 v3p1=2.d0*v1p0*exp(-2.d0*(sumup+vpau(ipp)))*vvxt*(1.d0-vvxt)
	 ...
	 v3ph=2.d0*max(0.d0,(exp(-vpac0(ipp))-exp(-vpac(ipp)))
     *   *exp(-sumcp0-sumcp)-(vpac(ipp)-vpac0(ipp))
     *   *exp(-sumup-vpau(ipp)))*(1.d0-vvxt)**2
         if(aks.lt.v3pd+v3p1+v3ph)then
	  ntry=0
12	  ntry=ntry+1
	  if(ipp.eq.ia(1).or.ntry.gt.100)then
           nppm(m+1)=npgen(2.d0*(vpac(ipp)-vpac0(ipp)),2,20)
           do i=1,nppm(m+1)
	    itypm(i,m+1)=1
	    ippm(i,m+1)=ipp
	   enddo
	  else
           nppm(m+1)=npgen(2.d0*(vpac(ipp)-vpac0(ipp)),1,20)
           do i=1,nppm(m+1)
	    itypm(i,m+1)=1
	    ippm(i,m+1)=ipp
	   enddo
	   do ip1=ipp+1,ia(1)
            ninc=npgen(2.d0*(vpac(ip1)-vpac0(ip1)),0,20)
	    if(ninc.ne.0)then
...
14      ntry=ntry+1
     ...
     *  .gt.1.d0-exp(vpau(ipp)+sumup+(1.d0-npprh)*dlog(2.d0)))goto 14
...
15    continue
      ...
      call qgfdf(xxp,yyp,xpomr,vpau,vtau,vpac,vtac,vpac0,vtac0
     *,sumup,sumut,sumcp,sumct,sumdp,sumdt,ip,it,0,it)
      
      if(nptg0.eq.1.and.((jt.eq.5.or.jt.eq.2.and.npprh0.lt.nppr0)
     *.and.psran(b10).lt.(v1t0*exp(-2.d0*(sumut+vtau(it)))
     *-(v1t-v1t0)*exp(-sumut-vtau(it))*(1.d0-exp(-sumut-vtau(it))))
     */max(.001d0,(vtac0(it)+v1t0)*exp(-2.d0*(sumut+vtau(it)))
     *-(vtac(it)-vtac0(it)+v1t-v1t0)*exp(-sumut-vtau(it))
     **(1.d0-exp(-sumut-vtau(it))))
     *.or.jt.eq.2.and.npprh0.eq.nppr0.and.psran(b10)
     *.lt.((v1t0*exp(-2.d0*(sumut+vtau(it)))-(v1t-v1t0)
     **exp(-sumut-vtau(it))*(1.d0-exp(-sumut-vtau(it))))*2.d0**nppr0
     **exp(-2.d0*(sumup+vpau(ip)))-2.d0*v1t
     **exp(-2.d0*(sumut+vtau(it))-sumup-vpau(ip)))
     */max(.001d0,((vtac0(it)+v1t0)*exp(-2.d0*(sumut+vtau(it)))
     *-(vtac(it)-vtac0(it)+v1t-v1t0)*exp(-sumut-vtau(it))
     **(1.d0-exp(-sumut-vtau(it))))*2.d0**nppr0
     **exp(-2.d0*(sumup+vpau(ip)))-2.d0*(vtac(it)+v1t)
     **exp(-2.d0*(sumut+vtau(it))-sumup-vpau(ip)))))then     !just one targ. leg
       nptg=1
       xpomti(1)=xpomr                            !LC momentum for the 3P-vertex
       ipomti(1)=it                               !targ. index for the leg
       vvxti(1)=1.d0-exp(-sumut-sumup-vpau(ip))   !screening factor
...
17     ii(m)=ii(m)+1                           !next cut fan in the vertex
	...
	goto 17
       ...
       call qgfdf(xxp,yyp,xpomm(m),vpau,vtau,vpac,vtac,vpac0,vtac0
     * ,sumup,sumut,sumcp,sumct,sumdp,sumdt,ip,itt,0,ia(2))
       sumct0=0.d0
       if(itt.lt.ia(2))then
        do i=itt+1,ia(2)
         sumct0=sumct0+vtac0(i)
       ...
       vvxp=1.d0-exp(-vpau(ip)-sumup)       
       v1t=min(vtac(itt),qgfani(xpomm(m)*scm,bbt,vvxp,1.d0-exp(-sumct)
     * ,1.d0-exp(-max(0.d0,sumut-sumct)),iddt(itt),2,4))
       v1t0=min(v1t,qgfani(xpomm(m)*scm,bbt,vvxp,1.d0-exp(-sumct)  !single leg
     * ,1.d0-exp(-max(0.d0,sumut-sumct)),iddt(itt),2,5))
     ...
     * .or.m.eq.levmax)then
	 ...
	 goto 21
        ...
        xpomti(nptg)=xpomm(m)
        vvxti(nptg)=1.d0-exp(-sumut-sumup-vpau(ip))
        ipomti(nptg)=itt
	bpomti(nptg)=bbt
     ...
     *  ,vvxti(nptg)
        ...
        if(debug.ge.4)write (moniou,210)m
        vtaud=qgfani(xpomm(m)*scm,bbt,1.d0-exp(-sumdt-sumup-vpau(ip))
     *  ,0.d0,0.d0,iddt(itt),2,1)
        if(itypom.eq.0)then
         v3pt0=((1.d0-exp(-vtaud))**2*exp(-2.d0*sumdt)
     *   +2.d0*(1.d0-exp(-vtaud))*exp(-sumdt)*(1.d0-exp(-sumdt))
     *   +max(0.d0,(1.d0-exp(-2.d0*vtac(itt)))*exp(-2.d0*sumct)
     *   -2.d0*vtac(itt)*exp(-2.d0*(sumut+vtau(itt))))
     *   -2.d0*max(0.d0,(exp(-vtac0(itt))-exp(-vtac(itt)))*exp(-sumct0
     *   -sumct)-(vtac(itt)-vtac0(itt))*exp(-sumut-vtau(itt)))
     *   -2.d0*(vtac(itt)-vtac0(itt)-v1t+v1t0)*exp(-sumut-vtau(itt))
     *   *(1.d0-exp(-sumut-vtau(itt))))*(1.d0-vvxp)**2
          vvx=1.d0-exp(-2.d0*(vpau(ip)+vtau(itt)+sumup+sumut))
	else
         v3pt0=((1.d0-exp(-vtaud))**2*exp(-2.d0*sumdt)
     *   +2.d0*(1.d0-exp(-vtaud))*exp(-sumdt)*(1.d0-exp(-sumdt))
     *   +max(0.d0,(1.d0-exp(-2.d0*vtac(itt)))*exp(-2.d0*sumct)
     *   -2.d0*(vtac(itt)-v1t0)*exp(-2.d0*(sumut+vtau(itt)))))
     *   *vvxp*(1.d0-vvxp)     
     *   +2.d0*max(0.d0,(exp(-vtac0(itt))-exp(-vtac(itt)))*exp(-sumct0
     *   -sumct)-(vtac(itt)-vtac0(itt))*exp(-sumut-vtau(itt)))
     *   *(1.d0-vvxp)**2
         vvx=max(0.d0,1.d0+exp(-2.d0*(vpau(ip)+vtau(itt)+sumup+sumut))
     *   -exp(-2.d0*(vpau(ip)+sumup)-vtau(itt)-sumut)
     *   -exp(-2.d0*(vtau(itt)+sumut)-vpau(ip)-sumup))
	endif
...
18      xpomm(m+1)=xpomm(m)/(xpomm(m)*scm)**psran(b10)
	...
	vtaud=qgfani(xpomm(m+1)*scm,bbt,1.d0-exp(-sumdt-sumup
     *  -vpau(ip)),0.d0,0.d0,iddt(itt),2,1)
        vvxp=1.d0-exp(-vpau(ip)-sumup)
        v1t=min(vtac(itt),qgfani(xpomm(m+1)*scm,bbt,vvxp,1.d0
     *  -exp(-sumct),1.d0-exp(-max(0.d0,sumut-sumct)),iddt(itt),2,4))
        v1t0=min(v1t,qgfani(xpomm(m+1)*scm,bbt,vvxp,1.d0-exp(-sumct)
     *  ,1.d0-exp(-max(0.d0,sumut-sumct)),iddt(itt),2,5))
        if(itypom.eq.0)then
         v3pt=((1.d0-exp(-vtaud))**2*exp(-2.d0*sumdt)
     *   +2.d0*(1.d0-exp(-vtaud))*exp(-sumdt)*(1.d0-exp(-sumdt))
     *   +max(0.d0,(1.d0-exp(-2.d0*vtac(itt)))*exp(-2.d0*sumct)
     *   -2.d0*vtac(itt)*exp(-2.d0*(sumut+vtau(itt))))
     *   -2.d0*max(0.d0,(exp(-vtac0(itt))-exp(-vtac(itt)))*exp(-sumct0
	...
	aks=psran(b10)*v3pt
        v3pd=(1.d0-exp(-vtaud))**2*exp(-2.d0*sumdt)      !diffr. cut probability
     *  +2.d0*(1.d0-exp(-vtaud))*exp(-sumdt)*(1.d0-exp(-sumdt))
        if(itypom.eq.0)then
	...
	if(itypom.eq.1)then
	 v3p1=2.d0*v1t0*exp(-2.d0*(sumut+vtau(itt)))*vvxp*(1.d0-vvxp)
	 if(aks.lt.v3pd+v3p1)then
          nptg=nptg+1
          if(nptg.gt.legmax)then
	   iret=1
	 ...
	 v3ph=2.d0*max(0.d0,(exp(-vtac0(itt))-exp(-vtac(itt)))
     *   *exp(-sumct0-sumct)-(vtac(itt)-vtac0(itt))
     *   *exp(-sumut-vtau(itt)))*(1.d0-vvxp)**2
	 if(aks.lt.v3pd+v3p1+v3ph)then
	  ntry=0
19	  ntry=ntry+1
	  if(itt.eq.ia(2).or.ntry.gt.100)then
           nppm(m+1)=npgen(2.d0*(vtac(itt)-vtac0(itt)),2,20)
           do i=1,nppm(m+1)
	    itypm(i,m+1)=1
	   ippm(i,m+1)=itt
	   enddo
	  else
           nppm(m+1)=npgen(2.d0*(vtac(itt)-vtac0(itt)),1,20)
           do i=1,nppm(m+1)
	    itypm(i,m+1)=1
	   ippm(i,m+1)=itt
	   enddo
	   do it1=itt+1,ia(2)
            ninc=npgen(2.d0*(vtac(it1)-vtac0(it1)),0,20)
	    if(ninc.ne.0)then
             nppm(m+1)=nppm(m+1)+ninc
             if(nppm(m+1).gt.legmax)then
	      iret=1
...
20	ntry=ntry+1
     ...
     *  .gt.1.d0-exp(vtau(itt)+sumut+(1.d0-nptgh)*dlog(2.d0)))goto 20
...
c-----------------------------------------------------------------------------
      implicit double precision (a-h,o-z)
      integer debug
      parameter(iapmax=209,npbmax=1000,npnmax=900,npmax=500
...
c      external psran
      ...
      if(debug.ge.1)write (moniou,201)nbpom,ncola,ncolb
      nsp0=nsp
...
c particle production for all cut Pomerons
     ...
     *     ,icdp,icdt,icz,iqq,0)            !QCD evolution + jet hadronization
	 ...
	 do l=1,npompr(npb)                !loop over proj. leg Pomerons
	  ...
	  if(debug.ge.2)write (moniou,214)l,wpi,wmi,wsoft,wgg,wqg,aks
...
201   format(2x,'qgsha - inelastic interaction, N of Pomeron blocks:',i4
     */4x,'N of wounded proj. nucleons:',i3
     *,2x,'N of wounded targ. nucleons:',i3)
...
214   format(2x,'qgsha: particle production for '
     ...
     */4x,'light cone momenta for the Pomeron:',2e10.3
     */4x,'wsoft=',e10.3,2x,'wgg=',e10.3,2x,'wqg=',e10.3
     *,2x,'aks=',e10.3)
...
c-------------------------------------------------------------------------
      ...
      integer debug
      parameter(iapmax=209,npbmax=1000,npnmax=900,npmax=500,legmax=900)
      dimension alpi(npnmax,iapmax),alti(npnmax,iapmax)
     *,alpt(iapmax),altt(iapmax)
      common /qgarr1/  ia(2),icz,icp
      common /qgarr2/  scm,wp0,wm0
      common /qgarr6/  pi,bm,amws
      common /qgarr9/  iwp(iapmax),iwt(iapmax),lqa(iapmax),lqb(iapmax)
...
c      external psran
      ...
      jexip=0
      jexit=0
      if(jexpr.eq.-2.or.jexpr.gt.0.and.psran(b10).lt.wex(icz)
     *.and.iabs(izp).lt.7)jexip=1
      if(jextg.eq.-2.or.jextg.gt.0.and.psran(b10).lt.wex(2)
     *.and.iabs(izt).lt.7)jexit=1
      
      if(wppr.eq.wp0.and.jexpr.gt.0.and.jexip.eq.0.and.iabs(izp).lt.7)
     *jexip=1
      if(wmtg.eq.wm0.and.jextg.gt.0.and.jexit.eq.0.and.iabs(izt).lt.7)
     *jexit=1
      ...
      if(jexip.eq.1)then
       if(ddmax1.gt.ddmin1)then
        dmass1=ddmin1*(1.d0-psran(b10)
     *  *(1.d0-(ddmax1/ddmin1)**(alpd-1.d0)))**(1.d0/(alpd-1.d0))
       else
        dmass1=am(icz)
	jexip=0
       endif
      ...
      if(jexit.eq.1)then
       ...
       if(ddmax2.gt.ddmin2)then
        dmass2=ddmin2*(1.d0-psran(b10)
     *  *(1.d0-(ddmax2/ddmin2)**(alpd-1.d0)))**(1.d0/(alpd-1.d0))
       else
        dmass2=am(2)
	jexit=0
       endif
...
2     ij=ij+1
      ...
      if(iq2.eq.0)then
       ...
       if(qgnrm(ept).gt.(am1+am(1))**2)then
...
c-------------------------------------------------------------------------------
      ...
      integer debug
      parameter(iapmax=209)
      dimension wabs(28),wdd(28),wqel(28),wcoh(28),wtot(28)
...
c=======================================================================
      function psran(arg)
c-----------------------------------------------------------------------
c  ran(dom generator for qgs)
c  special version of rmmar for sequence one and one random number
c  see subroutine rmmar
c  this functon is called from qgs routines
c  cern proglib# v113    rmmar           .version kernfor  1.0
c  orig. 01/03/89 fca + fj
c  simplified for taking just one random number
c  changes  : d. heck    ik3  fzk karlsruhe
c-----------------------------------------------------------------------
      double precision arg,psran
      real uni
      common /ranma2/ iu(1133),jseq
      common /ranma3/ twom24,twom48,cd,cm,cint,modcns
      ...
      equivalence (j97(0),iu(103))
c-----------------------------------------------------------------------
 1    uni = u(i97(0))-u(j97(0))
      if ( uni .lt. 0. ) uni = uni+1.
      u(i97(0)) = uni
      i97(0) = i97(0)-1
      if ( i97(0) .eq. 0 ) i97(0) = 97
      j97(0) = j97(0)-1
      if ( j97(0) .eq. 0 ) j97(0) = 97
      c(0)   = c(0) - cd
      if ( c(0) .lt. 0. ) c(0) = c(0)+cm
      uni = uni-c(0)
      if ( uni .lt. 0. ) uni = uni+1.
c  replace exact zeroes by uniform distr. *2**-24
      if ( uni .eq. 0. ) then
...
c  an exact zero here is very unlikely, but let's be safe.
      ...
      ntot(0) = ntot(0) + 1
      if ( ntot(0) .ge. modcns )  then
        ntot2(0) = ntot2(0) + 1
        ntot(0)  = ntot(0) - modcns
      endif

      if ( uni .gt. 0.999999 ) goto 1
      if ( uni .lt. 0.000001 ) goto 1
      psran = uni
      return
      end

...
c-----------------------------------------------------------------------------
      ...
      common /debug/   debug
cc ///////////
cc      moniou=6             !output channel for debugging
...
c normalization of parton density in the soft Pomeron
      rr=gamqgs(real(2.d0+betp-dels))/gamqgs(real(1.d0-dels))
     */gamqgs(real(1.d0+betp))/4.d0/pi
...
c Pomeron-hadron vertex
      fp(2)=(1.d0-qnorm2)*gamqgs(real(3.d0-alpd+ahl(2)))
     */gamqgs(real(1.d0+ahl(2)))/gamqgs(real(2.d0-alpd))
      do icz=1,3,2 
       fp(icz)=gamqgs(real(3.d0-alpd+ahl(icz)))/gamqgs(real(2.d0-alpd))
     * /gamqgs(real(1.d0+ahl(icz)))*(1.d0-qnorm1)
      enddo
...
ccc      inquire(file='qgsdat-II-03',exist=lcalc)
c/////////
      if(lcalc)then
       if(debug.ge.1)write (moniou,203)
c//////// KK
cc       open(1,file='qgsdat-II-03',form='unformatted')
cc       read (1) csborn,cs0,cstot,evk,qppdi,qlegi,qfanu,qfan
cc     * ,qpomu,qpom1,qpomr,qlegc,qpomc,qpomi,qpomi0,gsect,fsud,qrt
...
c integrated cut Pomeron eikonals
     ...
     * *gamqgs(real(1.d0-alpd+dels))**2*gamqgs(real(1.d0+ahl(icz)))
     * *gamqgs(real(1.d0+ahl(2)))/gamqgs(real(2.d0-alpd+dels+ahl(icz)))
     * /gamqgs(real(2.d0-alpd+dels+ahl(2)))
...
ccc /////// KK This need not be modifed if
c         you use bin2ascii.f later
...
c///////// KK
cc      inquire(file='sectnu-II-03',exist=lcalc)
...
c////////// KK
cc       open(2,file='sectnu-II-03',status='old')
...
1     continue
      do i=1,ia(1)
       iddp(i)=1+int(qgsrn(b10)+cc(2,icz)) !diffractive eigenstates for proj.
      enddo
      do i=1,ia(2)
       iddt(i)=1+int(qgsrn(b10)+cc(2,2))   !diffractive eigenstates for targ.
      enddo

c-------------------------------------------------
c squared impact parameter is sampled uniformly (b**2<bm**2)
      b=bm*dsqrt(qgsrn(b10))
...
c calculate nuclear screening factors for "middle point" -> eikonals
	...
	vv3p=qgpomi(scm,bbp,vvxp,vvxt,vvxpa,vvxta,icdp,icdt,icz,3) !multi-Pomeron
        ...
        if(qgsrn(b10).gt.1.d0-exp(-2.d0*vv))goto 3  !1.-exp(-2*vv) - probability
...
3      continue           !end of it-loop
     ...
     *  /(cc(1,icz)*exp(-2.d0*vabs(1))+cc(2,icz)*exp(-2.d0*vabs(2)))
        ...
        if(qgsrn(b10).lt.wdifr)then       !projectile diffraction
	 wdift=0.d0
         do it=1,ia(2)
	  if(iwt(it).ne.-1)then
...
5     continue                            !end of ip-loop
     ...
     *  /(cc(1,2)*exp(-2.d0*vabs(1))+cc(2,2)*exp(-2.d0*vabs(2)))
        ...
        if(qgsrn(b10).lt.wdifr)then       !target diffraction
	 wdift=0.d0
         do ip=1,ia(1)
	  if(iwp(ip).eq.-1)then
	   wdifi(ip)=0.d0
...
8     continue 
! define collision type (1 - NSD, 2 - DD, 3 - SD, 4 - CD)
      typevt=1
      if(ia(1).eq.1.and.ia(2).eq.1)then     !only for h-h
         if(lqa(1).gt.0.and.lqb(1).gt.0)then !NSD (at least one cut P attached)
            typevt=1
         elseif(nwp.eq.0.and.nwt.eq.0)then !no interaction
            typevt=0
         elseif(lqa(1).gt.0.and.iwt(1).eq.2
     *           .or.lqb(1).gt.0.and.iwp(1).eq.2
     *           .or.iwp(1).eq.2.and.iwt(1).eq.2)then !DD
            typevt=2                          
         elseif(lqa(1).gt.0.or.lqb(1).gt.0
     *           .or.iwp(1).eq.2.or.iwt(1).eq.2)then !SD
            typevt=3
         else                   !CD
            typevt=4
         endif
...
1     continue
      xpomr=scm**(-qgsrn(b10))               !proposed LC momentum for 3P-vertex    
      gbcor1=(1.d0-(1.d0-(1.d0-xpomr)**(1.d0+ahl(icz)))
     ***(1.d0+dels-alpd))
      gbcor2=(1.d0-(1.d0-(1.d0-1.d0/xpomr/scm)
     ***(1.d0+ahl(2)))**(1.d0+dels-alpd))	  
      if(qgsrn(b10).gt.gbcor1*gbcor2*(gbcor1+gbcor2)
     ...
     *,xxp,yyp,int(1.5d0+qgsrn(b10)))     !determine coordinates for the vertex      
      call qgfdf(xxp,yyp,xpomr,vpac,vtac,sumup,sumut,sumcp,sumct
     *,ip,it,ia(1),ia(2))
...
3      ntry=ntry+1
       ...
       if(ip.eq.ia(1).or.ntry.gt.100)then
        ...
        do i=1,nppr0
	 if(qgsrn(b10).lt.vpac0(ip)/vpac(ip))then
	  itypr0(i)=0
	 else
          npprh0=npprh0+1
	  itypr0(i)=1
	...
	do ipp=ip+1,ia(1)
	 ...
	 if(ninc.ne.0)then
	  ...
	  do i=nppr0-ninc+1,nppr0
	   if(qgsrn(b10).lt.vpac0(ipp)/vpac(ipp))then
	    itypr0(i)=0
	   else
            npprh0=npprh0+1
	    itypr0(i)=1
...
4      ntry=ntry+1
       ...
       if(it.eq.ia(2).or.ntry.gt.100)then
        ...
        do i=1,nptg0
	 if(qgsrn(b10).lt.vtac0(it)/vtac(it))then
	  itytg0(i)=0
	 else
          nptgh0=nptgh0+1
	  itytg0(i)=1
	...
	do itt=it+1,ia(2)
	 ...
	 if(ninc.ne.0)then
	  ...
	  do i=nptg0-ninc+1,nptg0
	   if(qgsrn(b10).lt.vtac0(itt)/vtac(itt))then
	    itytg0(i)=0
	   else
            nptgh0=nptgh0+1
	    itytg0(i)=1
...
6      ntry=ntry+1
     ...
     * -(vpac(ip)-vpac0(ip)+v1p-v1p0)*exp(-sumup)*(1.d0-exp(-sumup)))
      ...
      nptg=0      
      npin=0
      if((jt.eq.5.or.jt.eq.2.and.npprh0.lt.nppr0).and.qgsrn(b10)
     ...
     *.or.jt.eq.2.and.npprh0.eq.nppr0.and.qgsrn(b10)
     *.lt.(vtac(it)-vtac0(it)+v1t-v1t0)*(exp(-sumut)*(1.d0-exp(-sumut))
...
7      xpomr1=xpomr/(xpomr*scm)**qgsrn(b10)
	...
	goto 21
      ...
      if((jt.eq.8.or.jt.eq.3.and.nptgh0.lt.nptg0).and.qgsrn(b10)
     ...
     *.or.jt.eq.3.and.nptgh0.eq.nptg0.and.qgsrn(b10)
     *.lt.(vpac(ip)-vpac0(ip)+v1p-v1p0)*(exp(-sumup)*(1.d0-exp(-sumup))
...
8      xpomr1=xpomr**qgsrn(b10)
     ...
     *+gbitg*max(0.d0,vtact0(it)-vtac0(it)))/4.d0
      ...
      nprin=npgen(gbi/gbs,0,npmax)
      ntgin=npgen(gbi/gbs,0,npmax)
     ...
     * **(1.d0+dels-alpd)
       do in=1,nprin
	n=0
22      xpomr1=xpomr**qgsrn(b10)
        gbcor1=1.d0-(1.d0-(1.d0-xpomr1)**(1.d0+ahl(icz)))
     *  **(1.d0+dels-alpd)
        if(qgsrn(b10).gt.gbcor1/gbcor10)goto 22
...
23    continue
     ...
     * **(1.d0+ahl(2)))**(1.d0+dels-alpd)
       do in=1,ntgin
	n=0
24      xpomr1=xpomr/(xpomr*scm)**qgsrn(b10)
        gbcor2=1.d0-(1.d0-(1.d0-1.d0/xpomr1/scm)
     *  **(1.d0+ahl(2)))**(1.d0+dels-alpd)
        if(qgsrn(b10).gt.gbcor2/gbcor20)goto 24
...
25    continue     
     ...
     *.and.qgsrn(b10).lt.(v1p0*exp(-2.d0*sumup)
...
10     ii(m)=ii(m)+1                         !next cut fan in the vertex
     ...
     *  ,1.d0-exp(-sumut),iddp(ip1),icz,5))
       ...
       if(itypom.eq.0.and.qgsrn(b10).lt.v1p0/vpac0(ipp)
...
11      xpomm(m+1)=xpomm(m)**qgsrn(b10)
        gbcor1=1.d0-(1.d0-(1.d0-xpomm(m+1))**(1.d0+ahl(icz)))
     *  **(1.d0+dels-alpd)
        if(qgsrn(b10).gt.gbcor1/gbcor10)goto 11
...
14      ntry=ntry+1
	...
	if(ipp.eq.ia(1).or.ntry.gt.100)then
	 ...
	 do ip1=ipp+1,ia(1)
	  ...
	  if(ninc.ne.0)then
	   ...
	   do i=nppm(m+1)-ninc+1,nppm(m+1)
	    if(qgsrn(b10).lt.vpac0(ip1)/vpac(ip1))then
	     itypm(i,m+1)=0
	    else
             npprh=npprh+1
	     itypm(i,m+1)=1
	...
	if(itypom.eq.0.and.npprh.eq.nppm(m+1).and.qgsrn(b10)
...
15    continue
     ...
     *.and.qgsrn(b10).lt.(v1t0*exp(-2.d0*sumut)
...
17     ii(m)=ii(m)+1                           !next cut fan in the vertex
     ...
     *  ,iddt(it1),2,5))
       ...
       if(itypom.eq.0.and.qgsrn(b10).lt.v1t0/vtac0(itt)            !single leg
     * .or.m.eq.levmax)then
        nptg=nptg+1
...
18      xpomm(m+1)=xpomm(m)/(xpomm(m)*scm)**qgsrn(b10)
        gbcor2=1.d0-(1.d0-(1.d0-1.d0/xpomm(m+1)/scm)
     *  **(1.d0+ahl(2)))**(1.d0+dels-alpd)
        if(qgsrn(b10).gt.gbcor2/gbcor20)goto 18
...
20	ntry=ntry+1
	...
	if(itt.eq.ia(2).or.ntry.gt.100)then
	 ...
	 do it1=itt+1,ia(2)
	  ...
	  if(ninc.ne.0)then
	   ...
	   do i=nppm(m+1)-ninc+1,nppm(m+1)
	    if(qgsrn(b10).lt.vtac0(it1)/vtac(it1))then
	     itypm(i,m+1)=0
	    else
             nptgh=nptgh+1
	     itypm(i,m+1)=1
	...
	if(itypom.eq.0.and.nptgh.eq.nppm(m+1).and.qgsrn(b10)
...
c      external qgsrn
 	...
 	aks=aks-vvn
      ...
      elseif(npmin.eq.2)then
       aks=qgsrn(b10)*(1.d0-exp(-vv)*(1.d0+vv))
       vvn=vv*exp(-vv)
       do n=2,npmax
 	vvn=vvn*vv/dble(n)
 	aks=aks-vvn
...
c initial nucleon (hadron) types
      if(ia(1).ne.1)then
       do i=1,ia(1)                
        izp(i)=int(2.5d0+qgsrn(b10))   !i-th projectile nucleon type
       enddo
      else
      ...
      if(ia(2).ne.1)then
       do i=1,ia(2)
        izt(i)=int(2.5d0+qgsrn(b10))   !i-th target nucleon type
       enddo
      else
...
2      continue
       ...
       if(gbh.lt.-50.d0.or.qgsrn(b10).gt.exp(gbh))then
        nrej=nrej+1
...
c particle production for all cut Pomerons
	...
	if(nqs(npb).ne.0)then
	  ...
	  aks=qgsrn(b10)*(wsoft+wgg+wqg+wgq+wqq)
...
c to assure an efficient procedure
     ...
     *    .or.delf-alpd.lt.0.d0.and.alfl.le.0.d0)then
           up=1.d0-qgsrn(b10)**(1.d0/(1.d0-alpd+delf))
	   ...
	   gb=(tp/(1.d0-up))**(-alpd+delf)
          elseif(delf-alpd.lt.0.d0.and.alfl.gt.0.d0)then
           up=-log(1.d0-qgsrn(b10)*(1.d0-exp(-alfl*(1.d0-alpd+delf))))
     *     /alfl/(1.d0-alpd+delf)
           tp=up**(1.d0/(1.d0-alpd+delf))
	   gb=(1.d0-tp)**alfl*exp(alfl*(1.d0-alpd+delf)*up)/gb0
	  else
           tp=1.d0-qgsrn(b10)**(1.d0/(1.d0+alfl))
	   gb=tp**(delf-alpd)
          endif
	  if(qgsrn(b10).gt.gb)then
	   nrej=nrej+1
	   goto 1
          endif
...
2      zpm=(1.d0-qgsrn(b10)*(1.d0-xmin**(delh-dels)))
     * **(1.d0/(delh-dels))
       ...
       if(iqq.eq.0)then              !gg-ladder
        xp=zpm**qgsrn(b10)           !LC+ momentum share
...
c iqc(2) - the same for the lower one
      if(iqq.ne.0.and.iqq.ne.2)then          !q_v from the proj.
       ...
       nj=nj+1
       if(nj.gt.njmax)stop'increase njmax!!!'
       if(qgsrn(b10).lt.dc(2))then
        iqj(nj)=-4
       else
        iqj(nj)=-int(2.d0*qgsrn(b10)+1.d0)
       endif
       iqj(nj+1)=-iqj(nj)
       wp1=wpp-wpi
       wp2=wp1*qgsrn(b10)
       ...
       if(jqq.eq.0.or.iqq.eq.0.and.jqq.eq.2)then
        ...
        if(qgsrn(b10).lt.dc(2))then
         iqc(1)=3*(2.d0*int(.5d0+qgsrn(b10))-1.d0)
	else
         iqc(1)=int(2.d0*qgsrn(b10)+1.d0)
     *   *(2.d0*int(.5d0+qgsrn(b10))-1.d0)
        endif
12      zg=xp+qgsrn(b10)*(1.d0-xp)           !gluon splitting into qq~
        if(qgsrn(b10).gt.zg**dels*((1.d0-xp/zg)/ (1.d0-xp))**betp)
     *  goto 12
        xg=xp/zg
        wpq0=wpp*(xg-xp)
        wmq=1.d0/wpq0
	...
	if(iabs(iqc(1)).eq.3)iqj(nj)=iqj(nj)*4/3
      ...
      if((iqq-2)*(iqq-3)*(iqq-4).eq.0)then     !q_v from the targ.
       ...
       if(jqq.eq.0.or.iqq.eq.0.and.jqq.eq.1)then
        ...
        if(qgsrn(b10).lt.dc(2))then
         iqc(2)=3*(2.d0*int(.5d0+qgsrn(b10))-1.d0)
	else
         iqc(2)=int(2.d0*qgsrn(b10)+1.d0)
     *   *(2.d0*int(.5d0+qgsrn(b10))-1.d0)
        endif
14      zg=xm+qgsrn(b10)*(1.d0-xm)           !gluon splitting into qq~
        if(qgsrn(b10).gt.zg**dels*((1.d0-xm/zg)/ (1.d0-xm))**betp)
     *  goto 14
        xg=xm/zg
        wmq0=wpm*(xg-xm)
        wpq=1.d0/wmq0
	...
	if(iabs(iqc(2)).eq.3)iqj(nj)=iqj(nj)*4/3
      ...
      si=qgnrm(ept)
      jini=1
      jj=int(1.5d0+qgsrn(b10)) !1st parton at upper (jj=1) or downer (jj=2) leg
...
3     continue
      ...
      if(jini.eq.1)then                   !general ladder
       ...
       sjb=0.d0
       if(qgsrn(b10).lt.sj1/sj)then       !change to one-way ordered ladder
        jj=3-jj
	sj=sj1
	...
	jini=0
      ...
      if(qgsrn(b10).lt.sjb/sj)goto 6      !born process sampled
...
c propose x, q^2
4     continue
      if(qgsrn(b10).gt.djl)then
       x=(xmin1+qgsrn(b10)*(xmax1-xmin1))**(1.d0/delh) !parton LC share
      else
       x=1.d0-(1.d0-xmin2)*((1.d0-xmax)/(1.d0-xmin2))**qgsrn(b10)
      endif
      qq=qqmin/(1.d0+qgsrn(b10)*(qqmin/qqmax-1.d0))    !parton virtuality
      ...
      if(qgsrn(b10).gt.gb7)goto 4          !rejection

c-------------------------------------------------
c define color flow for the emitted jet; perform final state emission
      nqc(2)=0
      if(qgsrn(b10).lt.sj1/(sj1+sj2))then         !new gluon-leg ladder
       if(iqc(jj).eq.0)then                       !g -> gg
        jt=1
        jq=int(1.5d0+qgsrn(b10))
       ...
       if(iqc(jj).ne.0)then                       !q -> gq
        ...
        jq=int(1.5d0+qgsrn(b10))                  !orientation of color flow
        iq1=int(3.d0*qgsrn(b10)+1.d0)*(3-2*jq)    !jet flavor (type)
     ...
     *  /qgjit1(qq,qmin(3-jj),s2,iq,iqp(3-jj),2)
       ...
       if(qgsrn(b10).gt.gb)goto 1        !jet mass correction for the acceptance
...
7     q2=tmin/(1.d0-qgsrn(b10)*(1.d0-2.d0*tmin/si))   !proposed q^2
      z=q2/si                                         !parton LC momentum share
      qt2=q2*(1.d0-z)                                 !parton p_t^2
      if(qgsrn(b10).lt..5d0)then
       jm=2
       tq=si-q2
      else
       jm=1
     ...
     **qgsuds(qt2/fqscal,iqp(1))*qgsuds(qt2/fqscal,iqp(2))/gb0
      ...
      if(qgsrn(b10).gt.gb)goto 7                      !rejection
...
2     qlmax=dlog(qmax(nrow,nlev)/qtf/16.d0)
      ...
      if(qgsrn(b10).gt.qgsudi(qlmax,iq))then
       q=qgqint(qlmax,qgsrn(b10),iq)
       z=qgzsim(q,iq)
       ll=lnv(nlev+1)+1
       ldau(nrow,nlev)=ll
       lpar(ll,nlev+1)=nrow
       ...
       if(iq.ne.1)then
        ...
        if(qgsrn(b10).lt.wg)then
         iqm(1)=0
         iqm(2)=0
        else
         iqm(1)=int(3.d0*qgsrn(b10)+1.d0)*(3-2*jq)
         iqm(2)=-iqm(1)
        endif
        if(qgsrn(b10).lt..5d0)z=1.d0-z
       endif
       qv(nrow,nlev)=q
       zv(nrow,nlev)=z
       nrow=ll
...
c      external qgsrn
      ...
      if(debug.ge.3)write (moniou,201)    
1     s1=2.d0*qgsrn(b10)-1.d0
      s2=2.d0*qgsrn(b10)-1.d0
      s3=s1*s1+s2*s2
...
c      external qgsrn
      ...
      if(ia.ge.10)then                 !light nucleus -> Van-Hove method
       do i=1,ia
1       zuk=qgsrn(b10)*cr1(jj)-1.d0
        if(zuk)2,2,3
2       tt=rnuc(jj)/wsnuc(jj)*(qgsrn(b10)**.3333d0-1.d0)
        goto 6
3       if(zuk.gt.cr2(jj))goto 4
        tt=-dlog(qgsrn(b10))
        goto 6
4       if(zuk.gt.cr3(jj))goto 5
        tt=-dlog(qgsrn(b10))-dlog(qgsrn(b10))
        goto 6
5       tt=-dlog(qgsrn(b10))-dlog(qgsrn(b10))-dlog(qgsrn(b10))
6       rim=tt*wsnuc(jj)+rnuc(jj)
        if(qgsrn(b10).gt.(1.d0+wbnuc(jj)*rim**2/rnuc(jj)**2)
     *  /(1.d0+exp(-abs(tt))))goto 1
        z=rim*(2.d0*qgsrn(b10)-1.d0)
       ...
       do l=1,3
        ...
        do i=1,ia-1
         j=ia-i
         aks=rnuc(jj)*(qgsrn(b10)+qgsrn(b10)+qgsrn(b10)-1.5d0)
         k=j+1
...
c      external qgsrn
      ...
      is=iabs(ich)/ich
      if(icz.eq.1)then
       ic1=ich*(1-3*int(.5d0+qgsrn(b10)))
       ic2=-ic1-ich
      elseif(icz.eq.2)then
       if(qgsrn(b10).gt..33333d0.or.ich.lt.0)then
        ic1=ich-is
        ic2=3*is
       else
        ic1=4*is-ich
...
1     continue
      if(j.eq.1)then
       qgzsim=.5d0*(2.d0*zmin)**qgsrn(b10)
       gb=qgzsim*(qgfap(qgzsim,1,1)+qgfap(qgzsim,1,2))/7.5d0
      else
       qgzsim=zmin*((1.d0-zmin)/zmin)**qgsrn(b10)
...
c      external qgsrn
	...
	if(iabs(ic1).eq.1)then
	 ...
	 ich1=4*is
        ...
        ich1=ich*int(.5d0+qgsrn(b10))
        ic2=-ic1*iabs(ich1)-(ich+ic1)*iabs(ich-ich1)
       endif
      elseif(icz.eq.2)then
       ic1=int(1.3333d0+qgsrn(b10))   !valence quark type (for proton)
       if(ic1.eq.1)then
        ich1=int(qgsrn(b10)+.5d0)+2   !leading nucleon type
        ic2=1-ich1
       elseif(qgsrn(b10).lt..5d0)then
        ich1=2
	ic2=-2
       else
        ich1=7                        !uuu
...
c      external qgsrn
      ...
      if(jexpr.gt.0)then                                 !so161205
       if(qgsrn(b10).gt.(1.d0-wex(icz))**jexpr)jexip=1   !so161205
      endif                                              !so161205
      if(jextg.eq.-2.or.jextg.gt.0.and.qgsrn(b10)
      ...
      if(qgsrn(b10).lt..5d0)then
       if(jexip.eq.1)then
        if(ddmax1.gt.ddmin1)then
         dmass1=ddmin1*(1.d0-qgsrn(b10)*(1.d0-(ddmax1/ddmin1)
 	 ...
 	 jexip=0
       ...
       if(jexit.eq.1)then
        ...
        if(ddmax2.gt.ddmin2)then
         dmass2=ddmin2*(1.d0-qgsrn(b10)*(1.d0-(ddmax2/ddmin2)
     *   **(alpd-1.d0)))**(1.d0/(alpd-1.d0))
        else
         if(iabs(izt).ge.7)then
          dmass2=dmmin(2)
...
1       pti=ptmax*qgsrn(b10)
        if(qgsrn(b10).gt.exp(-dsqrt(pti)/ptmean))goto 1
       else
2       pti=(ptmean*dlog(qgsrn(b10)*qgsrn(b10)))**2
	  ...
	  if(qgsrn(b10).lt..5d0)then
	   ...
	   ic2=ics
         ...
         elseif(izp.ne.0)then
          ic1=izp*(1-3*int(.5d0+qgsrn(b10)))
          ic2=-izp-ic1
         else
          ic1=int(1.5d0+qgsrn(b10))*(2*int(.5d0+qgsrn(b10))-1)
          ic2=-ic1
         endif
        elseif(icz.eq.2)then
         if(iabs(izp).ge.7)then
...
2     eex=0.d0         
      do i=1,ia(1)-ns
       eex=eex+(qgsrn(b10)+qgsrn(b10)+qgsrn(b10)+
     * qgsrn(b10)+qgsrn(b10)-2.5d0)**2*2.4d0
      enddo     
...
c-----------------------------------------------------------------------------
      ...
      integer debug
      character *2 tyq
      dimension wp(2),ic(2),ept(4),ep(4),ep1(4),ey(3),ey0(3)
...
1     sww=dsqrt(ww)
     ...
     *iab.eq.5.and.sww.gt.restm+am0+amc+wwm)then
       if(iab.le.2)then
        if(sww.gt.restm+2.d0*amn.and.qgsrn(b10).lt.dc(1))then
c nucleon generation
         ...
         ic0=ic(j)+is
         ic(j)=-3*is
        elseif(sww.gt.restm+2.d0*amk.and.qgsrn(b10).lt.dc(2))then
c kaon generation
         restm=(restm+amk)**2
         bet=bek
         ami=amk**2
         ...
         ic0=ic(j)+3*is
         ic(j)=4*is
        elseif(sww.gt.restm+ameta+am0.and.qgsrn(b10).lt.deta)then
c eta generation
         restm=(restm+am0)**2
         bet=bek
         ami=ameta**2
...
c pion generation
       ...
       elseif(iab.eq.3)then
        if(sww.gt.restm+amk+amlam.and.qgsrn(b10).lt.dc(4)
...
c nucleon generation
       ...
       elseif(iab.eq.4)then
        if(sww.gt.restm+amn+amlam.and.qgsrn(b10).lt.dc(1))then
c lambda generation
         restm=(restm+amn)**2
         bet=ben
         ami=amlam**2
...
2       pti=ptmax*qgsrn(b10)
        if(qgsrn(b10).gt.exp(-dsqrt(pti)/bet))goto 2
       else
3       pti=(bet*dlog(qgsrn(b10)*qgsrn(b10)))**2
       ...
       z1=(1.-zmax)**alf
       z2=(1.-zmin)**alf
4      z=1.-(z1+(z2-z1)*qgsrn(b10))**(1./alf)
       if(qgsrn(b10).gt.(z/zmax)**blf)goto 4
       wp(j)=z*sww
       wp(3-j)=amt/wp(j)
       ep(1)=.5d0*(wp(1)+wp(2))
       ep(2)=.5d0*(wp(1)-wp(2))
      ...
      if(iab.le.2.and.iaj.le.2)then
       if(sww.gt.2.d0*amk.and.qgsrn(b10).lt.dc(2))then
        bet=bek
        ami=amk**2
        ami2=ami
        ic(j)=ic(j)+3*is
        ...
        ic0=-ic(1)-ic(2)
        if(ic0.ne.0)then
         ic(j)=ic0*int(.5d0+qgsrn(b10))
         ic(3-j)=ic0-ic(j)
        else
         if(qgsrn(b10).lt..2d0)then
          ic(j)=0
          ic(3-j)=0
         else
          ic(j)=3*is-2*ic(j)
        ...
        if(iabs(ic(j)).eq.3)then
         if(iaj.eq.3)then
          if(iabs(ic(3-j)).eq.3)then
           if(sww.gt.2.d0*amlam.and.qgsrn(b10).lt.dc(4))then
            bet=bek
            ami=amlam**2
            ami2=ami
            ic(j)=6*is
         ...
         elseif(iaj.lt.3)then
          if(sww.gt.amlam+amk.and.qgsrn(b10).lt.dc(4))then
           bet=bek
           ami=amlam**2
           ami2=amk**2
           ic(j)=6*is
	 ...
	 elseif(iab.eq.4)then
      ...
      elseif(iab.eq.4.or.iaj.eq.4)then
       if(iab.eq.4)then
        ami=amk**2
        if(iaj.eq.4)then
         ic(j)=-is*int(4.5d0+qgsrn(b10))
         ic(3-j)=-ic(j)
        elseif(iaj.eq.5)then
         ic(j)=-is*int(4.5d0+qgsrn(b10))
         ic(3-j)=-ic(j)-12*is
        else
         ic0=ic(3-j)+int(.6667d0+qgsrn(b10))*(-3*is-2*ic(3-j))
         ic(j)=ic0-3*is
         ic(3-j)=ic0-ic(3-j)
        endif
       else
        if(iab.le.2)then
         ami=am0**2
         ic0=ic(j)+int(.6667d0+qgsrn(b10))*(3*is-2*ic(j))
         ic(j)=ic0-ic(j)
         ic(3-j)=ic0+3*is
        elseif(iab.eq.5)then
         ami=amc**2
         ic(3-j)=is*int(4.5d0+qgsrn(b10))
         ic(j)=-ic(3-j)+12*is
        endif
       endif
      ...
      if(ptmax.lt.0.)ptmax=0.
      if(ptmax.lt.bet**2)then
5      pti=ptmax*qgsrn(b10)
       if(qgsrn(b10).gt.exp(-dsqrt(pti)/bet))goto 5
      else
6      pti=(bet*dlog(qgsrn(b10)*qgsrn(b10)))**2
...
2     ij=ij+1
	...
	am2=am(3)
        ...
        ipj(2,njet)=int(1.5d0+qgsrn(b10))*(2*jq-3)
	am2=am(1)
       endif
...
c      external qgsrn
      ...
      elseif(iab.eq.5)then
        ich(nsh)=10*int(.5d0+qgsrn(b10))-5
...
c longitudinal momenta for the strings
      if(jp.eq.0)then
       wp1=wpi*cos(pi*qgsrn(b10))**2
      else
1      xp=.5d0*qgsrn(b10)**(2.d0/(1.d0-alpd))
       if(qgsrn(b10).gt.(2.d0*(1.d0-xp))**(-.5d0*(1.d0+alpd)))goto 1
       wp1=wpi*xp
       if(qgsrn(b10).lt..5d0)wp1=wpi-wp1
      endif
      if(jt.eq.0)then
       wm1=wmi*cos(pi*qgsrn(b10))**2
      else
2      xm=.5d0*qgsrn(b10)**(2.d0/(1.d0-alpd))
       if(qgsrn(b10).gt.(2.d0*(1.d0-xm))**(-.5d0*(1.d0+alpd)))goto 2
       wm1=wmi*xm
       if(qgsrn(b10).lt..5d0)wm1=wmi-wm1
      endif
      wpi=wpi-wp1
      wmi=wmi-wm1
c string masses
...
c-----------------------------------------------------------------------
    ...
    4 gamqgs=
     ...
     2   ((((((c(8)*z+c(9))*z+c(10))*z+c(11))*z+c(12))*z+c(13))*z+1.0)
      if(x .gt. 0.0) return
      gamqgs=3.141592653589793/(sin(3.141592653589793*x)*gamqgs)
      return
    5 write(*,10)x

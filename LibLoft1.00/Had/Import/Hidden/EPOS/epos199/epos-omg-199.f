...
c        gamV=gamV
      ...
      rho=betD(i,iclpro,icltar)
     *   +gamb+epsG


      if(m.eq.1)then

        alp=alpD(i,iclpro,icltar)
        if(i.lt.2)then
          alp=alp
     *       *exp(rho*log(spp)-b2/delD(i,iclpro,icltar))
          bet=betDp(i,iclpro,icltar)
     *         +epsGp
     *         +gamb-alppar
          betp=betDpp(i,iclpro,icltar)
     *         +epsGt
     *         +gamb-alppar
        else
          ...
          b2a=absb*absb
          alp=alp
     *       *exp((betD(i,iclpro,icltar)+epsG)*log(spp)
     *            -b2a/delD(i,iclpro,icltar))
          bet=betDp(i,iclpro,icltar)-alppar+epsGp
          betp=betDpp(i,iclpro,icltar)-alppar+epsGt
        endif

        if((bet+1.0).lt.-eps.or.(betp+1.0).lt.-eps)then
          write(*,*)'m,i,b,spp,alp,bet,betp',m,i,b,spp,alp,bet,betp
...
c        if(i.eq.0)alp=alp
     ...
     *     *exp((betD(i,iclpro,icltar)+epsG)*log(spp)
     *          -b2a/delD(i,iclpro,icltar))
          bet=betDp(i,iclpro,icltar)-alppar+1.+epsGp
          betp=betDpp(i,iclpro,icltar)-alppar+1.+epsGt
        endif
        coefgdp=utgam2(1.d0+dble(alplea(iclpro)+bet))
        coefgdt=utgam2(1.d0+dble(alplea(icltar)+betp))
        alp=alp*sngl(utgam2(dble(bet))*utgam2(dble(betp))
     &                                          /coefgdp/coefgdt)
      else

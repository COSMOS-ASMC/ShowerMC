...
c this parameter is used to fix the increase of baryon and strangeness for Tevatron
      fkaadd=0.  !additionnal effect in hadronic collision on fkappa
...
c Z dependent parameters
      ...
      zodinc=0.         !increase pbreak, diquark and strangeness in excited remnants
      zosinc= 0.        !not used
      zipinc= 1.33    !inner pt modif factor increase
      zopinc= 0.      !outer pt and end pt modif factor increase
...
c not the electron in lab frame in fake DIS
                if(.not.(iappl.eq.1.and.i.eq.2*(maproj+matarg)+1))then
 ...
 111  rexdif(iii)=abs(rexdifi(iii))
  ...
  11      z=0.19*sqrt(-2*alog(rangen()))*cos(2*pi*rangen())
      ...
      elseif(noebin.lt.0)then  !fake e-A with pi0-A for hadron production
        ...
        idproj=110
        if(model.eq.2)idproj=idtarg   !for qgsjet01, projectile and target are not symetric and we want to look what happen on the projectile side (used for CR)
        laproj=-1
        maproj=1
        engy=-1
        ekin=-1
...
c      if(line(i:j).eq.'xEmsPx')call xEmsPxNo(2,0.,0.,0,0)
     ...
     &stop'xConxyzProjTarg->xConNuclDensProjTarg'
      ...
      if(iappl.ne.0.and.(iappl+1)/2.ne.4.and.iappl.ne.3.and.iappl.ne.1)
...
c elastic event
 ...
 5    nevt=0
      ...
      if(ntry.lt.1000.and.engy.ge.egymin)then    !if no inel scattering -> nothing !
        if(model.eq.1)then
          call emsaaa(iret)
        else
        ...
        if(iret.lt.0)then       !ncol=0
          if(noebin.lt.0)then
            ...
            noevt=1   !fake DIS : if no collision, try new Q2 and Y (in ainit)
            return
          endif
          goto 2
...
c----------------------------------------------------------------------
      ...
      n1=1
      if(iframe.eq.21.and.iappl.eq.1)n1=2*(maproj+matarg+1)
      n2=nptl
      imax=itext+1
      do i=itext,1,-1
      if(text(i:i).eq.'&')imax=i
...
c     &,(xorptl(j,i),j=1,4)
 ...
 128  format (51x,65('-'))
      ...
      erest(1)=0.
      erest(2)=0.
      if(iframe.eq.22.and.iappl.eq.1)then
        i=maproj+matarg+1
...
c        cpt=chad(iclpro)*chad(icltar)
        ...
        do m=1,2
          ...
          if(isetcs.eq.0)then
            vvv11=max(0.d0,PhiExact(zz,zz,.5,1.d0,1.d0,engy**2,b1))
            vvv12=max(0.d0,PhiExact(zz,zz,.5,1.d0,1.d0,engy**2,b2))
            vvv21=max(0.d0,PhiExact(zz,zz,1.,1.d0,1.d0,engy**2,b1))
            vvv22=max(0.d0,PhiExact(zz,zz,1.,1.d0,1.d0,engy**2,b2))
          else
            vvv11=max(0.d0,PhiExpo(zz,zz,.5,1.d0,1.d0,engy**2,b1))
            vvv12=max(0.d0,PhiExpo(zz,zz,.5,1.d0,1.d0,engy**2,b2))
            vvv21=max(0.d0,PhiExpo(zz,zz,1.,1.d0,1.d0,engy**2,b1))
            vvv22=max(0.d0,PhiExpo(zz,zz,1.,1.d0,1.d0,engy**2,b2))
c           vvv11=sqrt(vvv21)
c           vvv12=sqrt(vvv21)
            if(isetcs.eq.1.and.ionudi.eq.1)then     !to test simulations
              vvv11e=max(0.d0,PhiExact(zz,zz,.5,1.d0,1.d0,engy**2,b1))
              vvv12e=max(0.d0,PhiExact(zz,zz,.5,1.d0,1.d0,engy**2,b2))
              vvv21e=max(0.d0,PhiExact(zz,zz,1.,1.d0,1.d0,engy**2,b1))
              vvv22e=max(0.d0,PhiExact(zz,zz,1.,1.d0,1.d0,engy**2,b2))
...
c Model 5 Common
!!!!      COMMON/HIPARNT/HIPR1(100), IHPR2(50), HINT1(100), IHNT2(50)  !! by KK
...
c$$$        sigdif=sigtot-sigcut-sigela       !diffractive cross section
      ...
      elseif(model.eq.5)then
        ...
        sigdif=0.
        sigcut=0.
!!        sigtot=HINT1(13)    !!  by KK
!!        sigine=HINT1(12)    !!  by KK

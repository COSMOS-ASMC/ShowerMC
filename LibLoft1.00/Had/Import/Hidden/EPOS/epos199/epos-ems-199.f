...
c --- Fix Remnant Excitation
      ...
      if(maproj+matarg.eq.2)then     !pp
        if(itpr(1).ne.0)then
          anintine=anintine+1.
          if(itpr(1).gt.0)then
       if(ionudi.eq.1.or.iep(1).ne.0.or.iet(1).ne.0.or.itpr(1).eq.1)then
     ...
     &           (iet(1).eq.0.and.iep(1).eq.2))typevt=3   !SD
        ...
        do k=1,koll
          ...
          if(aidif.ge.0..and.itpr(k).gt.0)then
            aidifp=aidifp+float(iep(ip)+2-itpr(k))
            aidift=aidift+float(iet(it)+2-itpr(k))
            if(ionudi.eq.1)then !count all diff as inelastic (to compare to tabulated cs)
              aidif=aidif+1000.
            endif
          elseif(abs(itpr(k)).eq.1)then
            aiine=aiine+1.
            aidif=-ainfin
          endif
        enddo
...
c          print *,xpr(n,k),zdiff,w1!,zdiff2
          if(ish.ge.6)write(ifch,*)'ProPoTy:itpr',itpr(k),zdiff
...
c restore x from nuclear splitting
          ...
          if(ish.ge.7)write(ifch,*)'ProPoTy:idhpr',idhpr(n,k)
     &         ,' |',ip,ivp(ip),' |',it,ivt(it)
...
c-------------------------------------------------------------------------
      ...
      double precision plc,s,xxpnew,xypnew,xxtnew,xytnew
      common/cems5/plc,s
      common/emsptl/nppr(npommx,kollmx),npproj(mamx),nptarg(mamx)
      save xxe,xye
...
c-----------------------------------------------------------------------
      ...
      double precision plc,s,p5sq,aremn,aremnex,drangen!,xmdrmax
      common/cems5/plc,s
...
c        xtmp=dble(xminremn)
     ...
     *                               ,xmax*dble(masso),xmin0
      if(koll.eq.1)xmax=min(xmax,xpz(iremo1,jremo))
      if(xmin.ge.xmax*dble(masso)-eps)then
        ...
        if(xmin0.ge.xmax-eps)then
          if(.not.force)then
            iret=1
          elseif(excited)then
            xmz(irem,jrem)=xmin0*(1d0+drangen(xmin)) !random not to form a peak
          else
 ...
 900  xmz(irem,jrem)=xxx
     ...
     &                 ,ir,irem
        ...
        elseif(xpz(irem,jrem).gt.0.d0)then
          xmz(irem,jrem)=xmin*(1d0+drangen(xmin))   !random not to form a peak
        endif
        goto 1000
      endif
...
c............................no string = resonance...................
        ...
        pptl(4,nptl)=sngl(sqrt(
     &         ept(3)*ept(3)+ept(2)*ept(2)+ept(1)*ept(1)+dble(am*am)))
        pptl(5,nptl)=am
        istptl(nptl)=0
        iorptl(nptl)=mm
        if(.not.gdrop)istptl(mm)=41

...
c----------------------------------------------------------------------
     ...
     *,optl(mxptl),uptl(mxptl),sptl(mxptl),rptl(mxptl,3)
     ...
     &       ,nsegmt(mxptl)   !,ic(2),nclk(m3xgrid)
      double precision tpro,zpro,ttar,ztar,ttaus,detap,detat
     &                ,pptld(5,mxcl),p4tmp
      ...
      logical first
      double precision ptest(4),ttest,p52,p4mean(mxcl),zor,tor,xmxms !,ppp(5),qqq(5),www(5)
      save ncntpo
      data ncntpo /0/
      ncntpo=ncntpo+1
        ...
        if(istptl(n).ne.10)then
         ...
         !                       !(already droplet decay products !)
          ...
          elseif(pt2.gt.(0.5*ptclu)**2)then
            if(rangen().lt.(sqrt(pt2)-0.5*ptclu)/ptclu)then
              if(maproj.lt.20.or.matarg.lt.20.or.ioquen.eq.0)iaaptl(n)=0
            endif
          endif
        endif
...
c...count string segments in cell
      ...
      do n=1,nptla
        if(iaaptl(n).ne.0)then
          i=1+(xptl(n)+xgrid)/delxgr
          j=1+(yptl(n)+xgrid)/delxgr
          k=1+(sptl(n)+sgrid)/delsgr
          if(  i.ge.1.and.i.le.m1grid
     &    .and.j.ge.1.and.j.le.m1grid
     &    .and.k.ge.1.and.k.le.m3grid)
...
c      !...  0 = not valid
c
c      if(ish.ge.6)write(ifch,*)'check high pt segments'
c      do n=1,nptla
c        if(iaaptl(n).eq.-1)then
c          i=1+(xptl(n)+xgrid)/delxgr
c          j=1+(yptl(n)+xgrid)/delxgr
c          k=1+(sptl(n)+sgrid)/delsgr
...
c...calculate mean energy of segments going into in clusters
     ...
     &          .and.k.ge.1.and.k.le.m3grid)then
          ...
          if(jj.gt.0)then
            ...
            p4mean(jj)=p4mean(jj)+dble(pptl(4,n))
...
c     & ,pptl(4,n),pptl(3,n),idptl(n),jj,nseg(jj),i,j,k
 ...
 95   continue
      ...
      do jj=1,jjj
        if(nsegp4(jj).gt.5)then
          p4mean(jj)=p4mean(jj)/dble(nsegp4(jj))
        else
          p4mean(jj)=1d50
        endif
      enddo
...
c...mark segments going into in clusters, count them
     ...
     &         dble(pptl(4,n)).le.100.d0/dble(ntry)*p4mean(jj))then
              istptl(n) = 3
              nseg(jj)=nseg(jj)+1
c           if(jj.eq.9)write(ifch,*)'after',n
c     & ,pptl(4,n),pptl(3,n),idptl(n),jj,nseg(jj),i,j,k
...
c              write(ifmt,*)'Rejected particle : ',n,idptl(n)
     ...
     & ,ityptl(n),(pptl(k,n),k=1,5),am2tmp
     & ,dble(pptl(4,n))/dble(nsegp4(jj)),p4mean(jj),nsegp4(jj),jj
              nsegp4(jj)=nsegp4(jj)-1
              iaaptl(n)=0
              if(nsegp4(jj).gt.5)then
                p4mean(jj)=(p4mean(jj)*dble(nsegp4(jj)+1)-pptl(4,n))
     &               /dble(nsegp4(jj))
              else
                p4mean(jj)=1d50
              endif
...
c...add segments moving towards clusters
   ...
   94   continue
        if(ihit.eq.1)then
         ...
         xn=xptl(n)+delx
         yn=yptl(n)+dely
         ix=1+(xn+xgrid)/delxgr
         jx=1+(yn+xgrid)/delxgr
     ...
     *  ,ibptl(1,n),ibptl(2,n),ibptl(3,n),ibptl(4,n))
     ...
     &      -pptld(1,mm)**2-pptld(2,mm)**2
        ...
        elseif(p52.le.0d0)then
         jerr(3)=jerr(3)+1
         pptld(5,mm)=dble(utamnu(ke(1),ke(2),ke(3),ke(4),ke(5),ke(6),5))
     ...
     &    then
          ...
          write(ifch,*)'*****',jj,nsegp4(jj),p4mean(jj)

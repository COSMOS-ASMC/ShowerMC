...
c      common /cpptr/ pptr(4,mxptl),ystr(mxptl)
      ...
      dimension ic2(2),ic1(2),ic(2)
      common/pb/pb /cnsbp/nsbp  /cn8ptl/n8ptl
      common/czz/kky,krm,kdrm,kzrm,pui,pdi,psi,pci,zzzz,itp
     &,pduui,pdudi,pdusi,pduci,pdddi,pddsi,pddci,pdssi,pdsci,pdcci
...
c       {{14,0.25},{22,0.22},{34,0.22},{91.2,0.15}} !new
        if(iappl.ne.6)zz=x0/x   !energy dependence from ratio
      endif
      ...
      if(zz.gt.0.)then
        ...
        zzz=sqrt(1.+zzz)
        pb=pb/zzz
        fkap=fkap*zzz   !fix increase of ap and K at Tevatron
      endif
c      write(*,*)'pb',kky,krm,kdrm,x,x0,pb,zz,zpaptl(1,nk1)+zpaptl(1,nk2)
      zzzz=1.
      if(isplit.eq.1)then       !for pt
        zzzz=0.
        zzz=0.
c        if(krm.eq.0)then  !both hard and soft (not good for HERA-B or pp RHIC)
        if(kky.eq.1)then   !only hard
c          zzzz=zipinc*(zpaptl(1,nk1)+zpaptl(1,nk2))
c          zzzz=sqrt(1.+zzzz)
          zz=(zpaptl(1,nk1)+zpaptl(1,nk2))
          zzzz=zipinc*zz
c          zzz=fkainc*zz
...
c        fkap=fkap*zzz
     ...
     &                         ,itp,krm,kdrm,kzrm,fkap,zzzz,pb,x
      if(fkap.le.0..or.zzzz.le.0.)
     &   call utstop("fkappa<=0. not allowed !&")

      pui=1.
      pdi=pui*exp(-pi*difud/fkap)    !assymmetric u and d relevant because of assymmetry Kch / K0 in e+e-
      psi=pui*exp(-pi*difus/fkap)
      pduui=pui*exp(-pi*difuuu/fkap)
      pdudi=pui*exp(-pi*difuud/fkap)
      pdusi=pui*exp(-pi*difuus/fkap)
      pdddi=pui*exp(-pi*difudd/fkap)
      pddsi=pui*exp(-pi*difuds/fkap)
      pdssi=pui*exp(-pi*difuss/fkap)
      if(nrflav.gt.3)then
        pci=pui*exp(-pi*difuc/fkap)
        pduci=pui*exp(-pi*difuuc/fkap)
        pddci=pui*exp(-pi*difudc/fkap)
        pdsci=pui*exp(-pi*difusc/fkap)
        pdcci=pui*exp(-pi*difucc/fkap)
      else
        pci=0.
        pduci=0.
        pddci=0.
...
c      print *,krm,kdrm,kky,zzzz,ps/(pu+pd+ps),pdiqu/psum
     ...
     &         .and.pdds.gt.0.)then
      ...
      iflb(ib1+1)=ifl
      if(ish.ge.5)write(ifch,20) ifl,pu,pd,ps,pc,pdiqu/psum
     &,pduu,pdud,pdus,pduc,pddd,pdds,pddc,pdss,pdsc,pdcc
 20   format('flavor,pu,pd,ps,pc,pdiqu:',i5,5g13.5,/

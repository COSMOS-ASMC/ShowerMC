...
c-----------------------------------------------------------------------
      include 'epos.inc'
      include 'epos.incems'
      double precision p1,esoll,ppp,seedp,psoll
      dimension p1(5),p2(3:4),p0(5,mamx+mamx),pini(mxptl)
...
c          ndif0=ndif
          diff0=diff
        elseif(abs(scal-1.).le.1e-2.and.abs(diff/psoll).lt.5e-2)then
          goto 300
        elseif(force)then
          if(ish.ge.2)
 ...
 302  if(ish.ge.1)then
      ...
      if(abs(scal)+abs(diff/psoll).gt.2.5)then

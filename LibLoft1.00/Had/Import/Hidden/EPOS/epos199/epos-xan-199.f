...
c          nepfra=nfp
     ...
     *               stop'xini: type mismatch'
            ...
            elseif(line(i:j).eq.'A+B+1')then
              ...
              if(iappl.eq.9)xmitri(ntri(nhis),nhis)=1
...
c...........check ids
     ...
     $         .and.abs(idptl(j)).lt.10000) go=.true.
            elseif(istptl(j).eq.0.and.idcod(i,n).eq.9980)then
...
c-----------------------------------------------------------------------
     ...
     *   .and.abs(idptl(npts)).ne.3331)goto 60
        ...
        yyy=-sign(1.,float(ilprtg))*yyy   !trigger on antiproton (target : ilprtg=-1 or projectile :ilprtg=1)
        if(isdif.eq.1)then
        ...
        if(idptl(npts).eq.-1120)then
          ppt=sqrt(ppt)
          if(abs(pptl(3,npts)).gt.0.)then
            theta=sign(1.,float(ilprtg))*ppt/pptl(3,npts)
            if(theta.gt.2.5e-3.and.theta.lt.4.5e-3)then
              iii3=1
c              write(ifch,*)'la',
...
60      continue
      enddo
      if(isdif.eq.1)then
        if(iii3.eq.1 .and. iii1.eq.1) isdiff=1
      elseif(isdif.eq.2)then
        if(iii3.eq.1 .and. iii1.eq.1 .and. iii2.ne.1) isdiff=1
      else
...
c put the particle in a frame where the projectile (proton) has 820 GeV (HERA)
      ...
      elseif(inom.eq.80)then    !'phi'
        ...
        if(x.lt.pi)x=x+2*pi
        if(x.gt.pi)x=x-2*pi
      elseif(inom.eq.81)then   !jorptl
        x=jorptl(j)
      elseif(inom.eq.82)then    !'ptlead' ... pt of particle with higher pt
        x=0.
        if(nint(ypara(1,n)).ne.0)x=ypara(4,n)
      elseif(inom.eq.83)then    !'mu25' ... charged ptl mult |eta|<1  pt>.4
        x=multc25
...
c         write(ifch,*)'ici',i,idptl(i),x,abs(pptl(3,i)/pmax)
      ...
      elseif(inom.eq.293)then  ! tdect : -t of leading particle
        x=0.
        i=idlead
        if(i.gt.0)then
          pmax=pnullx
      ...
      elseif(inom.eq.295)then          !'rapgap' rapidity gap
        ...
        do i=1,nptl
          if(istptl(i).eq.0)then
            ...
            if(amt.gt.0..and.pptl(4,i)+abs(pptl(3,i)).gt.0.d0)then !not correct if particles off-shell
              amt=sqrt(amt)
              rap=sign(1.,pptl(3,i))*log((pptl(4,i)+abs(pptl(3,i)))/amt)
            else
              rap=0.
            endif
            if(ityptl(i).ge.40.and.ityptl(i).lt.50)then     !proj
              rap1p=min(rap,rap1p)
            elseif(ityptl(i).ge.50.and.ityptl(i).lt.60)then !targ
              rap1m=max(rap,rap1m)
            else            !string
              rap2p=min(rap,rap2p)
              rap2m=max(rap,rap2m)
...
c            if(rap.gt.0.)then
c              if(rap.ge.rap1p)then
c                rap2p=rap1p
c                rap1p=rap
c              elseif(rap.ge.rap2p)then
c                rap2p=rap
c              endif
c            elseif(rap.lt.0.)then
c              if(rap.le.rap1m)then
c                rap2m=rap1m
c                rap1m=rap
c              elseif(rap.le.rap2m)then
c                rap2m=rap
c              endif
c            endif
          endif
        enddo
        rapmn=rap1m !max(rap1m,rap2m)
        rapmx=rap2p !min(rap1p,rap2p)
c        x=max(rap1p-rap2p,rap2m-rap1m)
        x=rapmx-rapmn
...
c            charge=0.
 ...
 124            continue 
      ...
      elseif(inom.eq.335)then                       
        x=typevt                                    !ND(1), DD(2), or SD(3)
      elseif(inom.eq.338)then    !m25evt ... multipl |eta|<2.5, pt>0.5
        x=multc25
      elseif(inom.eq.341)then                       !'mc1evt' charged particle

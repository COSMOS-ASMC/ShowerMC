...
c---------------------------------------------------------------------
 ...
 100    iq1=1+INT(nfla*rangen())
     ...
     &        (idprt(idaprt(1,i)).gt.0.and.jt.eq.1)))then
          ...
          jorprt(idaprt(3-js,i))=jorprt(i)
          jorprt(i)=idaprt(js,i)
 ...
 441  nk2=nk1+1
     ...
     &    ,iorprt(i),idaprt(1,i),idaprt(2,i),jorprt(i)
        enddo
        write(ifch,*)
        do i=1,nprtj
          if(pprt(5,i).eq.0.)
...
c          endif
     ...
     &           ,pprt(1,ij(ii)),pprt(2,ij(ii)),pprt(3,ij(ii)))
      ...
      if(ish.ge.5)then
        if(ij(1).ne.j1)then
          write(ifch,98) io,(pprt(j,io),j=1,5),pz(io)
     &    ,idprt(io)
          write(ifch,*) '->'
        endif
        write(ifch,99) ij(1),(pprt(j,ij(1)),j=1,5),pz(ij(1))
     &  ,idprt(ij(1)),'->',id(1,1),id(2,1)
        if(ij(2).ne.0)write(ifch,99) ij(2),
     &  (pprt(j,ij(2)),j=1,5),pz(ij(2))
     &  ,idprt(ij(2)),'->',id(1,2),id(2,2)
        write(ifch,*)
      endif

 98   format(i4,6g10.3,1i4)
 99   format(i4,6g10.3,1i4,a,2i4)

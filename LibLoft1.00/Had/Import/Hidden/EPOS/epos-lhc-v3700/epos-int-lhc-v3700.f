...
c----------------------------------------------------------------------
     ...
     *,optl(mxptl),uptl(mxptl),sptl(mxptl),rptl(mxptl,3)
     ...
     &,ppp(5),be(4),energ,bp,rmean(2,mxcl)         !,qqq(5),www(5)
      ...
      save visco !//////////// KK since undefined use happend at very high E
...
c     & ,pptl(4,n),pptl(3,n),idptl(n),jj,nseg(jj),i,j,k
 ...
 95   continue
     ...
     &     *(p4mean(4,jj)-p4mean(3,jj))-p4mean(1,jj)**2-p4mean(2,jj)**2
        if(amc2.gt.0)amctot=amctot+sqrt(amc2)
      ...
      if(iLHC.eq.1.and.amctot.gt.0.)then
        do ip=1,maproj
          ...
          enddo
        enddo
        amctot=amctot/amuseg
        if(amctot.gt.1.)then
          visco=1.
          yrmaxi=yradmx
c          if(fvisco.gt.0)yrmaxi=yrmaxi*(1.-visco)
!!!!!!!!!!!    KK Mar.17,2014  KK  (later dble in exp)
...
c          yrmaxi=yrmaxi*amctot**yradpi
     ...
     &         ((sinh(yyrmax)*yyrmax-cosh(yyrmax)+1d0)/(yyrmax**2/2d0)))
        ...
        if(ylongmx.lt.0.)then
          delzet=
     * abs(ylongmx)*log(exp(dble( yradmi/abs(ylongmx)) )+amctot-1.)
...
c        rho=amctot/volu
        ...
        if(iLHC.eq.1.and.visco.gt.0.)then   !///////KK  visco ;undefined happend
          if(npair.gt.0)then
...
c works but with large (negative) fplmin and effect a bit strong on very heavy particles .... and problem giving very large pt (probably because of large pz some times)
...
c implified version with yradpp~proton mass (0.9) but fplmin=-5 (small bump at y=0)
c                bet=bet+sqrt(pt2+min(yradpp,pptl(5,ii))**2)
...
c                        ytmp=yrad2(i)
     ...
     &   *exp(dble( -fplmin*max(0.,pt**2-(pe(4)/tecm)**2)) ) )
...
c                  pt=abs(fplmin)*pt
     ...
     *                 /(1./sqrt(pptl(5,j))+pow))
                ...
                if(pow.lt.0.)then
                  scal0=1./((1.-pow)
     *              *exp( dble( -0.25*max(-4.,log(rangen()))) )  )
                  pow=1.

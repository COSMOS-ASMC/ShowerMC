...
c     kinddecay : (See the comments in following code. Too many)
      ...
      include '../include/atmnc-particle-code.inc' ! particle code of atmn
      include '../include/atmnc-particle-mass.inc' ! particle mass

      integer positive, negative
      parameter(positive=+1, negative=-1)

      integer*4 kin, nchild, kchild(3), nextc(3)
      real*8 pmother(5), pchild(5,3), spinmu(5), zeta(3)
...
c                           get decay particle at rest frame of mother
      ...
      if(kin.eq.kmup .or. kin.eq.kmum) then
                                !                 mu -> numu nue e
         call mudecay(kin, pmother, polmu, nchild, kchild, pchild,
     &        kinddecay) 
      else if(kin.eq.kpip .or. kin.eq.kpim .or. kin.eq.kpi0) then
                                !           pic --> mu + nu
                                !           pi0 --> g + g
        call pidecay(kin, pmother, nchild, kchild, pchild, polmu0,
     &     kinddecay)
         ...
         call k0sdecay(nchild, kchild, pchild, kinddecay)
                                ...
                                !           K0l -> 3l nu, 3pi
         call k0ldecay(nchild, kchild, pchild, polmu0, kinddecay)
      else if(kin.eq.kkp) then
                                !     K+ -> mu+nu, 3lnup, 3pi
         call kcdecay(positive, nchild, kchild, pchild, polmu0, 
     &        kinddecay)
      else if(kin.eq.kkm) then
                                !     K- -> mu-nu, 3lnup, 3pi
         call kcdecay(negative, nchild, kchild, pchild, polmu0,
     &        kinddecay)
      else
      ...
      if(nchild.gt.3) then
         stop ' too many child in kdecay'
                                ...
                                ! lorenz boost to mother particle frame
 ...
 11      continue
         ...
         if(kchild(i).eq.kmup .or. kchild(i).eq.kmum) then
            rp = sqrt(p1(1)**2 + p1(2)**2 + p1(3)**2)

            if(polmu0 .gt. 0.99999d0) then
c   Complete polarized muon in mother's rest: by Hayakawa formula
               rp0 = sqrt(p0(1)**2 + p0(2)**2 + p0(3)**2)
               polmu = (p0(4)*p1(4))/(rp*rp0)
     &              - (pmother(4)/pmother(5))*aMmu**2/(rp*rp0)
            else
c   General case: sample spin in mother's rest --> boost --> polmu in lab
               call samplespin(polmu0, p0, spinmu0)
               call lboost(telement, spinmu0, spinmu)
               polmu = spinmu(4)*aMmu/rp
            end if

cc original calculation method for polmu
c            if(polmu0 .gt. 0.99999d0) then
c               call samplespin(polmu0, p0, spinmu0)
c            end if
c               call lboost(telement, spinmu0, spinmu)

c            call spin2zeta(spinmu, p1, zeta)
c            rz = sqrt(zeta(1)**2 + zeta(2)**2 + zeta(3)**2)
c            polmuO = (p1(1)*zeta(1)+p1(2)*zeta(2)+p1(3)*zeta(3))/(rp*rz)

c            write(*,'(a,3I3,10F12.5)')'test_polmu',
c     &           kin, kchild(i), kinddecay,
c     &           pmother(4), p1(4), polmu, polmuO
         end if
 1    continue
      end
     ...
     &     kinddecay)
      ...
      include '../include/atmnc-particle-code.inc' ! particle code of atmnc
      include '../include/atmnc-particle-mass.inc' ! particle mass
...
c         2)  ---->pi0 + pi0       31.39  (kinddecay=2)
 ...
 201     continue
      ...
      subroutine k0ldecay(nchild, kchild, pchild, polmu,
     &     kinddecay)
      implicit real*8 (a-h,o-z)
...
c            k0l decay
c  -- process --
c         1)  ---->e  pi nue    38.7 %(kinddecay=1)
c         2)  ---->mu pi numu   27.1  (kinddecay=2, k0==>mu+, k0bar==>mu-)
...
c     spin is sampled in decaycontrol
c         call samplespin(polmu, p3, spinmu)
      ...
      subroutine kcdecay(motherc, nchild, kchild, pchild, polmu,
     &     kinddecay)
      implicit real*8 (a-h,o-z)
...
c            k+- decay
c  -- process --
c         1) k---->mu+nu          63.5 % (kinddecay = 1)
c         2)  ---->pic + pi0      21     (kinddecay = 2)
c         3)  ---->pi0+e+nu        4.8   (kinddecay = 3)
c         4)  ---->pi0+mu+nu       3.2   (kinddecay = 4)
...
c         6)  ---->pic+pi0+pi0     1.7   (kinddecay = 6)
 ...
 101     continue
         ...
         polmu = 1.0d0
c     spin is sampled in decaycontrol, (actually, Hayakawa formula is applied)
c         call samplespin(polmu, p2, spinmu)
...
c         call samplespin(polmu, p3, spinmu)
      ...
      subroutine mudecay(kmuin, pmu, polmu, nchild, kchild, pchild,
     &     kinddecay) 
      implicit real*8 (a-h,o-z)

      include '../include/atmnc-particle-code.inc' ! particle code of atmnc
      parameter (aM=0.0d0) ! all masses are 0 but for muon

c control muon decay:
c inputs :
c     kmuin (should be kmup(15) or kmum(16))
c     pmu (1-5) momentum energy, mass
cc obsolute  smu (1-5) spacial spin vector of muon 
cc     for mu-, ture direction, but for mu+ inverse direction.
cc     I.E. in both case, nue tends to run toward 'smu' direction
c     polmu : muon polarization.
c outputs :
c     nchild : number of decay particle (=3, fixed)
c     kchild(3) : kind 
c     pchild(3) : momentum
...
c      call spin2zeta(smu, pmu, zeta)
      call sample_angvec(polmu, pmu, zeta) ! zeta is generated from "polmu"
...
c **Sample muon spin direction for given p and polarization
 ...
 14      continue
                                 ...
                                 !      if costeta != 1, then 
                                 !      1: Sample a vector perpendicular to dirp
         ru =0.0
         do 1 while(ru.lt.1e-3)
                                 !         1-1: sample a vector inside of 
                                 !               a unit sphere
            rr=2.0
 ...
 3          continue
            ...
            do 2 i=1,3
               ...
               ru   = ru + u(i)**2
 2          continue
                                 !         1: Resulted vector shouldn't be zero 
                                 !             (see condition in 'do 1 while')
 1       continue
...
c            Enue = ((aMmu**2 - aMe**2)/2/aMmu)*x
      ...
      subroutine pidecay(kpiin, ppi, nchild, kchild, pchild, polmu,
     &     kinddecay)
      implicit real*8 (a-h,o-z)
...
c     kinddecay: 1 mu + nu (pic), 2:  g + g (pi0)
      ...
      if(kpiin.eq.kpip .or. kpiin.eq.kpim) then
         ...
         polmu = 1.0d0
c  spin is sampled in decay control (actually Hayakawa formula is applied)
c         call samplespin(polmu, p2, spinmu)

...
c     kchild(3) : kind
c !    pchild(3) : momentum ! Now spin vector never go out.
...
c     kinddecay : (See the comments in following code. Too many)
      ...
      real*8 pchild(5,3), zeta(3) !, spinmu(5)
      real*8 p0(5), p1(5), telement(5) !, spinmu0(5)
c      dimension tel2(5), spinmu1(5), spinmu2(5)

      common /muonpol/ pol

c      save pol ! muon polarization is stored inside of this routine !
c      write(*,'(a,I5,5F10.5)')'decay  in',kin,pmother

c                           get decay particle at rest frame of mother

      if(kin.eq.kmup .or. kin.eq.kmum) then
c                                                 mu -> numu nue e
         call mudecay(kin, pmother, pol, nchild, kchild, pchild,
     &        kinddecay) 
      else if(kin.eq.kpip .or. kin.eq.kpim .or. kin.eq.kpi0) then
c                                                 pic --> mu + neu
c                                                 pi0 --> g + g
        call pidecay(kin, pmother, nchild, kchild, pchild, zeta,
     &     kinddecay)
...
c                                           K0l -> 3l nu, 3pi
         call k0ldecay(nchild, kchild, pchild, zeta, kinddecay)
      else if(kin.eq.kkp) then
c                                     K+ -> mu+nu, 3lnup, 3pi
         call kcdecay(positive, nchild, kchild, pchild, zeta,
     &        kinddecay)
      else if(kin.eq.kkm) then
c                                     K- -> mu-nu, 3lnup, 3pi
         call kcdecay(negative, nchild, kchild, pchild, zeta,
     &        kinddecay)
      else
...
c                           lorenz boost to mother particle moving frame
 ...
 11      continue
         ...
         if(kchild(i).eq.kmup .or. kchild(i).eq.kmum) then
            pp = sqrt(p1(1)**2 + p1(2)**2 + p1(3)**2)
c     This polarization will be used imediately after (for next mu)
            pol = (zeta(1)*p1(1) + zeta(2)*p1(2) + zeta(3)*p1(3))/pp
         end if
c      write(*,'(a,I5,5F10.5)')'decay out',kchild(i),(pchild(j,i),j=1,5)
 1    continue
...
c                                            2)  ---->pi0 + pi0       31.39
 ...
 201     continue
      ...
      subroutine k0ldecay(nchild, kchild, pchild, zeta,
     &     kinddecay)
      implicit real*8 (a-h,o-z)
      include '../include/atmnc-particle-code.inc' ! particle code of atmnc
      include '../include/atmnc-particle-mass.inc' ! particle mass
...
c    pchild(5,n) : 1-3 momentum, 4 energy, 5 mass of child particle
c !!   spinmu(5) : if mu exist in child, give 4-dim spin vector(5=0) ! No More
c    zeta(3) : instead above 
c            k0l decay
c  -- process --
...
c         4)  ---->pi+ pi- pi0  12.4  (kinddecay=4)
      integer*4  kchild(3)
      real*8 pchild(5,3), p1(5), p2(5), p3(5), zeta(3) ! spinmu(5)
...
c         call samplespin(pol, p3, spinmu)
         call sample_angvec(pol, p3, zeta)
...
c                                                  4)  pi+ pi- pi0
      ...
      subroutine kcdecay(motherc, nchild, kchild, pchild, zeta,
     &     kinddecay)
      implicit real*8 (a-h,o-z)
      include '../include/atmnc-particle-code.inc' ! particle code of atmnc
      include '../include/atmnc-particle-mass.inc' ! particle mass
...
c                                            1: numu, 2:mu
 ...
 101     continue
         ...
         pol = 1.0d0
c         call samplespin(pol, p2, spinmu)
         call sample_angvec(pol, p2, zeta)
...
c                                             6)  ---->pic+pi0+pi0     1.7
      ...
      subroutine mudecay(kmuin, pmu, pol, nchild, kchild, pchild,
     &     kinddecay) 
      implicit real*8 (a-h,o-z)
...
c      call spin2zeta(smu, pmu, zeta)
      call sample_angvec(pol, pmu, zeta)
c      write(*,*) 'zeta mudecay', zeta
...
c            Enue = ((aMmu**2 - aMe**2)/2/aMmu)*x
      ...
      subroutine pidecay(kpiin, ppi, nchild, kchild, pchild, zeta,
     &     kinddecay)
      implicit real*8 (a-h,o-z)
c     control pi-decay:
c   input,
...
c     kchild(nchild) : kind of child particles
c     pchild(5,nchild) : 1-3 momentum, 4 energy, 5 mass of child particle
c   !!  spinmu(5) : muon spin if there is muon in children !! No More
c   !!  spin direction for mu+ and antispin direction for mu- !!
c     zeta : polarization vector in muon rest frame
c      I.e. zeta represent the direction for which numu and e are 
c      emitted less energetically, when mu decays.
c     kinddecay: 1 mu + nu (pic), 2:  g + g (pi0)
      ...
      integer*4 n, kchild(3) 
      real*8 pchild(5, 3), zeta(3), p1(5), p2(5) ! spinmu(5)
...
c
       real*8 pi, hpi, gn1
       integer i
       include '../include/atmnc-constants.inc'
c       parameter (pi=3.14159265d0, hpi=pi/2)
       parameter (hpi=pi/2)
c
          gn1=1.
          do   i=1, n-2

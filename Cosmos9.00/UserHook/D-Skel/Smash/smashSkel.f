...
!        open  output smashed skeleton files
     ...
     *    basename(1:klena(basename))//'00003 etc' 
      call cerrorMsg(msg, 1)
...
!            input may be like:      1  hosta   2.5
            ...
            if(  ii  .gt. Ncpu .or. ii .lt. 1 ) then
               write(0,*) i,  '-th line has invalid number=',
     *            ii, ' in ThinHosts'
               stop 0000
            endif
            numba(ii) = -ii
...
!////////////
!                  write(0,*) ' cpu #=',i, 
!     *            ' Zfirst=',Zfirst.pos.depth*0.1,'g/cm2',
!     *            ' ir=', ir
!//////////////////
               else
     ...
     *      'It is too large; enlarge Maxob', 0)
         ...
         do i = 1, Np
            read(11) oo(i)
         enddo
         write(0,*) ' observed ptcles have been read'
         nlow = 1
         ctc=0
         do while (nlow .ge. 0)
            read(11) nlow, pp
...
!///////////
     ...
     *      form="unformatted",      iostat=icon)
            if(icon .eq. 0) then
!/////////////
               write(0,*) ' cpu ',i, ' cpuc=',cpuc, ' obs=',navob
!//////////////
...
!c         endif
!////////////////
            write(0,*)' i=', i,  ' nOnCpu(i)=',nOnCpu(i)
!            j = 1
!            write(0,*)'j=1 code=',ct( idxlist(j, i)).p.code,
!     *      ' where=', ct( idxlist(j, i)).where
!///////////////////

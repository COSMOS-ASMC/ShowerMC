...
!                   varialbe NRFAIFILE
!                   each .dat file    main output from sim. 
!                   give it by env. vari.  DATFILE
!       output file:  size reduced file;  stdout.  redirect toa >>  .dat (for combined .dat)
!                   and nrfaifile for that; -r is attached. 
!                    you must combine .nrfai-r files; rename each .nrfai
!                    and use assemNrfai.csh in Assemble.
!             "all" in .nrfai-r is really "all", so  they should not be 
!              added any molre in assemNrfai.csh
!
...
#include "../FleshHist/Zprivate0.h"
     ...
     *              wx, wy, wz, wgt, wwgt
      ...
      character*3 id
      character*5 id5
      integer fnonrfai, fnonrfai2, fnodat, fnodat2
...
!      fnodat2 = 8
      ...
      call copenfw2(fnodat, datfile, 1, icon)
      if(icon .ne. 1) then
...
!      call copenfw2(fnodat2, datfile2, 1, icon)
!      if(icon .ne. 0) then
!         write(0,*) ' error ', datfile2, ' cannot be created'
!         stop 1236
!      endif
      nrfaifile2 =" "
...
!
     ...
     *           maxsites, limit
            ...
            write(0,*) ' EvNo0=',EvNo0, 
     *      E0,NN, cosz, Emin,  nrbina, nfaia, ansites0, maxsites,
     *      limit
...
!      read(fnodat,'(a)') input0
!      write(fnodat2,'(a)') input0(1:klena(input0))
!         skipe header
!      write(*,'(a)') input0(1:klena(input0))
      nrec= 0
      do while(.true.)
         read(fnodat,'(a)', end=100, Err=500) input0

...
#include "../FleshHist/Zprivate3.h"
     ...
     *        charge, ridx, faiidx, codex
      ...
      character*160 Lfile(ndepth)
      character*80  baseDir
      integer lengBaseDir
      character*32  execID
      integer  lengExecID
      character*3 id
...
!      fnodat2 = 8
      ...
      lengBaseDir=kgetenv2("BASEDIR", baseDir)
      if(lengBaseDir .eq. 0) then
         write(0,*) 'BASEDIR not given' 
         stop 8888
      endif
      lengExecID=kgetenv2("EXECID", execID)
      if(lengExecID .eq. 0) then
         write(0,*) 'EXECID not given'
         stop 9999
      endif
...
!      endif
!
!        open files with -Lxx.dat (xx=recl0)
!
      do i = 1, ansites0
         if(recl0(i) .gt. 0) then
            write(Lfile(recl0(i)),'(a,"/", a, "-L",i2.2,".dat")') 
     *        baseDir(1:lengBaseDir), execID(1:lengExecID), recl0(i)
            open(100+recl0(i), file=Lfile(recl0(i)), access="append" ,
     *              form="formatted",   iostat=icon)
            if(icon .ne. 0) then
               write(0,*) Lfile(recl0(i)), " could not be opend" 
               stop 9999
            endif
         endif
      enddo

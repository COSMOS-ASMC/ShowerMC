...
CPP="ifort -DPCLinuxIFC64 -P -E "
FC="ifort -DPCLinuxIFC64 -Vaxlib -align dcommons "
#CC="icc -DPCLinuxIFC64 "
CC="cc"
...
if [ $mode -eq 1 ];then
    ...
    ifort -Llib chook.o -o cosmosPCLinuxIFC64 -lcosmos
    ...
    if [ -f ${WKD}/cosmosPCLinuxIFC64 ];then
	mv ${WKD}/cosmosPCLinuxIFC64 ${WKD}/cosmosPCLinuxIFC64_PGO
    fi

    cd ${WKD}/src
    $FC -prof-use -prof-dir $WKD -O3 -ipo $INC -c *.f
    $FC -prof-use -prof-dir $WKD -O3 -ipo $INC -o ${WKD}/cosmosPCLinuxIFC64 *.o
    cd $WKD
...
#    $FC -O0 $INC -c *.f
#    $FC -O2 $INC -c *.f
#     $FC -O2 -static -ipo   $INC -c *.f   ! ng
#    $FC -O2 -static -ipo_separate   $INC -c *.f   NG

#   next time for 100 events p 1TeV: real 3m57.837s     user3m57.321s
#    $FC -fast   $INC -c *.f   
#        next   1 event:  real8m54.234s
#    $FC -O1 -static -ipo   $INC -c *.f   
#     100 events      normal -O0 compile  4m46.732s/4m29.192/4m15.280s
#                      (3 times);  with Jam < 100 GeV.  5min4sec
#     100 evetns: next. real 4m24.178s 
    $FC -O0 -static    $INC -c *.f   
    mv chook.o ${WKD}
    ar r ../lib/libcosmos.a *.o
    cd $WKD
    ...
    echo \$ ifort -O0 -Llib chook.o -o cosmosPCLinuxIFC64 -lcosmos

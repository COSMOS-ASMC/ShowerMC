...
!         if this is made 1, this routine will never be called.
!         make never 5 if  the event is not good for you
!         and retry the event generation.
...
!
      ...
      if(  IntInfArray(ProcessNo).process /= 'coll' ) then  ! even if this is absent, OK
         return                                             ! ;inside cutptcl same is done
      endif

!       first make Z axis as the incident particle
      do i = 1, Nproduced
         call cirot3vec(i, MovedTrack.p.fm, Pwork(i).fm, Pwork(i).fm)
      enddo

!           make projectile move +z direction
!        with the same energy as MovedTrack.p
      proj =  MovedTrack.p
      pz =sqrt( dot_product(proj.fm.p(1:3), proj.fm.p(1:3)) )
      proj.fm.p(1:3) =(/0.d0, 0.d0, pz/)
      call cutptcl(proj)

!          again convert Z axis to be the one where
!       MovedTrack is defined
      do i = 1, Nproduced
         call crot3vec( MovedTrack.p.fm, Pwork(i).fm, Pwork(i).fm )
      enddo
      never = 0
      end

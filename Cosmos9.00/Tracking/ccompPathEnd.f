...
!!!   unable  it for normal jog. See UC/Temp/Readme
!!!
!     ccompPathEnd
...
!                may take place occasionally
          ...
          if( NoOfMedia > 1 ) then
!              see if goes into diff. medium. before MovedTrack's pos 
...
!     move the current TrackBefmove is moved by leng m and
!     store the updated one in MovedTrack
!     After this is called,  MovedTrack has info. for moved pos. 
!     MovedTrack.pos.where may not be correct. It must be
!     corrected by seeing if the track passes acrosss a
!     observation layer or entering new media region or
!     acrosses such region.
       implicit none
...
#include  "ZmediaLoft.h" 
!     
       real*8  cvh2den, dedt, dedtF,  thick, leng, cupsilon, Eloss
       save dedt, dedtF
       real*8  csyncTELoss
       integer jcut
...
!            
!     dE/dX
!     <<< moved to inside after cpop in ctracking.f       
!       [rho]= kg/m3 =  1000 g/10^6 cm3 = 10-3 g/cm3
!
!       rho = cvh2den(TrackBefMove%pos%height) * 1.0d-3  ! g/cm3
!
!       Media(MediaNo)%rhoc = rho/Media(MediaNo)%rho
!     rhoc is used to get rho inside epdedxe for the dendity
!     effect in dE/dx; ==> rhoc is not touched now
!       instead rrho is used.       
!       >>>>     

!         dedt ( restricted dE/dt (GeV/(g/cm2))
!         dedtF ( full dE/dt including knockon; analytic one)
...
!              dedt, dedtF are in GeV/(g/cm2).
!     we must convert it to  GeV/(kg/m2)  unit.
!      Since (g/cm2) = 10 (kg/m2)       
!     GeV/(g/cm2) = GeV/( 10(kg/m2) )so   X GeV/(g/cm2) is
!         X/10 (GeV/(kg/m2)).       
       dedt= dedt/10.
       dedtF = dedtF/10.
...
!     this is for multiple scattering
          call celecDef(dispx, dispy) ! dispx, y are dummy.         effect alrady put in ! MovedTrack. 
       endif
       if(.not. nodef) then
...
!                                               same as cbDefByRK
     ...
     *          leng/2 * TrackBefMove%vec%w%r(i)
          ...
          call cgetMagF( YearOfGeomag,  middle,
     *                tempmag, icon)
...
!     call cmulScat(theta)
     ...
     *     IntInfArray(ProcessNo)%thickness*0.1d0, ! g/cm2
     *     Media(MediaNo), theta)
...
!     of nodeX-th layer.  (the bottom of the layer, irrespectively of
!      upgoing or down going.  
      real(8),intent(out):: Rx(3) !  x-sing point. should be in the
!     nodeX node.
      real(8),intent(out):: dir(3) ! diection cose of TrackBefMove to
            !       Movedtrack
...
!        atmos%z(node+1)>  vh>= atmos%z(node) 
!     see if there is  diff medium from that of the present node.
!     Get the nearest such node ; nodeB(below) and  nodeU(upper)
!     if 0, non existent      
...
!     where is xing point with nodeB; x-point should be
!     upper bound of nodeB so that we must see the boundary at nodeB+1
            Hx= atmos%z(nodeB+1) + Eradius !  radius at the top of
!                 candidate node
     ...
     *           Hx, Rx(:), len, tobecut)
         ...
         elseif( nodeU > 0 ) then
!          rare case but possible. down going first but might cross with
!          an upper node.  so Rx is on  the bottom of nodeU.
            Hx= atmos%z(nodeU) + Eradius !
            call cgetNodeXP(TrackBefMove%pos%xyz%r(:),
     *           TrackBefMove%pos%radiallen, MovedTrack%pos%xyz%r(:),
     *           Hx,  Rx(:), len, tobecut)
            if( tobecut == 0 ) then
      ...
      else  ! upgoing or horizontal
         if( nodeU > 0 ) then
!            seems ptcl goes upper node
            Hx= atmos%z(nodeU) + Eradius ! radius of bottom of the
!     candidate node
            call cgetNodeXP(TrackBefMove%pos%xyz%r(:),
     *           TrackBefMove%pos%radiallen, MovedTrack%pos%xyz%r(:),
     *           Hx, Rx(:), len, tobecut)
         ...
         elseif( nodeB > 0 ) then
            ...
            Hx= atmos%z(nodeB+1) + Eradius ! radius of candidate node
            call cgetNodeXP(TrackBefMove%pos%xyz%r(:),
     *           TrackBefMove%pos%radiallen, MovedTrack%pos%xyz%r(:),
     *            Hx, Rx(:), len, tobecut)
            if( tobecut == 0 ) then
               if( nwarning  < 10 ) then
                  ...
                  write(0,*)
     *                 'very rare;  neglect the ptcl: code, p=',
     *             MovedTrack%p%code,  MovedTrack%p%fm%p(1:4)
!     !                  stop
               endif
            endif
         endif
      ...
      if( tobecut == 0 ) then
!!         if( len > 1.d-3 ) then  ! seems ok Jul.24/2019
...
!     subroutine cgetNodeXP(R1, H1, R2, H2,  Hx, Rx, len, icon)
      subroutine cgetNodeXP(R1, H1, R2, Hx, Rx, len, icon)      
      implicit none
!     Earth x-y-z system
      real(8),intent(in):: R1(3) ! postion vector  (abs(R1) = R0 + h1= H1)
                                 !  TrackBefMove%pos%xyz%r(3)
      real(8),intent(in):: H1   !  R0 + h1  (h1 vertical height)
...
!     (if exist)
      ...
      real(8),intent(out):: len !  length from R1 to Rx.  (m)
!       len is bit larger than exact one to force the  xp is in  side the new media
      integer,intent(out):: icon ! 
          ! output:    if Rx is obtained 0. else 1.
!     ===================
...
!     Defaults are rel=F, eps=1.0d-3 (0.1cm).
!      
      logical,save:: relSave=.false. ! epsSave is  absolute value
      real(8),save:: epsSave=5.0d-4   !in m   0.05 cm
...
!        update it
      ...
      MoveStat = Truncated
      if( TrackBefMove%p%charge /= 0) then       
...
!     path is shorter than to MovedTrack
         if(MovedTrack%p%fm%p(4) .lt. MovedTrack%p%mass) then
!     very low energy; and media is from  air to  high density one
!     EnergyLoss estimation becomes wrong. (too big). And this happens.
!     We kill this particle here with adjusted EnergyLoss
            if( nwarning < 10) then
               write(0,*) 'E < m; bef KE=',
     *           TrackBefMove%p%fm%p(4) - MovedTrack%p%mass
               write(0,*) ' len cut=', len
               write(0,*) ' bef cut length=',
     *             IntInfArray(ProcessNo)%length
               write(0,*) 'Energy loss=', EnergyLoss
...
!            MovedTrack%p%fm%p(:)=(/0.,0.,0.,0./)
            ...
            EnergyLoss =  TrackBefMove%p%fm%p(4) - TrackBefMove%p%mass
            MovedTrack%p%fm%p(1:3) = 0.
            MovedTrack%vec%w%r(1:3) =(/0.,0.,1./)
         else
            call ce2p(MovedTrack)
         endif
      endif
